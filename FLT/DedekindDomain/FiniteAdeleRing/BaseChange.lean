...
noncomputable local instance adicCompletion_nontriviallyNormedField :
    NontriviallyNormedField (adicCompletion K v) :=
  Valued.toNontriviallyNormedField (hv := adicCompletion_RkOne A K v)
...
theorem baseChange_of_algebraMap_adicComletion_surjective (v : HeightOneSpectrum A)
  ...
  intro x ⟨l, hl⟩
  use (1 ⊗ₜ l)
  simp [f, baseChange_of_algebraMap_adicComletion, ← hl]
  rfl
...
theorem comap_algebra_finite (v : HeightOneSpectrum A) (w : HeightOneSpectrum B)
  ...
  exact baseChange_of_algebraMap_adicComletion_surjective A K L B v w hvw
