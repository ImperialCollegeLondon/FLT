...
omit [TopologicalSpace M] [IsModuleTopology R M] [TopologicalSpace N] [IsModuleTopology S N] in
...
lemma continuous_incl1 : @Continuous M (M × N) (moduleTopology R M) (moduleTopology (R × S) (M × N))
  ...
  refine sInf_le ?_
  constructor
  · exact induced_continuous_smul R S M N
  · exact induced_continuous_add R S M N
...
lemma id_eq : @id (M × N) = ((incl1 M N) ∘ (Prod.fst)) + ((incl2 M N) ∘ (Prod.snd)) := by
  ext a
  all_goals simp
