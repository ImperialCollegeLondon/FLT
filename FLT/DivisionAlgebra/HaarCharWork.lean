...
noncomputable
def basis_change_map :
    Module.Basis (Fin (Module.finrank ℝ R)) ℝ R → (Fin (Module.finrank ℝ R) → R) :=
  fun B i =>
    if i = Classical.choose (ex_ne_zero_coeff_one R B)
    then B i - (∑ x, B x - 1)
    else B i

omit [Module.Finite ℝ R] [Module.Free ℝ R] [TopologicalSpace R] [IsModuleTopology ℝ R] in
lemma basis_change_map_spans (B : Module.Basis (Fin (Module.finrank ℝ R)) ℝ R) :
    ⊤ ≤ Submodule.span ℝ (Set.range (basis_change_map R B)) := by
  obtain ⟨a, ha⟩ : ∃ a, Classical.choose (ex_ne_zero_coeff_one R B) = a := by
    exact exists_eq'
  suffices h : B a ∈ Submodule.span ℝ (Set.range (basis_change_map R B)) by
    have (i : Fin (Module.finrank ℝ R)) :
        B i ∈ Submodule.span ℝ (Set.range (basis_change_map R B)) := by
  ...
  have : ((B.repr 1) a) ≠ 0 := by
    simpa [← ha, ← Module.Basis.coord_apply] using Classical.choose_spec (ex_ne_zero_coeff_one R B)
...
omit [Module.Finite ℝ R] [Module.Free ℝ R] [TopologicalSpace R] [IsModuleTopology ℝ R] in
lemma basis_change_map_lin_independent (B : Module.Basis (Fin (Module.finrank ℝ R)) ℝ R) :
    LinearIndependent ℝ (basis_change_map R B) := by
  apply linearIndependent_of_top_le_span_of_card_eq_finrank
  · exact basis_change_map_spans R B
  · exact Fintype.card_fin (Module.finrank ℝ R)

/-- For any basis `a`, `basis_change_map a` is a basis. -/
noncomputable
def basis_change_map_is_basis (a : Module.Basis (Fin (Module.finrank ℝ R)) ℝ R) :
    Module.Basis (Fin (Module.finrank ℝ R)) ℝ R :=
  Module.Basis.mk (v := basis_change_map R a) (basis_change_map_lin_independent R a)
    (basis_change_map_spans R a)

omit [TopologicalSpace R] [IsModuleTopology ℝ R] in
lemma ex_basis_sum_basis_eq_one :
    ∃ b : Module.Basis (Fin (Module.finrank ℝ R)) ℝ R, ∑ x, b x = 1 := by
  use basis_change_map_is_basis R (Module.finBasis ℝ R)
  obtain ⟨a, ha⟩ : ∃ a, Classical.choose (ex_ne_zero_coeff_one R (Module.finBasis ℝ R)) = a := by
    exact exists_eq'
  have h : {a} ∪ ({i | i ≠ a} : (Finset (Fin (Module.finrank ℝ R)))) =
      Set.univ (α := (Fin (Module.finrank ℝ R))) := by
    ext i
...
abbrev iso : R ≃ₜ+ (Fin (Module.finrank ℝ R) → ℝ) where
  toFun := Module.Basis.equivFun (Classical.choose (ex_basis_sum_basis_eq_one R))
...
lemma ringHaarChar_eq (y : (Fin (Module.finrank ℝ R) → ℝ)ˣ)
  ...
  apply MeasureTheory.addEquivAddHaarChar_eq_addEquivAddHaarChar_of_continuousAddEquiv (iso R).symm
  intro x
  obtain ⟨a, ha⟩ := hy
  have h1 : (iso R).symm x = ∑ j, x j • (Classical.choose (ex_basis_sum_basis_eq_one R)) j :=
    Module.Basis.equivFun_symm_apply _ _
  have h2 : (iso R).symm (↑y * x) =
      ∑ j, (↑y * x) j • (Classical.choose (ex_basis_sum_basis_eq_one R)) j :=
    Module.Basis.equivFun_symm_apply _ _
  have : a • (∑ x, (Classical.choose (ex_basis_sum_basis_eq_one R)) x) =
      (∑ x, a • (Classical.choose (ex_basis_sum_basis_eq_one R)) x) := Finset.smul_sum
  simp_rw [ContinuousAddEquiv.mulLeft_apply, IsUnit.unit_spec, h1, h2,
    ...
    Classical.choose_spec (ex_basis_sum_basis_eq_one R), one_mul, Finset.smul_sum,
    ← IsScalarTower.smul_assoc, smul_eq_mul] -- I perhaps could have oversqueezed this lemma

end ringHaarChar.Module
