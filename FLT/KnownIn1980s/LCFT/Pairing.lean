...
variable (K M : Type u) [Field K] [ValuativeRel K] [TopologicalSpace K]
...
noncomputable def Representation.multiplicative :
    Representation ℤ (Field.absoluteGaloisGroup K) (Additive (AlgebraicClosure K)ˣ) where
  toFun g := {
...
noncomputable abbrev LocalGaloisRep.dual : TopRep ℤ (Field.absoluteGaloisGroup K) :=
  iHom (.of ρ) (mkDiscrete (Rep.of (Representation.multiplicative K)))
...
noncomputable def pairing (i j : ℕ) (hij : 2 = i + j) :
    continuousCohomology i (.of ρ) ⟶ TopModuleCat.linHom (k := ℤ)
      ...
      (mkDiscrete (Rep.of (Representation.multiplicative K)))) :=
