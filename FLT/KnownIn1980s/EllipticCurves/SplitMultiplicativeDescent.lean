...
`j ↦ j(tateCurve q)`. This rests on the discriminant product formula `tateCurve_Δ`
(the `q`-expansion of `Δ = η²⁴`, imported as a sorry from FLT PR #1099) plus routine
evaluation bookkeeping (`tateCurve_c₄`, `evalInt_subst`, `evalInt_jInv`, `j_tateCurve_q`).
...
and `exists_variableChange_tateCurve_charUniform` hold with **no** hypothesis on the
...
/-! ## Evaluation-on-the-disc helpers and the Tate discriminant

`evalInt q F = ∑' n, (coeff n F) qⁿ` (`TateCurve.evalInt`, `TateParameter`) evaluates an integral
power series on the open unit disc. The three algebra lemmas below are routine; the discriminant
identity `tateCurve_Δ` is the one deep input, imported as a sorry from FLT PR #1099. -/

open TateCurve PowerSeries in
omit [ValuativeRel k] [IsNonarchimedeanLocalField k] in
/-- Evaluation of an integer constant series. -/
...
open TateCurve PowerSeries in
/-- Pulling an integer constant `a` out of a product before evaluating, valid on the open unit
disc where `evalInt` is multiplicative. -/
...
open TateCurve PowerSeries in
/-- **[FLT PR #1099]** `FLT/KnownIn1980s/EllipticCurves/TateCurveSeries.lean:593`
(`tateCurve_Δ_eq_evalInt`, branch `SmwYin:TateCurveUniformisation`). **Silverman, ATAEC
V.1.1(b).** The discriminant of the Tate curve is the evaluation of the formal discriminant
`ΔFormal = X∏(1-Xⁿ)²⁴` on the open unit disc. This is the one deep classical input — Jacobi's
`E₄³ - E₆² = 1728·η²⁴` via the `q`-expansion of the modular discriminant — whose modular-forms
development is imported from PR #1099 rather than reproved here. -/
theorem tateCurve_Δ (q : k) (hq : valuation k q < 1) :
...
`1 - 48·tateA₄ q`; conclude by `evalInt`-linearity from `tateA₄_eq_evalInt`. -/
theorem tateCurve_c₄ (q : k) (hq : valuation k q < 1) :
...
open TateCurve PowerSeries in
/-- `tateA₄ q` lies in the open unit disc when `|q| < 1`: `a₄Formal = -5s₃` has vanishing
constant term, so `|evalInt q a₄Formal| ≤ |q| < 1`. -/
theorem valuation_tateA₄_lt_one (q : k) (hq : valuation k q < 1) :
    valuation k (tateA₄ q) < 1 := by
  rw [tateA₄_eq_evalInt q hq]
  refine lt_of_le_of_lt (valuation_evalInt_le_pow q hq (M := 1) ?_) (by simpa using hq)
  intro m hm
  rw [Nat.lt_one_iff.mp hm, coeff_a₄Formal]
  simp

open TateCurve PowerSeries in
/-- `tateA₆ q` lies in the open unit disc when `|q| < 1`, as for `tateA₄`. -/
theorem valuation_tateA₆_lt_one (q : k) (hq : valuation k q < 1) :
    valuation k (tateA₆ q) < 1 := by
  rw [tateA₆_eq_evalInt q hq]
  refine lt_of_le_of_lt (valuation_evalInt_le_pow q hq (M := 1) ?_) (by simpa using hq)
...
open TateCurve PowerSeries in
/-- The Tate curve of a parameter in the open unit disc has `|c₄| = 1`: `c₄ = 1 - 48·tateA₄ q`
and `|48·tateA₄ q| < 1`, so the ultrametric leading term `1` dominates (unit `c₄`, hence
minimal). -/
theorem valuation_tateCurve_c₄_eq_one (q : k) (hq : valuation k q < 1) :
    valuation k (tateCurve q).c₄ = 1 := by
  have hc : (tateCurve q).c₄ = 1 - 48 * tateA₄ q := by
    simp only [WeierstrassCurve.c₄, WeierstrassCurve.b₂, WeierstrassCurve.b₄, tateCurve]; ring
  have h48 : valuation k (48 * tateA₄ q) < 1 := by
    rw [map_mul]
    calc valuation k (48 : k) * valuation k (tateA₄ q)
        ≤ 1 * valuation k (tateA₄ q) := by
          gcongr; simpa using valuation_intCast_le_one (R := k) 48
      _ = valuation k (tateA₄ q) := one_mul _
      _ < 1 := valuation_tateA₄_lt_one q hq
  rw [hc, (valuation k).map_sub_eq_of_lt_left (h48.trans_eq (map_one _).symm), map_one]

open TateCurve PowerSeries in
/-- The Tate curve of a parameter in the punctured open unit disc has `|Δ| = |q| < 1`, from
`tateCurve_Δ` and the `ΔFormal` leading-coefficient fragments. -/
theorem valuation_tateCurve_Δ_lt_one (q : k) (hq0 : q ≠ 0) (hq : valuation k q < 1) :
    valuation k (tateCurve q).Δ < 1 := by
  rw [tateCurve_Δ q hq, valuation_evalInt_eq q hq0 hq constantCoeff_ΔFormal coeff_one_ΔFormal]
  exact hq
...
theorem isIntegral_tateCurve (q : k) (hq : valuation k q < 1) :
  ...
  isIntegral_of_exists_lift 𝒪[k]
    ...
    ⟨0, by simp [tateCurve]⟩
    ⟨0, by simp [tateCurve]⟩
    ⟨⟨tateA₄ q, (Valuation.mem_integer_iff _ _).mpr (valuation_tateA₄_lt_one q hq).le⟩, rfl⟩
    ⟨⟨tateA₆ q, (Valuation.mem_integer_iff _ _).mpr (valuation_tateA₆_lt_one q hq).le⟩, rfl⟩
...
open TateCurve in
/-- The Tate curve of the Tate parameter is elliptic: `v(Δ) = v(q) < 1 ≠ 0`. -/
instance : (tateCurve E.q).IsElliptic := by
  refine ⟨isUnit_iff_ne_zero.mpr <| (valuation k).ne_zero_iff.mp ?_⟩
  rw [tateCurve_Δ E.q E.valuation_q_lt_one, valuation_evalInt_eq E.q E.q_ne_zero
    E.valuation_q_lt_one constantCoeff_ΔFormal coeff_one_ΔFormal]
  exact (valuation k).ne_zero_iff.mpr E.q_ne_zero
...
open TateCurve Polynomial IsLocalRing in
...
instance : (tateCurve E.q).HasSplitMultiplicativeReduction 𝒪[k] := by
  ...
  have hc₄adic :
    ...
    have hk : valuation k (tateCurve E.q).c₄ = 1 :=
      valuation_tateCurve_c₄_eq_one E.q E.valuation_q_lt_one
  ...
  have ha₄ : r I.a₄ = 0 := res0 _ (by
    rw [hIdef, integralModel_a₄_eq]; exact valuation_tateA₄_lt_one E.q E.valuation_q_lt_one)
  have ha₆ : r I.a₆ = 0 := res0 _ (by
    rw [hIdef, integralModel_a₆_eq]; exact valuation_tateA₆_lt_one E.q E.valuation_q_lt_one)
...
/-- **[PR #1088]** `FLT/Mathlib/AlgebraicGeometry/EllipticCurve/Aut.lean:100`. For a Weierstrass
curve with `c₄, c₆ ≠ 0` (i.e. `j ∉ {0, 1728}`), any change of variables fixing it has
`u`-component `±1` — the `u`-part of `Aut(E) = {±1}` and the source of the sign cocycle in the
descent. (Self-contained, so proved here rather than sorried.) -/
theorem u_eq_one_or_eq_neg_one {F : Type*} [Field F] (E : WeierstrassCurve F) (hc4 : E.c₄ ≠ 0)
    (hc6 : E.c₆ ≠ 0) {C : VariableChange F} (hCE : C • E = E) : C.u = 1 ∨ C.u = -1 := by
  ...
  have hu2 : (C.u : F) * (C.u : F) = 1 := by linear_combination hu6 - (C.u : F) ^ 2 * hu4
  rcases mul_self_eq_one_iff.mp hu2 with h | h
  · exact .inl (Units.val_eq_one.mp h)
  · exact .inr (Units.ext h)

/-- **[PR #1088]** `FLT/Mathlib/AlgebraicGeometry/EllipticCurve/VariableChange.lean:35`. The
negation automorphism `[-1] : (x, y) ↦ (x, -y - a₁x - a₃)` of a Weierstrass curve, as the
admissible change of variables `⟨-1, 0, -a₁, -a₃⟩`. -/
def negVariableChange {F : Type*} [Field F] (E : WeierstrassCurve F) : VariableChange F :=
  ⟨-1, 0, -E.a₁, -E.a₃⟩

/-- **[PR #1088]** `FLT/Mathlib/AlgebraicGeometry/EllipticCurve/VariableChange.lean:53`. Base
change commutes with `[-1]`: mapping `[-1]` on `W` through `φ` gives `[-1]` on `W.map φ`. This is
what makes the sign character a *homomorphism* — `Gal(Ω/k)` fixes `[-1]`. -/
lemma negVariableChange_map {A B : Type*} [Field A] [Field B] (W : WeierstrassCurve A)
  ...
  ext <;> simp [negVariableChange, WeierstrassCurve.map]

/-- **[PR #1088]** `FLT/Mathlib/AlgebraicGeometry/EllipticCurve/Aut.lean:134`. For `j ∉ {0, 1728}`,
the only admissible changes of variables fixing `E` are `1` and `[-1]`; that is, `Aut(E) = {±1}`.
Proved in PR #1088; taken here as an interface sorry, with everything specific to the *descent*
built on top of it. -/
...
By `Aut(E) = {1, [-1]}` (`u_eq_one_or_eq_neg_one`), `σ ↦ (C⁻¹ · σ(C))` is a homomorphism
...
Proved here (on top of `eq_one_or_eq_negVariableChange_of_smul_eq`, the `grobner`-blocked
...
theorem exists_stabilizer_index_le_two (W₁ W₂ : WeierstrassCurve k) [W₁.IsElliptic] [W₂.IsElliptic]
  ...
  have haFixed : ∀ σ τ : Ω ≃ₐ[k] Ω,
    ...
    · rw [hneg, negVariableChange_map, hWinv W₁ σ]
...
/-- **Tate-curve descent, characteristic-uniform.** For `E` with split multiplicative
reduction, `E` is isomorphic to the Tate curve of its Tate parameter — with no characteristic
hypothesis. Feed `j_tateCurve_q` (Half A) to `exists_variableChange_of_splitMult`; the Tate
curve is split multiplicative by `B0`. -/
theorem exists_variableChange_tateCurve_charUniform (E : WeierstrassCurve k) [E.IsElliptic]
