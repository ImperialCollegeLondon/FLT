...
theorem WeierstrassCurve.isCoprime_Φ_ΨSq {R₀ : Type*} [CommRing R₀] (W : WeierstrassCurve R₀)
  ...
  -- The resultant lies in the ideal generated by `Φ n` and `ΨSq n` (a Bézout combination,
  -- `Polynomial.exists_mul_add_mul_eq_C_resultant`), and by `resultant_Φ_ΨSq` it equals
  -- `±Δ ^ k`, a unit here since `Δ` is a unit. Scaling the Bézout identity by its inverse
  -- gives `a * Φ n + b * ΨSq n = 1`, i.e. coprimality.
  have hne : n.natAbs ^ 2 ≠ 0 := pow_ne_zero 2 (Int.natAbs_ne_zero.mpr hn)
  obtain ⟨p, q, -, -, e⟩ := Polynomial.exists_mul_add_mul_eq_C_resultant (W.Φ n) (W.ΨSq n)
    (W.natDegree_Φ_le n) (W.natDegree_ΨSq_le n) (Or.inl hne)
  have hu : IsUnit ((W.Φ n).resultant (W.ΨSq n) (n.natAbs ^ 2) (n.natAbs ^ 2 - 1)) := by
    rcases W.resultant_Φ_ΨSq hn with h | h <;> rw [h]
    · exact hΔ.pow _
    · exact (hΔ.pow _).neg
  obtain ⟨u, hu⟩ := hu
  refine ⟨Polynomial.C (↑(u⁻¹) : R₀) * p, Polynomial.C (↑(u⁻¹) : R₀) * q, ?_⟩
  have huu : (↑(u⁻¹) : R₀) * (W.Φ n).resultant (W.ΨSq n) (n.natAbs ^ 2) (n.natAbs ^ 2 - 1) = 1 := by
    rw [← hu]; exact u.inv_mul
  calc
    Polynomial.C (↑(u⁻¹) : R₀) * p * W.Φ n + Polynomial.C (↑(u⁻¹) : R₀) * q * W.ΨSq n
        = Polynomial.C (↑(u⁻¹) : R₀) * (W.Φ n * p + W.ΨSq n * q) := by ring
    _ = Polynomial.C (↑(u⁻¹) : R₀) *
          Polynomial.C ((W.Φ n).resultant (W.ΨSq n) (n.natAbs ^ 2) (n.natAbs ^ 2 - 1)) := by rw [e]
    _ = Polynomial.C 1 := by rw [← map_mul, huu]
    _ = 1 := map_one _
