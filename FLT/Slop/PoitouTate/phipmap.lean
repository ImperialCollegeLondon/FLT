...
`Z¹(G, X) ⊆ C(G, X)` of a topological representation `X : TopRep R G` (functions with
`c (g * h) = c g + ρ(g) (c h)`), and the identification of the kernel of the degree-`1`
differential of the homogeneous cochain complex with them.
-/
...
def oneCocycles : Submodule R C(G, ↥X) where
  carrier := {c | ∀ g h : G, c (g * h) = c g + X.ρ g (c h)}
...
@[simp] lemma mem_oneCocycles {c : C(G, ↥X)} :
    c ∈ oneCocycles X ↔ ∀ g h : G, c (g * h) = c g + X.ρ g (c h) := Iff.rfl
