...
This file classifies the *wild* finite subgroups of `PGL(2, K)`,
i.e. those whose order is divisible by the characteristic `p`.
The proof proceeds by case-splitting on whether the unique Sylow
`p`-subgroup is normal (equivalently, whether `G` fixes a point
on the projective line).

**Branch 1 (fixed point).** Using the dilation homomorphism
`dilationHomOfFix`, the complement to the Sylow `p`-subgroup
is shown to be cyclic of coprime order by the Schur–Zassenhaus
theorem, yielding a semidirect product decomposition.

**Branch 2 (no fixed point).** The Diophantine constraints from
`branch2_params` force the Sylow parameters into one of four
cases. When `z₁ = 1`, the group has order 12, 24, or 60 and
is recognised as `A₄`, `S₄`, or `A₅`. When `z₁ ≥ 2`, the
field-reconstruction machinery of `PSLRecognition` identifies
the group as `PSL(2, F_q)` or `PGL(2, F_q)`.

## Main definitions

- `dilationHomOfFix`: the homomorphism extracting the dilation parameter for a subgroup
                      fixing infinity.

## Main results

- `fixes_point_coprime_isCyclic`: a subgroup fixing a point and of order coprime to `p` is cyclic.
- `branch1_semidirect`: a subgroup fixing a point is isomorphic to a semidirect product.
- `dickson_branch2_z1_eq_1`: a subgroup without a fixed point and with `z₁ = 1` is isomorphic to
                             `A₄`, `S₄`, or `A₅`.
- `dickson_branch2_z1_ge_2`: a subgroup without a fixed point and with `z₁ ≥ 2` is isomorphic to
                             `PSL(2, q)` or `PGL(2, q)`.
...
/-- The homomorphism extracting the dilation parameter `c` (as a unit `c²`) for elements of a
subgroup `H` that fixes infinity. -/
noncomputable def dilationHomOfFix (H : Subgroup (PGL p)) [Finite H]
  ...
  map_mul' g₁ g₂ :=
    Units.ext (dilationParam_mul_eq p ↑g₁ ↑g₂ (hH_fix g₁) (hH_fix g₂) (hH_fix (g₁ * g₂)))
...
theorem dilationHom_ker_order (H : Subgroup (PGL p)) [Finite H]
  ...
  change (g : PGL p) ^ p = 1
  rw [hβ, h_order_translation,
    show p • β = 0 by rw [nsmul_eq_mul, CharP.cast_eq_zero (K p) p, zero_mul],
    translationPGL_zero p]
...
theorem fixes_infinity_coprime_isCyclic (H : Subgroup (PGL p)) [Finite H]
  ...
  haveI : Finite f.range := h_iso.finite_iff.mp ‹Finite H›
  exact (MulEquiv.isCyclic h_iso).mpr (isCyclic_subgroup_units f.range)
...
theorem group_fixes_sylow_point (G : Subgroup (PGL p)) [Finite G]
  ...
  have h_conj_fixedPoint : ∀ h ∈ H, h • ((g : PGL p) • x) = (g : PGL p) • x := by
    ...
    have h_mul : (h' : PGL p) * (g : PGL p) =
        (g : PGL p) * ((g : PGL p)⁻¹ * (h' : PGL p) * (g : PGL p)) := by
...
omit h_odd in
/-- A finite subgroup of `PGL(2, K)` that fixes a point on the projective line and has order
coprime to `p` must be cyclic. -/
...
/-- A finite subgroup of `PGL(2, K)` whose order is divisible by `p` and which fixes a common
point on the projective line is isomorphic to a semidirect product of an elementary abelian
`p`-group and a cyclic group of coprime order. -/
theorem branch1_semidirect (G : Subgroup (PGL p)) [Finite G]
  ...
  letI : CommGroup P.toSubgroup := { Subgroup.toGroup P.toSubgroup with mul_comm := hP_comm }
  ...
  have ⟨fK⟩ : Nonempty (K ≃* Multiplicative (ZMod (Nat.card K))) :=
    ⟨(zmodCyclicMulEquiv ‹IsCyclic K›).symm⟩
  ...
  have hm_ge : m ≥ 1 :=
    (Nat.pow_right_injective (Fact.out : Nat.Prime p).two_le <|
      P.card_eq_multiplicity.symm.trans hP_card) ▸
  ...
  have ht_div : Nat.card K ∣ p ^ m - 1 := by
    ...
    have h_card_norm : Nat.card (Subgroup.normalizer (P : Set G)) = Nat.card G := by
      erw [Subgroup.normalizer_eq_top_iff.mpr hP_normal]
...
theorem branch2a_constraints
    ...
    pm = 3 ∧ r = 1 ∧ n = 12 := by
  have hpm_inv : 1 / (pm : ℚ) ≤ 1 / 3 := by
    rw [one_div, one_div]; exact inv_anti₀ zero_lt_three (Nat.cast_le.mpr hpm)
  have hn_inv_pos : 1 / (n : ℚ) > 0 := one_div_pos.mpr (Nat.cast_pos.mpr hn_pos)
  have h_term_ge : ∀ i, (1 / 4 : ℚ) ≤ partitionTerm (s i) (z i) := fun i ↦ by
    have hz_inv : 1 / (z i : ℚ) ≤ 1 / 2 := by
      rw [one_div, one_div]; exact inv_anti₀ zero_lt_two (Nat.cast_le.mpr (hz_pos i))
  ...
  have hr_pos : r > 0 := by
    ...
    have : pm = n := Nat.cast_inj.mp <| inv_inj.mp (by
      rw [← one_div, ← one_div]; linarith only [h_eq] : (pm : ℚ)⁻¹ = (n : ℚ)⁻¹)
    omega
  have hr_eq_one : r = 1 := by
    by_contra! h_ne; have h_ge : r ≥ 2 := by omega
    have h_lt: (r : ℚ) / 4 < 1 / 2 :=
      calc
        (r : ℚ) / 4 = ∑ i : Fin r, (1 / 4 : ℚ) := by
  ...
  · rw [h, partitionTerm] at h_eq
    have h_leq : 1 / (z 0 : ℚ) ≤ 1 / 2 := by
      rw [one_div, one_div]; exact inv_anti₀ zero_lt_two (Nat.cast_le.mpr (hz_pos 0))
    linarith only [h_lhs_lt, h_eq, h_leq]
  · rw [h, partitionTerm] at h_eq
    have hz_eq : z 0 = 2 := by
      by_contra! h_ne; have h_ge : z 0 ≥ 3 := by have : z 0 ≥ 2 := hz_pos 0; omega
      have h_leq : 1 / (z 0 : ℚ) ≤ 1 / 3 := by
        rw [one_div, one_div]; exact inv_anti₀ zero_lt_three (Nat.cast_le.mpr h_ge)
    ...
    have hpm_eq : pm = 3 := by
      ...
      have h_leq: 1 / (pm : ℚ) ≤ 1 / 4 := by
        rw [one_div, one_div]; exact inv_anti₀ zero_lt_four (Nat.cast_le.mpr h_ge)
...
theorem recognition_A4 (G : Subgroup (PGL p))
  ...
  have h_subgroup_eq : ∀ {H K : Subgroup G}, H ≤ K → Nat.card H = Nat.card K → H = K :=
    fun hHK hcard ↦ SetLike.ext' (Set.eq_of_subset_of_ncard_le hHK hcard.ge)
  have h_card_val : ∀ P' : Sylow p G, Nat.card (P' : Subgroup G) = 3 := fun P' ↦
    (Sylow.card_eq_multiplicity P').trans (by
      rw [hn, hp3, Nat.factorization_eq_one (m := 4) rfl Nat.prime_three (by norm_num), pow_one])
  have h_norm : ∀ P : Sylow p G, Subgroup.normalizer (P : Set G) = P := fun P ↦ by
    refine Eq.symm (h_subgroup_eq Subgroup.le_normalizer ?_)
    have h_card_G := Subgroup.index_mul_card (Subgroup.normalizer (P : Set G))
  ...
  have h_ker : ϕ.ker = ⊥ := eq_bot_iff.mpr fun g hg ↦ Subgroup.mem_bot.mpr (by
    have hg_norm : ∀ P : Sylow p G, g ∈ (P : Subgroup G) := fun P ↦ by
      exact h_norm P ▸
        Sylow.smul_eq_iff_mem_normalizer.mp (DFunLike.ext_iff.mp (MonoidHom.mem_ker.mp hg) P)
    obtain ⟨P, Q, hPQ⟩ : ∃ P Q : Sylow p G, P ≠ Q :=
      Fintype.one_lt_card_iff.mp (by rw [hn3]; norm_num)
    have h_inter : Nat.card ↥((P : Subgroup G) ⊓ (Q : Subgroup G)) = 1 := by
      ...
      · exact h_eq_1
      · have h_inf_eq : (P : Subgroup G) ⊓ (Q : Subgroup G) = (P : Subgroup G) :=
          h_subgroup_eq inf_le_left (by rw [h3, h_card_val P])
        have h_eq : (P : Subgroup G) = (Q : Subgroup G) :=
          h_subgroup_eq (h_inf_eq ▸ inf_le_right) (by rw [h_card_val P, h_card_val Q])
        exact absurd (by cases P; cases Q; congr) hPQ
    have h_sub : Subsingleton ↥((P : Subgroup G) ⊓ (Q : Subgroup G)) :=
      (Nat.card_eq_one_iff_unique.mp h_inter).1
    exact congrArg Subtype.val <| match h_sub with
      | ⟨h_allEq⟩ => h_allEq (⟨g, Subgroup.mem_inf.mpr ⟨hg_norm P, hg_norm Q⟩⟩ :
          ↥((P : Subgroup G) ⊓ (Q : Subgroup G))) ⟨1, Subgroup.one_mem _⟩
  )
  have h_inj := (MonoidHom.ker_eq_bot_iff ϕ).mp h_ker
  have h_range : Nat.card ϕ.range = 12 :=
    hn ▸ Nat.card_congr (MonoidHom.ofInjective h_inj).symm.toEquiv
  have h_perm : Nat.card (Equiv.Perm (Sylow p G)) = 24 := by
    rw [Nat.card_eq_fintype_card, Fintype.card_perm, hn3]; rfl
...
theorem pgl_unique_index_two_subgroup (m : ℕ) (hm : m ≥ 1) (hpm : p ^ m ≥ 5)
  ...
  let H' := H.comap φ
  have h_equiv : H' ≃* H :=
    (H'.equivMapOfInjective φ (pglMap_injective _ (RingHom.injective _))).trans
  ...
  have hH'_index : H'.index = 2 := by
    ...
    have h_even : 2 ∣ p ^ (2 * m) - 1 := by
      rw [← even_iff_two_dvd,
        Nat.even_sub (Nat.one_le_pow _ _ (Nat.le_trans (by norm_num : 1 ≤ 2) hp_gt_two.le))]
    ...
    have h_nz : p ^ m * (p ^ (2 * m) - 1) / 2 ≠ 0 :=
      ne_of_gt (Nat.div_pos (Nat.le_of_dvd (mul_pos (show 0 < p ^ m by omega)
        (Nat.sub_pos_of_lt hp2m)) h_dvd) (by norm_num))
...
theorem recognition_PSL_iso (G : Subgroup (PGL p))
  ...
  have hm1 : m ≥ 1 :=
    m_ge_one_of_pow_ge_three p m (Nat.le_trans (Nat.le.step (Nat.le.step Nat.le.refl)) hm)
  obtain ⟨g, hg⟩ := psl_G_le_pgl_range_from_orbit p G m hm1 hm hn
  obtain ⟨iso'⟩ := pgl_unique_index_two_subgroup p m hm1 hm _ hg (by
    rw [← Nat.card_congr (G.equivMapOfInjective (MulEquiv.toMonoidHom (MulAut.conj g))
      (MulEquiv.injective _)).toEquiv, hn])
...
theorem recognition_PGL_iso (G : Subgroup (PGL p))
  ...
  obtain ⟨g, hg⟩ := pgl_subgroups_conjugate p G H m hm1 hn <|
    (Nat.card_congr (Equiv.ofInjective (pglMap (galoisFieldRingHom (p := p) m))
      (pglMap_injective _ (RingHom.injective (galoisFieldRingHom (p := p) m)))).symm).trans
      (by rw [card_PGL2, Fintype.card_eq_nat_card.trans (GaloisField.card p m (ne_of_gt hm1)),
        pow_mul'])
...
theorem sylow_count_of_order_12 (G : Subgroup (PGL p))
  ...
  have h_card_div : Fintype.card (Sylow p G) ∣ 4 := by
    have h_idx : ∀ P : Sylow p G,
        Nat.card (G ⧸ P.toSubgroup) = Nat.card G / p ^ Nat.factorization (Nat.card G) p := by
      intro P
      have h_card_eq : Nat.card P.toSubgroup = p ^ Nat.factorization (Nat.card G) p := by
        convert P.card_eq_multiplicity
      rw [← h_card_eq, Subgroup.card_eq_card_quotient_mul_card_subgroup P.toSubgroup,
        Nat.mul_div_cancel _ Nat.card_pos]
    have h_dvd : ∀ P : Sylow p G, Fintype.card (Sylow p G) ∣ Nat.card (G ⧸ P.toSubgroup) := by
      intro P
      have h_norm : Fintype.card (Sylow p G) = Nat.card (G ⧸ Subgroup.normalizer (P : Set G)) := by
...
theorem recognition_PSL_general (G : Subgroup (PGL p))
  ...
  · have hp3 : p = 3 := by
      ...
      have h2 : p ≤ 4 := (Nat.le_self_pow (show m ≠ 0 by
        linarith only [m_ge_one_of_pow_ge_three p m hm]) p).trans (by linarith only [h5])
      interval_cases p
      · rfl
      · revert hp; norm_num
    subst hp3
    have hm1 : m = 1 := le_antisymm (by
      by_contra hc
      have h_pow : 3 ^ 2 ≤ 3 ^ m :=
        Nat.pow_le_pow_right (show 0 < 3 by norm_num) (show 2 ≤ m by linarith only [hc])
...
theorem partition_equation
  ...
  · have hpm3 : 3 ≤ pm := sylow_card_ge_3 p G hG_p P
    ...
    have hz1_ge1 : 1 ≤ z1 :=
      Nat.div_pos (Subgroup.card_le_of_le Subgroup.le_normalizer) Nat.card_pos
    ...
    by_cases hz1 : z1 = 1
    · simp only [partitionTerm, if_pos hz1, Finset.sum_const, Finset.card_univ, Fintype.card_fin,
        nsmul_eq_mul, Nat.cast_one, Nat.cast_ofNat, one_mul]
      rw [hz2_eq, h_n_eq, h_D_eq, hz1]
      simp only [Nat.cast_one, mul_one]
      field_simp [hpm_pos, hz1_pos, hnp_pos]
      ring
    · simp only [partitionTerm, if_neg hz1, Finset.sum_const, Finset.card_univ, Fintype.card_fin,
        nsmul_eq_mul, Nat.cast_one, Nat.cast_ofNat, one_mul]
      rw [hz2_eq, h_n_eq, h_D_eq]
      field_simp [hpm_pos, hz1_pos, hnp_pos]
      ring

/-- If a finite subgroup `G` of `PGL(2, K)` has order divisible by `p`, does not fix a point,
and has a normaliser quotient parameter `z₁ = 1`, then `p = 3` and `G` is isomorphic to `A₄`,
`S₄`, or `A₅`. -/
theorem dickson_branch2_z1_eq_1 (G : Subgroup (PGL p)) [Finite G]
  ...
  have h_branch2a : pm = 3 ∧ r = 1 ∧ n = 12 :=
    branch2a_constraints pm n r z s hpm (by linarith only [hn_gt, hpm])
      (by linarith only [hn_gt]) hs_bound hz_pos h_eq_subst
  have hp_eq_3 : p = 3 := by
    have h_p_dvd : p ∣ pm := by
      have h_pow := hpm_eq.trans (Sylow.card_eq_multiplicity P)
      exact h_pow.symm ▸ dvd_pow_self p fun h ↦ by
        rw [h, pow_zero] at h_pow; linarith only [h_branch2a.1, h_pow]
...
/-- If a finite subgroup `G` of `PGL(2, K)` has order divisible by `p`, does not fix a point,
and has a normaliser quotient parameter `z₁ ≥ 2`, then `G` is isomorphic to `PSL(2, F_q)` or
`PGL(2, F_q)` for some `q = p^m`. -/
theorem dickson_branch2_z1_ge_2 (G : Subgroup (PGL p)) [Finite G]
  ...
  · have h_pow := Sylow.card_eq_multiplicity P
    ...
    exact Or.inr <| Or.inl ⟨1, le_refl 1,
      recognition_PSL_pm3 p G hp_eq_3 (by rw [card_eq_pm_z1_np' p G P, h.1, h.2.1, h.2.2]) h.2.2⟩
  · obtain ⟨m, hm⟩ : ∃ m, pm = p ^ m := ⟨_, hpm_eq.trans (Sylow.card_eq_multiplicity P)⟩
    have h2_dvd : 2 ∣ p ^ m - 1 := by
      ...
      exact iff_of_false (fun h_e ↦ ne_of_gt h_odd.1 <|
        (Nat.Prime.even_iff Fact.out).mp (Nat.even_pow.mp h_e).1) (by norm_num)
    ...
    · rw [card_eq_pm_z1_np' p G P, h.2.1, h.2.2, ← hpm_eq, hm, hk]
      ...
      have h_pow_sq : p ^ (2 * m) = (k * 2 + 1) ^ 2 := by rw [mul_comm 2, Nat.pow_mul, hk]
      rw [h_pow_sq]
      have h_sq_sub_one : (k * 2 + 1) ^ 2 - 1 = (k * 2 * (k + 1)) * 2 := by
        rw [show (k * 2 + 1) ^ 2 = (k * 2 * (k + 1)) * 2 + 1 by ring, Nat.add_sub_cancel]
      rw [h_sq_sub_one, ← mul_assoc, Nat.mul_div_cancel _ (by norm_num : 2 > 0)]
      ring
    · exact hG_p
    · exact hn_p_gt1
  · obtain ⟨m, hm⟩ : ∃ m : ℕ, pm = p ^ m := ⟨_, hpm_eq.trans (Sylow.card_eq_multiplicity P)⟩
    refine Or.inr <| Or.inr <| Or.inl ⟨m, m_ge_one_of_pow_ge_three p m (by linarith only [hm, hpm]),
      recognition_PGL_iso p G m (by linarith only [hm, hpm]) ?_⟩
    rw [card_eq_pm_z1_np' p G P, h.1, h.2, ← hpm_eq, hm]
    have h_pow_sq : p ^ (2 * m) = (p ^ m) ^ 2 := by rw [← Nat.pow_mul, mul_comm 2, Nat.pow_mul]
    rw [h_pow_sq]
    obtain ⟨k, hk⟩ : ∃ k, p ^ m = k + 1 :=
      ⟨p ^ m - 1, by have := Nat.one_le_pow m p (by linarith only [h_odd.1]); omega⟩
    rw [hk, show k + 1 - 1 = k from rfl]
    have h_sq_sub_one : (k + 1) ^ 2 - 1 = k * (k + 2) := by
      rw [show (k + 1) ^ 2 = k * (k + 2) + 1 by ring, Nat.add_sub_cancel]
    rw [h_sq_sub_one]
    ring
  · have h_pow := Sylow.card_eq_multiplicity P
    have hp_eq_3 : p = 3 := (Nat.prime_dvd_prime_iff_eq Fact.out Nat.prime_three).mp <|
      h.1 ▸ (h_pow ▸ dvd_pow_self p fun hm ↦ by
        rw [hm, pow_zero] at h_pow; linarith only [h.1, h_pow])
    exact Or.inr <| Or.inr <| Or.inr ⟨hp_eq_3,
      recognition_A5_proof p G hp_eq_3 (by rw [card_eq_pm_z1_np' p G P, h.1, h.2.1, h.2.2]) h.2.2⟩

/-- **Dickson's classification for wild subgroups**.
Any finite subgroup `G` of `PGL(2, K)` whose order is divisible by the characteristic `p`
is isomorphic to one of the following:
1. An elementary abelian `p`-group.
2. A semidirect product of an elementary abelian `p`-group and a cyclic group.
3. `PSL(2, F_{p^m})` for some `m ≥ 1`.
4. `PGL(2, F_{p^m})` for some `m ≥ 1`.
5. `A₄`, `S₄`, or `A₅` (occurring when `p = 3`). -/
theorem classification_wild (G : Subgroup (PGL p)) [Finite G]
    ...
    (∃ m : ℕ, m ≥ 1 ∧
      Nonempty (G ≃* (GL (Fin 2) (GaloisField p m) ⧸
        Subgroup.center (GL (Fin 2) (GaloisField p m))))) ∨
    (p = 3 ∧ Nonempty (G ≃* alternatingGroup (Fin 5))) := by
  have ⟨P⟩ : Nonempty (Sylow p G) := Sylow.nonempty
  ...
  · have hn_p_gt1 : Fintype.card (Sylow p G) > 1 :=
      lt_of_le_of_ne (Fintype.card_pos_iff.mpr Sylow.nonempty) (Ne.symm h_sylow)
    obtain ⟨pm, z1, n, r, z, s, hpm_eq, hn_eq, hpm, hz1_ge, hz1_dvd,
        hs_bound, hz_pos, hn_gt, h_eq⟩ :=
      partition_equation p G hG_p P hn_p_gt1
    by_cases hz1_eq : z1 = 1
    · exact dickson_branch2_z1_eq_1 p G hG_p P hn_p_gt1 pm z1 n r z s hpm_eq hn_eq hpm hz1_ge
        hz1_dvd hs_bound hz_pos hn_gt h_eq hz1_eq
