...
This file develops the Sylow theory specific to finite subgroups
`G` of `PGL(2, K)` in characteristic `p`. Starting from the
fixed-point dichotomy (`fixedPoints_dichotomy`), it shows that
every Sylow `p`-subgroup has a *unique* common fixed point on
the projective line, and that every nontrivial element in the
subgroup fixes only this point. The injectivity of the map
`P ↦ fixedPoint(P)` then forces distinct Sylow `p`-subgroups
to intersect trivially, which in turn yields a sharper Sylow
counting congruence: the number of Sylow `p`-subgroups is
congruent to 1 modulo `|P|` (not just modulo `p`).

## Main results

- `sylow_unique_fixedPoint`: a Sylow `p`-subgroup has only one fixed point on the projective line.
- `sylow_pairwise_trivial_intersection`: distinct Sylow `p`-subgroups intersect trivially.
- `count_order_p_elements`: counts the total number of elements of order `p` in `G`.
- `sylow_fixedPoint_injective`: the map assigning to each Sylow `p`-subgroup
                                its unique fixed point is injective.
- `card_sylow_mod_card`: the number of Sylow `p`-subgroups is congruent to 1
...
theorem sylow_card_prime_pow (G : Subgroup (PGL p)) [Finite G]
  ...
  ⟨(Nat.card G).factorization p,
    Nat.pos_of_ne_zero (Finsupp.mem_support_iff.mp <|
      Nat.mem_primeFactors.mpr ⟨Fact.out, hG_p, Nat.card_pos.ne'⟩),
...
/-- A Sylow `p`-subgroup has exactly one common fixed point on the projective line. -/
...
theorem normalizer_stabilizes_fixedPoint (G : Subgroup (PGL p)) [Finite G]
    ...
    ∀ g ∈ Subgroup.normalizer (P : Set G), (g : PGL p) • x = x := fun g hg ↦
...
theorem sylow_element_order_p (G : Subgroup (PGL p)) [Finite G]
  ...
  haveI : Finite P' :=
    Finite.of_equiv P (Subgroup.equivMapOfInjective _ _ Subtype.coe_injective).toEquiv
...
noncomputable def sylow_fixedPoint (G : Subgroup (PGL p)) [Finite G]
...
theorem sylow_element_fixes (G : Subgroup (PGL p)) [Finite G]
    ...
    (g : PGL p) • sylow_fixedPoint p G hG_p P = sylow_fixedPoint p G hG_p P :=
...
theorem sylow_pairwise_trivial_intersection (G : Subgroup (PGL p)) [Finite G]
  ...
  have h_comm_G : ∀ a ∈ P, ∀ b ∈ Q, a * b = b * a := fun a ha b hb ↦ by
    ...
    exact Subtype.ext <| commute_of_orderOf_prime_common_fixedPoint p (a : PGL p) (b : PGL p)
      ...
      (shared_element_shared_fixedPoint p G hG_p P Q g hgP hgQ hg_ne_one ▸
        sylow_element_fixes p G hG_p Q b hb)
  let S : Subgroup G := {
...
open Classical in
/-- The total number of elements of order `p` in `G` is exactly `n_p * (|P| - 1)`. -/
theorem count_order_p_elements (G : Subgroup (PGL p)) [Finite G]
    (hG_p : p ∣ Nat.card G) (P : Sylow p G) :
    Nat.card {g : G | orderOf g = p} =
      Fintype.card (Sylow p G) * (Nat.card P - 1) := by
  have h_elements_order_p : {g : G | orderOf g = p} =
      ⋃ Q ∈ (Finset.univ : Finset (Sylow p G)), ((Q : Subgroup G) \ {1} : Set G) := by
    ext g
    simp only [Set.mem_setOf_eq, Set.mem_iUnion, Set.mem_sdiff, Set.mem_singleton_iff]
    refine ⟨fun hg ↦ ?_, fun ⟨Q, _, hgQ, hg_ne_one⟩ ↦ ?_⟩
    · have h_sylow : IsPGroup p (Subgroup.zpowers g) := fun ⟨x, hx⟩ ↦ by
        ...
        exact Subtype.ext <| show (g ^ k) ^ (p ^ 1) = 1 by
          ...
          have h_pow : g ^ (p : ℤ) = 1 := by rw [zpow_natCast, ← pow_orderOf_eq_one g, hg]
          rw [h_pow, one_zpow]
      obtain ⟨Q, hQ⟩ := IsPGroup.exists_le_sylow h_sylow
      exact ⟨Q, Finset.mem_univ Q, hQ (Subgroup.mem_zpowers g),
        fun h_eq ↦ (Fact.out : Nat.Prime p).ne_one <|
  ...
  have h_card_union : ∀ (s : Finset (Sylow p G)),
      (⋃ Q ∈ s, ((Q : Subgroup G) \ {1} : Set G)).ncard =
        ∑ Q ∈ s, ((Q : Subgroup G) \ {1} : Set G).ncard := by
    intro s
    ...
    | insert Q s hQs ih =>
      have h_eq : (⋃ R ∈ insert Q s, ((R : Subgroup G) \ {1} : Set G)) =
          ((Q : Subgroup G) \ {1} : Set G) ∪ ⋃ R ∈ s, ((R : Subgroup G) \ {1} : Set G) := by
        ext x
        ...
        exact ⟨fun ⟨R, hR, hxR⟩ ↦ hR.elim (fun h ↦ Or.inl <| h ▸ hxR) (fun h ↦ Or.inr ⟨R, h, hxR⟩),
               fun h ↦ h.elim (fun hxQ ↦ ⟨Q, Or.inl rfl, hxQ⟩)
                 (fun ⟨R, hRs, hxR⟩ ↦ ⟨R, Or.inr hRs, hxR⟩)⟩
  ...
  have h_card_diff : ∀ Q : Sylow p G,
      ((Q : Subgroup G) \ {1} : Set G).ncard = Nat.card P - 1 := fun Q ↦ by
...
theorem fixes_infinity_commutes_order_p
  ...
  obtain ⟨c, hc⟩ : ∃ c : (K p)ˣ,
      Units.map (algebraMap (K p) (Matrix (Fin 2) (Fin 2) (K p))).toMonoidHom c =
        (B * A)⁻¹ * (A * B) := by
    rw [hg, hh] at h_comm
    have h_center : (B * A)⁻¹ * (A * B) ∈ Subgroup.center (GL (Fin 2) (K p)) :=
      QuotientGroup.eq.mp h_comm
  ...
  have h_d : d = (c : K p) * d := by
    ...
    calc d = 0 * β + d * 1 := by ring
      ...
      _ = (c : K p) * d := by ring
  have hc1 : (c : K p) = 1 :=
    mul_right_cancel₀ (fun h ↦ h_ne (by rw [h, mul_zero])) (h_d.symm.trans (one_mul d).symm)
  ...
  by_cases hb : b = 0
  · left
    rw [hg, orderOf_eq_one_iff, QuotientGroup.eq_one_iff,
...
theorem common_fixedPoint_commutes_order_p
    ...
    (hh_order : orderOf h = p) (h_comm : h * g = g * h) :
    orderOf g = 1 ∨ orderOf g = p := by
  ...
  have h_comm' : (k * h * k⁻¹) * (k * g * k⁻¹) = (k * g * k⁻¹) * (k * h * k⁻¹) := by
    change (MulAut.conj k) h * (MulAut.conj k) g = (MulAut.conj k) g * (MulAut.conj k) h
...
theorem order_one_or_p_in_sylow (G : Subgroup (PGL p)) [Finite G]
    (P : Sylow p G) (g : G) (hg_norm : g ∈ Subgroup.normalizer (P : Set G))
...
theorem normalizer_fixes_element_in_sylow (G : Subgroup (PGL p)) [Finite G]
    (hG_p : p ∣ Nat.card G) (P : Sylow p G)
    (g : G) (hg_norm : g ∈ Subgroup.normalizer (P : Set G))
    (h : G) (hh_in_P : h ∈ (P : Subgroup G)) (hh_ne : h ≠ 1)
    (h_conj_eq : g * h * g⁻¹ = h) :
    g ∈ (P : Subgroup G) := by
  have h_comm_G : g * h = h * g := by
    rw [← mul_one (g * h), ← inv_mul_cancel g, ← mul_assoc, h_conj_eq]
  have h_fixed : (g : PGL p) • sylow_fixedPoint p G hG_p P = sylow_fixedPoint p G hG_p P :=
    normalizer_stabilizes_fixedPoint p G P (sylow_fixedPoint p G hG_p P)
  ...
  have h_order := common_fixedPoint_commutes_order_p p (g : PGL p) (h : PGL p)
    (sylow_fixedPoint p G hG_p P) h_fixed
    (sylow_element_fixes p G hG_p P h hh_in_P)
    (sylow_element_order_p p G P h hh_in_P hh_ne)
    (congr_arg Subtype.val h_comm_G.symm)
  exact order_one_or_p_in_sylow p G P g hg_norm h_order

noncomputable instance conjOnP (G : Subgroup (PGL p)) (P : Sylow p G) :
    MulAction (↑(Subgroup.normalizer (P : Set G))) (↑(P : Subgroup G)) where
  ...
  mul_smul g₁ g₂ h := by
    apply Subtype.ext
    change (g₁.val * g₂.val) * h.val * (g₁.val * g₂.val)⁻¹ =
      (g₁.val * (g₂.val * h.val * g₂.val⁻¹)) * g₁.val⁻¹
    rw [mul_inv_rev, mul_assoc g₁.val, mul_assoc g₁.val, ← mul_assoc (g₂.val * h.val),
        ← mul_assoc g₁.val]

theorem stabilizer_conj_eq_P (G : Subgroup (PGL p)) [Finite G]
    (hG_p : p ∣ Nat.card G) (P : Sylow p G)
    (h : ↑(P : Subgroup G)) (hh_ne : h ≠ 1) :
    MulAction.stabilizer (↑(Subgroup.normalizer (P : Set G))) h =
    (P : Subgroup G).subgroupOf (Subgroup.normalizer (P : Set G)) := by
  haveI : Finite ↥(P.map G.subtype) := Set.Finite.to_subtype
    (Set.Finite.subset (Set.finite_range G.subtype) (by
      rintro x ⟨y, _, rfl⟩
  ...
  have h_abelian : ∀ g : G, g ∈ (P : Subgroup G) → g * h.val * g⁻¹ = h.val := by
    ...
    have h_comm_map := (isPGroup_comm_exponent_fixedPoint p (P.map G.subtype)
      (P.2.map (Subgroup.subtype G))
        ⟨⟨⟨G.subtype h.val, Subgroup.mem_map_of_mem G.subtype h.2⟩, 1,
          fun h_eq ↦ hh_ne (Subtype.ext (Subtype.ext
            (congr_arg (fun x : P.map G.subtype ↦ x.val) h_eq)))⟩⟩).1
    have h_comm : g * h.val = h.val * g := Subtype.ext (congr_arg (fun x : P.map G.subtype ↦ x.val)
      (h_comm_map ⟨G.subtype g, Subgroup.mem_map_of_mem G.subtype hg⟩
        ⟨G.subtype h.val, Subgroup.mem_map_of_mem G.subtype h.2⟩))
    rw [h_comm, mul_inv_cancel_right]
  ext ⟨g, hg⟩
  constructor <;> intro hg'
  · exact normalizer_fixes_element_in_sylow p G hG_p P g hg h.val h.2
      (fun h_eq ↦ hh_ne (Subtype.ext h_eq)) (congr_arg Subtype.val hg')
...
theorem normalizer_complement_divides_main (G : Subgroup (PGL p)) [Finite G]
    ...
    Nat.card (Subgroup.normalizer (P : Set G)) / Nat.card P ∣
      Nat.card P - 1 := by
  letI : MulAction (Subgroup.normalizer (P : Set G)) (P : Subgroup G) := conjOnP p G P
  letI : SMul (Subgroup.normalizer (P : Set G)) (P : Subgroup G) := (conjOnP p G P).toSMul
  have h_orbit_card : ∀ h : (P : Subgroup G), h ≠ 1 →
      Nat.card (MulAction.orbit (Subgroup.normalizer (P : Set G)) h) =
        Nat.card (Subgroup.normalizer (P : Set G)) / Nat.card P := by
    intro h hh_ne
    have e : ↥((P : Subgroup G).subgroupOf (Subgroup.normalizer (P : Set G))) ≃ (P : Subgroup G) :=
      ⟨fun x ↦ ⟨x.1.1, x.2⟩, fun x ↦ ⟨⟨x.1, Subgroup.le_normalizer x.2⟩, x.2⟩,
       fun _ ↦ rfl, fun _ ↦ rfl⟩
    have h_stab : Nat.card (MulAction.stabilizer (Subgroup.normalizer (P : Set G)) h) =
        Nat.card P := by
  ...
  calc Nat.card (Subgroup.normalizer (P : Set G)) / Nat.card P
    _ ∣ ∑ ω ∈ Finset.image (fun h : (P : Subgroup G) ↦ MulAction.orbit
        (Subgroup.normalizer (P : Set G)) h) (Finset.univ.erase 1), Nat.card ω := by
    ...
    _ = (Finset.univ.erase (1 : (P : Subgroup G))).card := by
      ...
      intro i hi
      have e : ↥(Finset.filter (fun j ↦ MulAction.orbit (Subgroup.normalizer (P : Set G)) j =
          MulAction.orbit (Subgroup.normalizer (P : Set G)) i) (Finset.univ.erase 1)) ≃
          MulAction.orbit (Subgroup.normalizer (P : Set G)) i := by
        apply Equiv.subtypeEquivProp
        ext x
        simp only [Finset.mem_filter, Finset.mem_erase]
        refine ⟨fun h ↦ h.2 ▸ MulAction.mem_orbit_self x,
                fun hx ↦ ⟨⟨?_, Finset.mem_univ x⟩, MulAction.orbit_eq_iff.mpr hx⟩⟩
        rintro rfl
        rcases hx with ⟨g, hg⟩
...
theorem group_order_gt_normalizer (G : Subgroup (PGL p)) [Finite G]
  ...
  rw [normalizerQuotient,
    ...
    Subgroup.card_eq_card_quotient_mul_card_subgroup (Subgroup.normalizer (P : Set G)),
    ← Nat.card_congr (Sylow.equivQuotientNormalizer P), Nat.card_eq_fintype_card (α := Sylow p G)]
  have h := Nat.mul_lt_mul_of_pos_right hn_p_gt1
    (Nat.card_pos (α := Subgroup.normalizer (P : Set G)))
  rw [one_mul] at h
  exact h
...
/-- The map assigning a Sylow `p`-subgroup to its unique fixed point is injective. -/
theorem sylow_fixedPoint_injective (G : Subgroup (PGL p)) [Finite G]
    (hG_p : p ∣ Nat.card G) :
    Function.Injective (sylow_fixedPoint p G hG_p) := by
  intro P Q hPQ
  by_contra h_contra
  have h_comm_G : ∀ g ∈ (P : Subgroup G), ∀ h ∈ (Q : Subgroup G), g * h = h * g := by
    intro g hg h hh
    refine Subtype.ext ?_
    by_cases hg1 : g = 1; · rw [hg1]; exact Commute.one_left (h : PGL p)
    by_cases hh1 : h = 1; · rw [hh1]; exact Commute.one_right (g : PGL p)
    exact commute_of_orderOf_prime_common_fixedPoint p (g : PGL p) (h : PGL p)
      (sylow_fixedPoint p G hG_p P)
  ...
  have h_pq : ∀ g ∈ (P : Subgroup G) ⊔ (Q : Subgroup G),
      ∃ a ∈ (P : Subgroup G), ∃ b ∈ (Q : Subgroup G), g = a * b := by
    intro g hg
    ...
    next x y hx hy =>
      ...
      exact ⟨a * c, P.mul_mem ha hc, b * d, Q.mul_mem hb hd, by
        rw [← mul_assoc, mul_assoc a b c, (h_comm_G c hc b hb).symm,
            ← mul_assoc a c b, mul_assoc]⟩
    next x hx =>
      obtain ⟨a, ha, b, hb, rfl⟩ := hx
  ...
  have h_comm_subgroup : (P : Subgroup G) ⊔ (Q : Subgroup G) = (P : Subgroup G) := by
    ...
    · obtain ⟨m, hm⟩ := IsPGroup.iff_card.mp P.2
      ...
      have h_ord_div {H : Subgroup G} (x : G) (hx : x ∈ H) : orderOf (x : PGL p) ∣ Nat.card H :=
        (show orderOf (x : PGL p) ∣ Nat.card (Subgroup.zpowers x) by
            simp only [orderOf_submonoid, Nat.card_zpowers, dvd_refl]).trans
          (Subgroup.card_dvd_of_le (Subgroup.zpowers_le.mpr hx))
      use m + n
      ext
      push_cast
      rw [hg_eq]
      refine orderOf_dvd_iff_pow_eq_one.mp <|
        (Commute.orderOf_mul_dvd_lcm (Subtype.ext_iff.mp (h_comm_G a ha b hb))).trans <|
        Nat.lcm_dvd ?_ ?_
  ...
  have h_card : Nat.card (Q : Subgroup G) = Nat.card (P : Subgroup G) := by
    rw [P.card_eq_multiplicity, Q.card_eq_multiplicity]
  exact h_contra <| Sylow.ext <| SetLike.ext' <|
    (Set.eq_of_subset_of_ncard_le h_eq h_card.ge).symm

theorem sylow_free_action (G : Subgroup (PGL p)) [Finite G]
    (hG_p : p ∣ Nat.card G) (P Q : Sylow p G) (hPQ : P ≠ Q)
    (g : G) (hg : g ∈ (P : Subgroup G)) (hg_ne : g ≠ 1) :
    (g : PGL p) • sylow_fixedPoint p G hG_p Q ≠ sylow_fixedPoint p G hG_p Q := by
  intro h_fix
  obtain ⟨x, hx⟩ := Set.ncard_eq_one.mp <|
    order_p_one_fixedPoint p (g : PGL p) (sylow_element_order_p p G P g hg hg_ne)
  have hP_mem : sylow_fixedPoint p G hG_p P ∈ fixedPoints p ↑g :=
    sylow_element_fixes p G hG_p P g hg
  have hQ_mem : sylow_fixedPoint p G hG_p Q ∈ fixedPoints p ↑g := h_fix
  rw [hx, Set.mem_singleton_iff] at hP_mem hQ_mem
  exact hPQ <| sylow_fixedPoint_injective p G hG_p <| hP_mem.trans hQ_mem.symm

open Classical in
/-- The number of Sylow `p`-subgroups is congruent to 1 modulo the size of a Sylow subgroup. -/
theorem card_sylow_mod_card (G : Subgroup (PGL p)) [Finite G]
  ...
  have h_free_action : ∀ g : G, g ∈ (P : Subgroup G) → g ≠ 1 → ∀ Q ∈ S, g • Q ≠ Q := by
    ...
    have hgQ : g ∈ (Q : Subgroup G) := order_one_or_p_in_sylow p G Q g
  ...
  have h_orbit_size : ∀ Q ∈ S, Nat.card (MulAction.orbit (P : Subgroup G) Q) = Nat.card P := by
    ...
    have h_mul := Subgroup.card_eq_card_quotient_mul_card_subgroup
      (MulAction.stabilizer (P : Subgroup G) Q)
    rw [← Nat.card_congr (MulAction.orbitEquivQuotientStabilizer (P : Subgroup G) Q),
        h_stab] at h_mul
    have h_bot : Nat.card (⊥ : Subgroup (P : Subgroup G)) = 1 :=
      Nat.card_eq_fintype_card.trans
        (Fintype.card_ofSubsingleton (1 : (⊥ : Subgroup (P : Subgroup G))))
    rw [h_bot, mul_one] at h_mul
    exact h_mul.symm
  have h_partition : (Finset.card S : ℕ) =
      ∑ o ∈ Finset.image (fun Q ↦ MulAction.orbit (↥(P : Subgroup G)) Q) S, Nat.card o := by
    rw [Finset.card_eq_sum_ones]
    refine (Finset.sum_image' (fun _ ↦ 1) fun Q hQ ↦ ?_).symm
    have h_orbit_eq : Finset.filter (fun j ↦ MulAction.orbit (↥(P : Subgroup G)) j =
        MulAction.orbit (↥(P : Subgroup G)) Q) S =
      ...
      have hgP : g⁻¹ • P = P :=
        Sylow.smul_eq_iff_mem_normalizer.mpr (Subgroup.le_normalizer (inv_mem hg))
      exact (Finset.mem_sdiff.mp hQ).2
        (Finset.mem_singleton.mpr (by rw [← inv_smul_smul g Q, hgQ_G, hj_eq_P, hgP]))
    simp only [h_orbit_eq, Nat.card_eq_fintype_card, Fintype.card_ofFinset,
...
/-- The number of Sylow `p`-subgroups for a large subgroup equals `p^m + 1`. -/
theorem n_p_eq_pgl (G : Subgroup (PGL p)) [Finite G]
  ...
  obtain ⟨k, hk⟩ : ∃ k : ℕ, Fintype.card (Sylow p G) = k * p ^ m + 1 :=
    ⟨(Fintype.card (Sylow p G) - 1) / p ^ m,
      by rw [Nat.div_mul_cancel h_dvd, Nat.sub_add_cancel hn_p_gt1.le]⟩
  have h_div : Fintype.card (Sylow p G) ∣ p ^ (2 * m) - 1 := by
    have h_idx : (P : Subgroup G).index = p ^ (2 * m) - 1 :=
      Nat.eq_of_mul_eq_mul_right (pow_pos (Fact.out : Nat.Prime p).pos m) <| by
        rw [← sylow_order_of_pgl_order p G m hm hn P, Subgroup.index_mul_card, hn,
            mul_comm, sylow_order_of_pgl_order p G m hm hn P]
  ...
  have h_expand : (p ^ m - 1) * p ^ m + (p ^ m - 1) = k * a * p ^ m + a := by
    calc (p ^ m - 1) * p ^ m + (p ^ m - 1) = (p ^ m - 1) * (p ^ m + 1) :=
        by rw [Nat.mul_add, mul_one]
      _ = k * a * p ^ m + a := by rw [ha, Nat.add_mul, one_mul, mul_right_comm]
  have h_a_lt : a < p ^ m := by
    by_contra h_not_lt
    have h_div_eq : ((p ^ m - 1) + (p ^ m - 1) * p ^ m) / p ^ m =
        (a + k * a * p ^ m) / p ^ m := by
      rw [add_comm (p ^ m - 1), h_expand, add_comm a]
    rw [Nat.add_mul_div_right (p ^ m - 1) (p ^ m - 1) hP_pos,
        Nat.add_mul_div_right a (k * a) hP_pos, Nat.div_eq_of_lt hP_sub_lt, zero_add] at h_div_eq
    have h_ge : 1 + p ^ m ≤ a / p ^ m + k * a := Nat.add_le_add
      (by rw [Nat.le_div_iff_mul_le hP_pos, one_mul]; exact le_of_not_gt h_not_lt)
      (Nat.le_trans (le_of_not_gt h_not_lt) (le_trans (le_of_eq (one_mul a).symm)
        (Nat.mul_le_mul_right a hk_pos)))
  ...
  have h_a_eq : a = p ^ m - 1 := by
    have h_mod_eq : ((p ^ m - 1) + (p ^ m - 1) * p ^ m) % p ^ m =
        (a + k * a * p ^ m) % p ^ m := by
      rw [add_comm (p ^ m - 1), h_expand, add_comm a]
    rw [Nat.add_mul_mod_self_right (p ^ m - 1) (p ^ m - 1) (p ^ m),
        Nat.add_mul_mod_self_right a (k * a) (p ^ m), Nat.mod_eq_of_lt hP_sub_lt,
        Nat.mod_eq_of_lt h_a_lt] at h_mod_eq
    exact h_mod_eq.symm
  have h_k_eq_one : k = 1 := by
    rw [h_a_eq] at h_expand
    exact (Nat.eq_of_mul_eq_mul_right
      (Nat.sub_pos_of_lt (Nat.lt_of_lt_of_le (by norm_num) hP))
      (show 1 * (p ^ m - 1) = k * (p ^ m - 1) by
        rw [one_mul]
        exact Nat.eq_of_mul_eq_mul_right hP_pos (Nat.add_right_cancel h_expand))).symm
