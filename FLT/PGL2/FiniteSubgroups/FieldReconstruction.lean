...
This file reconstructs the finite field `F_q` (where `q = p^m`)
inside the algebraic closure `K` of `F_p` and uses it to prove
that any two finite subgroups of `PGL(2, K)` of order `|PGL(2, q)|`
are conjugate. The argument has four main steps:

1. **Field embedding.** The subfield `F_q_in_K` is defined as the
   set of roots of `X^q - X` in `K`, and is shown to coincide with
   the range of the canonical `GaloisField p m →+* K`.
2. **3-transitivity.** The canonical image of `PGL(2, F_q)` in
   `PGL(2, K)` acts 3-transitively on the projective line
   `P¹(F_q)`, by explicit construction of translations, dilations,
   and inversions mapping any ordered triple to any other.
3. **Rigidity.** Any element of `PGL(2, K)` that preserves `P¹(F_q)`
   setwise must lie in the canonical image of `PGL(2, F_q)`, because
   3-transitivity forces its matrix entries to belong to `F_q`.
4. **Conjugacy.** For a subgroup `G` of the correct order, the orbit
   of infinity under an appropriate conjugate of `G` equals `P¹(F_q)`,
   so `G` preserves `P¹(F_q)` and hence is conjugate to the
   standard copy of `PGL(2, F_q)`.

## Main definitions

- `F_q_in_K`: the subfield `F_q` embedded in `K`, defined as elements fixed by `x ↦ x^{p^m}`.
- `translationPGL`: the element in `PGL(2, K)` acting as translation `z ↦ z + b`.
- `dilationPGL`: the element in `PGL(2, K)` acting as dilation `z ↦ c²z`.
- `P1_Fq`: the projective line `P¹(F_q)` embedded in the projective line over `K`.

## Main results

- `galoisFieldRingHom_range_eq_F_q`: the range of the canonical embedding from `GaloisField` to `K`
                                     is exactly `F_q_in_K`.
- `pgl_Fq_three_transitive`: the canonical image of `PGL(2, q)` acts 3-transitively on `P¹(F_q)`.
- `preserves_P1Fq_in_range`: any element of `PGL(2, K)` preserving `P¹(F_q)` must belong to
                             the image of `PGL(2, F_q)`.
...
/-- The finite subfield `F_q` of `K p` (where `q = p^m`), defined as the
set of roots of `X^{p^m} - X`. -/
noncomputable def F_q_in_K (m : ℕ) : Set (K p) :=
  {x : K p | x ^ (p ^ m) = x}

omit h_odd in
theorem F_q_add_closed (m : ℕ) (x y : K p) (hx : x ∈ F_q_in_K p m) (hy : y ∈ F_q_in_K p m) :
    x + y ∈ F_q_in_K p m := by
  change (x + y) ^ (p ^ m) = x + y
  rw [add_pow_char_pow, (hx : x ^ (p ^ m) = x), (hy : y ^ (p ^ m) = y)]

omit h_odd in
theorem F_q_mul_closed (m : ℕ) (x y : K p) (hx : x ∈ F_q_in_K p m) (hy : y ∈ F_q_in_K p m) :
    x * y ∈ F_q_in_K p m := by
  change (x * y) ^ (p ^ m) = x * y
  rw [mul_pow, (hx : x ^ (p ^ m) = x), (hy : y ^ (p ^ m) = y)]

omit h_odd in
theorem F_q_zero (m : ℕ) : (0 : K p) ∈ F_q_in_K p m :=
  (zero_pow (pow_ne_zero m (Nat.Prime.ne_zero Fact.out)))

omit h_odd in
theorem F_q_one (m : ℕ) : (1 : K p) ∈ F_q_in_K p m :=
  one_pow (p ^ m)

open Classical in
omit h_odd in
theorem F_q_card (m : ℕ) (hm : m ≥ 1) : Set.ncard (F_q_in_K p m) = p ^ m := by
  let P : Polynomial (K p) := Polynomial.X ^ (p ^ m) - Polynomial.X
  have hP_deg : P.natDegree = p ^ m := by
    refine (Polynomial.natDegree_sub_eq_left_of_natDegree_lt ?_).trans
      (Polynomial.natDegree_X_pow (p ^ m))
  ...
  have hP_nodup : P.roots.Nodup := by
    ...
    rw [Polynomial.separable_def, Polynomial.derivative_sub,
        Polynomial.derivative_X_pow, Polynomial.derivative_X]
    rw [Nat.cast_pow, CharP.cast_eq_zero (K p) p, zero_pow (by omega),
        Polynomial.C_0, zero_mul, zero_sub]
    exact ⟨0, -1, by ring⟩
  rw [show F_q_in_K p m = (P.roots.toFinset : Set (K p)) by
    ext x
...
theorem additive_subgroup_eq_F_q (m : ℕ) (hm : m ≥ 1)
    ...
    V = F_q_in_K p m := by
  have hV_subset : F_q_in_K p m ⊆ V := fun x hx ↦ by
    rcases eq_or_ne x 0 with rfl | hx0
    · exact hV_zero
    · exact mul_one x ▸ hV_stable x
        (mul_left_cancel₀ hx0 <| by
          rw [← pow_succ', Nat.sub_add_cancel (Nat.one_le_pow _ _ (Nat.Prime.pos Fact.out)),
              mul_one, hx])
        hx0 1 hV_one
...
noncomputable def translationGL (b : K p) : GL (Fin 2) (K p) :=
  Matrix.GeneralLinearGroup.mkOfDetNeZero !![1, b; 0, 1]
    (by erw [Matrix.det_fin_two, mul_zero, sub_zero, mul_one]; exact one_ne_zero)

noncomputable def dilationGL (c : K p) (hc : c ≠ 0) : GL (Fin 2) (K p) :=
  Matrix.GeneralLinearGroup.mkOfDetNeZero !![c, 0; 0, 1]
    (by erw [Matrix.det_fin_two, mul_zero, sub_zero, mul_one]; exact hc)

/-- The element in `PGL(2, K)` corresponding to the translation map `z ↦ z + b`. -/
noncomputable def translationPGL (b : K p) : PGL p :=
  QuotientGroup.mk (translationGL p b)

/-- The element in `PGL(2, K)` corresponding to the dilation map `z ↦ c² z`. -/
...
theorem n_p_gt_one_of_pgl_order (G : Subgroup (PGL p)) [Finite G]
  ...
  have hd : p ^ (2 * m) - 1 ∣ p ^ m - 1 := by
    have h_norm : Subgroup.normalizer (P : Set G) = ⊤ := by
      refine (Subgroup.eq_top_iff' _).mpr fun g ↦ ?_
      rw [← Sylow.smul_eq_iff_mem_normalizer]
      exact hP (g • P)
    have h_dvd := normalizer_complement_divides_main p G P
      (hG ▸ dvd_mul_of_dvd_left (dvd_pow_self p (ne_of_gt hm)) _)
  ...
  exact absurd hd <| Nat.not_dvd_of_pos_of_lt
    ...
    (by
      rw [tsub_lt_tsub_iff_right (Nat.one_le_pow _ _ (Nat.Prime.pos Fact.out))]
      exact pow_lt_pow_right₀ (Nat.Prime.one_lt Fact.out) (by
        rw [two_mul]; exact lt_add_of_pos_right m hm))
...
theorem z1_eq_pm_minus_one (G : Subgroup (PGL p)) [Finite G]
  ...
  have h_norm_card : Nat.card (Subgroup.normalizer (P : Set G)) = p ^ m * (p ^ m - 1) :=
    Nat.eq_of_mul_eq_mul_left (Nat.succ_pos (p ^ m)) <| calc
      (p ^ m + 1) * Nat.card (Subgroup.normalizer (P : Set G))
        = Nat.card G := by
          rw [← n_p_eq_pgl p G m hm hG hn_p, ← Nat.card_eq_fintype_card,
      ...
      _ = (p ^ m + 1) * (p ^ m * (p ^ m - 1)) := by
          rw [hG, Nat.mul_comm 2 m, Nat.pow_mul, ← Nat.one_pow 2, Nat.sq_sub_sq (p ^ m) 1,
              ← Nat.mul_assoc, Nat.mul_comm (p ^ m) (p ^ m + 1), Nat.mul_assoc]
...
theorem p_subgroup_fixing_infinity_translations
  ...
  have h_translation_set : ∀ g ∈ H, ∃ b : K p, g = translationPGL p b := fun g hg ↦ by
    ...
    · exact (order_p_fixing_infinity_is_translation p g (hH_fix g hg)
        (isPGroup_orderOf_eq_prime p H hH_p ⟨g, hg⟩
          (fun h ↦ hg_one (congrArg Subtype.val h)))).imp fun b hb ↦ hb.2
  ...
  refine ⟨h_translation_set, by
    rw [← Set.ncard_image_of_injective (translationSet p H)
      (fun (a b : K p) (hab : translationPGL p a = translationPGL p b) ↦ by
      let Y := Matrix.GeneralLinearGroup.mkOfDetNeZero !![(1 : K p), 0; 1, 1] (by
        rw [Matrix.det_fin_two]
        change (1 : K p) * (1 : K p) - (0 : K p) * (1 : K p) ≠ (0 : K p)
        rw [mul_one, zero_mul, sub_zero]; exact one_ne_zero)
      have h_comm : translationGL p (a - b) * Y = Y * translationGL p (a - b) :=
        (Subgroup.mem_center_iff.mp (by
          exact (Subgroup.center (GL (Fin 2) (K p))).inv_mem_iff.mp
            (mul_one (translationGL p (a - b))⁻¹ ▸ QuotientGroup.eq.mp
              (show translationPGL p (a - b) = 1 by
                rw [sub_eq_add_neg, ← translationPGL_mul, ← translationPGL_inv,
                    hab, mul_inv_cancel]))) Y).symm
      have h_00 := congrFun (congrFun ((Units.val_mul (translationGL p (a - b)) Y).symm.trans
        ((congrArg (fun (U : GL (Fin 2) (K p)) ↦ (U : Matrix (Fin 2) (Fin 2) (K p))) h_comm).trans
          (Units.val_mul Y (translationGL p (a - b))))) 0) 0
      simp only [Matrix.mul_apply, Fin.sum_univ_two] at h_00
      change (1 : K p) * (1 : K p) + (a - b) * (1 : K p) =
        (1 : K p) * (1 : K p) + (0 : K p) * (0 : K p) at h_00
      exact eq_of_sub_eq_zero (Eq.trans
        (Eq.trans
          (show a - b = ((1 : K p) * (1 : K p) + (a - b) * (1 : K p)) -
              ((1 : K p) * (1 : K p) + (0 : K p) * (0 : K p)) by ring)
...
omit h_odd in
/-- The range of the canonical ring homomorphism from `GaloisField p m` to `K p`
is exactly `F_q_in_K p m`. -/
theorem galoisFieldRingHom_range_eq_F_q (m : ℕ) (hm : m ≥ 1) :
    Set.range (galoisFieldRingHom (p := p) m) = F_q_in_K p m := by
...
theorem translationPGL_in_range (m : ℕ) (hm : m ≥ 1) (b : K p)
  ...
  refine ⟨QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero
    !![(1 : GaloisField p m), b'; 0, 1] (by
      rw [Matrix.det_fin_two]
...
theorem upper_triangular_in_pgl_range (m : ℕ) (hm : m ≥ 1) (a b : K p) (ha : a ≠ 0)
    (ha_mem : a ∈ F_q_in_K p m) (hb_mem : b ∈ F_q_in_K p m) :
  ...
  have hb_div_a : b / a ∈ F_q_in_K p m :=
    show (b / a) ^ (p ^ m) = b / a from calc
      (b / a) ^ (p ^ m) = b ^ (p ^ m) / a ^ (p ^ m) := by rw [div_pow]
      _                 = b / a ^ (p ^ m)           := by rw [hb_mem]
      _                 = b / a                     := by rw [ha_mem]
  exact hA.symm ▸ Subgroup.mul_mem _
    (dilationPGL_in_range p m hm a ha ha_mem)
    (translationPGL_in_range p m hm (b / a) hb_div_a)
...
theorem fixes_zero_upper_triangular (a b d : K p) (h_det : a * d ≠ 0) :
    (QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![a, b; 0, d] (by
      rw [Matrix.det_fin_two]; change a * d - b * 0 ≠ 0
      rw [mul_zero, sub_zero]; exact h_det)) : PGL p) •
  ...
  simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
    Matrix.cons_val_zero, Matrix.cons_val_one, mul_zero, zero_add, mul_one] at h0
  exact h0.symm

omit h_odd in
theorem fixes_one_diagonal (a d : K p) (h_det : a * d ≠ 0) :
    (QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![a, (0 : K p); 0, d] (by
      rw [Matrix.det_fin_two]; norm_num; exact mul_ne_zero_iff.mp h_det)) : PGL p) •
        P1point p 1 = P1point p 1 →
    (QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![a, (0 : K p); 0, d] (by
      rw [Matrix.det_fin_two]
      norm_num
      exact mul_ne_zero_iff.mp h_det)) : PGL p) = 1 := fun h_smul ↦ by
  rw [P1point] at h_smul
  erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff] at h_smul
  rcases h_smul with ⟨c, hc⟩
  ...
  simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
    Matrix.cons_val_zero, Matrix.cons_val_one, mul_one, add_zero, zero_add] at h0 h1
  ...
  match i, j with
  | 0, 0 =>
    change ((g : Matrix (Fin 2) (Fin 2) (K p)) * !![a, 0; 0, d]) 0 0 =
      (!![a, 0; 0, d] * (g : Matrix (Fin 2) (Fin 2) (K p))) 0 0
    simp only [h_a_eq_d, Matrix.mul_apply, Fin.sum_univ_two, Matrix.cons_val_zero,
      Matrix.cons_val_one, Matrix.of_apply, zero_mul, add_zero, mul_comm]
  | 0, 1 =>
    change ((g : Matrix (Fin 2) (Fin 2) (K p)) * !![a, 0; 0, d]) 0 1 =
      (!![a, 0; 0, d] * (g : Matrix (Fin 2) (Fin 2) (K p))) 0 1
    simp only [h_a_eq_d, Matrix.mul_apply, Fin.sum_univ_two, Matrix.cons_val_zero,
      Matrix.cons_val_one, Matrix.of_apply, zero_mul, add_zero, zero_add, mul_comm]
  | 1, 0 =>
    change ((g : Matrix (Fin 2) (Fin 2) (K p)) * !![a, 0; 0, d]) 1 0 =
      (!![a, 0; 0, d] * (g : Matrix (Fin 2) (Fin 2) (K p))) 1 0
    simp only [h_a_eq_d, Matrix.mul_apply, Fin.sum_univ_two, Matrix.cons_val_zero,
      Matrix.cons_val_one, Matrix.of_apply, zero_mul, add_zero, zero_add, mul_comm]
  | 1, 1 =>
    change ((g : Matrix (Fin 2) (Fin 2) (K p)) * !![a, 0; 0, d]) 1 1 =
      (!![a, 0; 0, d] * (g : Matrix (Fin 2) (Fin 2) (K p))) 1 1
    simp only [h_a_eq_d, Matrix.mul_apply, Fin.sum_univ_two, Matrix.cons_val_zero,
      Matrix.cons_val_one, Matrix.of_apply, zero_mul, zero_add, mul_comm]
...
theorem upper_triangular_normalizes_translations
  ...
  match i, j with
  | 0, 0 =>
    change (!![a, b; 0, d] * !![1, x; 0, 1]) 0 0 =
      (!![1, a / d * x; 0, 1] * !![a, b; 0, d]) 0 0
    simp only [Matrix.mul_apply, Fin.sum_univ_two, Matrix.cons_val_zero, Matrix.cons_val_one,
      Matrix.of_apply, mul_one, mul_zero, add_zero, one_mul]
  | 0, 1 =>
    change (!![a, b; 0, d] * !![1, x; 0, 1]) 0 1 =
      (!![1, a / d * x; 0, 1] * !![a, b; 0, d]) 0 1
    simp only [Matrix.mul_apply, Fin.sum_univ_two, Matrix.cons_val_zero, Matrix.cons_val_one,
      Matrix.of_apply, mul_one, one_mul]
    rw [mul_right_comm, div_mul_cancel₀ a (right_ne_zero_of_mul h_det), add_comm]
  | 1, 0 =>
    change (!![a, b; 0, d] * !![1, x; 0, 1]) 1 0 =
      (!![1, a / d * x; 0, 1] * !![a, b; 0, d]) 1 0
    simp only [Matrix.mul_apply, Fin.sum_univ_two, Matrix.cons_val_zero, Matrix.cons_val_one,
      Matrix.of_apply, mul_one, mul_zero, add_zero, zero_mul]
  | 1, 1 =>
    change (!![a, b; 0, d] * !![1, x; 0, 1]) 1 1 =
      (!![1, a / d * x; 0, 1] * !![a, b; 0, d]) 1 1
    simp only [Matrix.mul_apply, Fin.sum_univ_two, Matrix.cons_val_zero, Matrix.cons_val_one,
      Matrix.of_apply, mul_one, zero_mul, zero_add, one_mul]

omit h_odd in
theorem general_matrix_in_pgl_range (m : ℕ) (hm : m ≥ 1) (a b c d : K p)
    (h_det : a * d - b * c ≠ 0)
    (ha : a ∈ F_q_in_K p m) (hb : b ∈ F_q_in_K p m)
    (hc : c ∈ F_q_in_K p m) (hd : d ∈ F_q_in_K p m) :
    (QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero
...
/-- The standard projective line `P¹(F_q)` embedded inside the projective line over `K p`. -/
noncomputable def P1_Fq (m : ℕ) : Set (ProjectiveLine p) :=
  {infinity p} ∪ (P1point p '' F_q_in_K p m)

omit h_odd in
theorem infinity_mem_P1_Fq (m : ℕ) : infinity p ∈ P1_Fq p m :=
  Set.mem_union_left _ rfl

omit h_odd in
theorem P1point_mem_P1_Fq (m : ℕ) (c : K p) (hc : c ∈ F_q_in_K p m) :
    P1point p c ∈ P1_Fq p m :=
  Set.mem_union_right _ ⟨c, hc, rfl⟩

theorem F_q_neg (m : ℕ) (x : K p) (hx : x ∈ F_q_in_K p m) : -x ∈ F_q_in_K p m := by
  simp only [F_q_in_K, Set.mem_setOf_eq] at hx ⊢
  rw [neg_pow, hx, Odd.neg_one_pow (Odd.pow (Nat.Prime.odd_of_ne_two Fact.out
    (ne_of_gt Fact.out))), neg_one_mul]

theorem F_q_sub_closed (m : ℕ) (x y : K p) (hx : x ∈ F_q_in_K p m) (hy : y ∈ F_q_in_K p m) :
    x - y ∈ F_q_in_K p m :=
  (sub_eq_add_neg x y).symm ▸ F_q_add_closed p m x (-y) hx (F_q_neg p m y hy)

theorem three_transitive_case1 (m : ℕ) (hm : m ≥ 1)
    (β γ : K p) (hβ : β ∈ F_q_in_K p m) (hγ : γ ∈ F_q_in_K p m) (hne : β ≠ γ) :
  ...
  have h_det : (γ - β) * 1 - β * 0 ≠ 0 := by
    rw [mul_one, mul_zero, sub_zero]; exact sub_ne_zero.mpr hne.symm
  refine ⟨QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![γ - β, β; 0, (1 : K p)]
    (by rw [Matrix.det_fin_two]; exact h_det)), ?_, ?_, ?_, ?_⟩
  · exact upper_triangular_in_pgl_range p m hm (γ - β) β (sub_ne_zero.mpr hne.symm)
      (F_q_sub_closed p m γ β hγ hβ) hβ
  · exact (fixesInfinity_iff_upperTriangular p _).mpr
      ⟨γ - β, β, 1, by rw [mul_one]; exact sub_ne_zero.mpr hne.symm, rfl⟩
  · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
    ...
    | 0 =>
      ...
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_zero, mul_one, zero_add, one_mul]
    | 1 =>
      change 1 * ![β, 1] 1 = Matrix.mulVec !![γ - β, β; 0, 1] ![0, 1] 1
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_zero, mul_one, zero_add]
  · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
    use 1; ext i
    match i with
    | 0 =>
      ...
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_one, one_mul]
      exact (sub_add_cancel γ β).symm
    | 1 =>
      change 1 * ![γ, 1] 1 = Matrix.mulVec !![γ - β, β; 0, 1] ![1, 1] 1
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_one, zero_add]

theorem three_transitive_case2 (m : ℕ) (hm : m ≥ 1)
    (α γ : K p) (hα : α ∈ F_q_in_K p m) (hγ : γ ∈ F_q_in_K p m) (hne : α ≠ γ) :
    ∃ h : PGL p, h ∈ (pglMap (galoisFieldRingHom (p := p) m)).range ∧
      h • infinity p = P1point p α ∧
      h • P1point p 0 = infinity p ∧
      h • P1point p 1 = P1point p γ := by
  have h_det : α * 0 - (γ - α) * 1 ≠ 0 := by
    rw [mul_zero, mul_one, zero_sub, neg_ne_zero]; exact sub_ne_zero.mpr (Ne.symm hne)
  refine ⟨QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![α, γ - α; 1, (0 : K p)]
    (by rw [Matrix.det_fin_two]; exact h_det)), ?_, ?_, ?_, ?_⟩
  · exact general_matrix_in_pgl_range p m hm α (γ - α) 1 0 h_det hα
      (F_q_sub_closed p m γ α hγ hα) (F_q_one p m) (F_q_zero p m)
  · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
    use 1; ext i
    match i with
    | 0 =>
      ...
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, one_mul, mul_one, mul_zero, add_zero]
    | 1 =>
      change 1 * ![α, 1] 1 = Matrix.mulVec !![α, γ - α; 1, 0] ![1, 0] 1
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_one, mul_zero, add_zero]
  · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
    use Units.mk0 (γ - α) (sub_ne_zero.mpr (Ne.symm hne))
    ext i
    match i with
    | 0 =>
      change (γ - α) * ![1, 0] 0 = Matrix.mulVec !![α, γ - α; 1, 0] ![0, 1] 0
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_one, mul_zero, zero_add]
    | 1 =>
      change (γ - α) * ![1, 0] 1 = Matrix.mulVec !![α, γ - α; 1, 0] ![0, 1] 1
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_zero, zero_mul, zero_add]
  · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
    use 1; ext i
    match i with
    | 0 =>
      ...
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_one, one_mul]; ring
    | 1 =>
      change 1 * ![γ, 1] 1 = Matrix.mulVec !![α, γ - α; 1, 0] ![1, 1] 1
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_one, add_zero]

theorem three_transitive_case3 (m : ℕ) (hm : m ≥ 1)
    (α β : K p) (hα : α ∈ F_q_in_K p m) (hβ : β ∈ F_q_in_K p m) (hne : α ≠ β) :
    ∃ h : PGL p, h ∈ (pglMap (galoisFieldRingHom (p := p) m)).range ∧
      h • infinity p = P1point p α ∧
      h • P1point p 0 = P1point p β ∧
      h • P1point p 1 = infinity p := by
  have h_det : α * -1 - (-β) * 1 ≠ 0 := by
    rw [mul_neg_one, mul_one, sub_neg_eq_add, add_comm, ← sub_eq_add_neg]
    exact sub_ne_zero.mpr (Ne.symm hne)
  refine ⟨QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![α, -β; 1, (-1 : K p)]
    (by rw [Matrix.det_fin_two]; exact h_det)), ?_, ?_, ?_, ?_⟩
  · exact general_matrix_in_pgl_range p m hm α (-β) 1 (-1) h_det hα
      (F_q_neg p m β hβ) (F_q_one p m) (F_q_neg p m 1 (F_q_one p m))
  · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
    use 1; ext i
    match i with
    | 0 =>
    ...
    | 1 =>
      ...
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_one, mul_zero, add_zero]
  · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
    use Units.mk0 (-1) (neg_ne_zero.mpr one_ne_zero)
    ext i
    match i with
    | 0 =>
      change (-1) * ![β, 1] 0 = Matrix.mulVec !![α, -β; 1, -1] ![0, 1] 0
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_zero, zero_add, mul_one]; ring
    | 1 =>
      change (-1) * ![β, 1] 1 = Matrix.mulVec !![α, -β; 1, -1] ![0, 1] 1
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_zero, zero_add, mul_one]
  · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
    use Units.mk0 (α - β) (sub_ne_zero.mpr hne)
    ext i
    match i with
    | 0 =>
      change (α - β) * ![1, 0] 0 = Matrix.mulVec !![α, -β; 1, -1] ![1, 1] 0
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_one]; ring
    | 1 =>
      change (α - β) * ![1, 0] 1 = Matrix.mulVec !![α, -β; 1, -1] ![1, 1] 1
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_zero, mul_one]; ring

theorem three_transitive_case4 (m : ℕ) (hm : m ≥ 1)
    (α β γ : K p) (hα : α ∈ F_q_in_K p m) (hβ : β ∈ F_q_in_K p m) (hγ : γ ∈ F_q_in_K p m)
  ...
  have h_det : α * (β - γ) * (γ - α) - β * (γ - α) * (β - γ) ≠ 0 := by
    intro heq
    have h1 : α * (β - γ) * (γ - α) - β * (γ - α) * (β - γ) =
      (α - β) * (β - γ) * (γ - α) := by ring
    exact mul_ne_zero (mul_ne_zero (sub_ne_zero.mpr hαβ) (sub_ne_zero.mpr hβγ))
      (sub_ne_zero.mpr hαγ.symm) (by rw [h1] at heq; exact heq)
  refine ⟨QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero
    !![α * (β - γ), β * (γ - α); β - γ, γ - α] (by rw [Matrix.det_fin_two]; exact h_det)),
    ?_, ?_, ?_, ?_⟩
  ...
  · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
    use Units.mk0 (β - γ) (sub_ne_zero.mpr hβγ)
    ext i
    match i with
    | 0 =>
      change (β - γ) * ![α, 1] 0 =
        Matrix.mulVec !![α * (β - γ), β * (γ - α); β - γ, γ - α] ![1, 0] 0
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_zero, add_zero, mul_one]; ring
    | 1 =>
      change (β - γ) * ![α, 1] 1 =
        Matrix.mulVec !![α * (β - γ), β * (γ - α); β - γ, γ - α] ![1, 0] 1
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_zero, add_zero, mul_one]
  · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
    use Units.mk0 (γ - α) (sub_ne_zero.mpr hαγ.symm)
    ext i
    match i with
    | 0 =>
      change (γ - α) * ![β, 1] 0 =
        Matrix.mulVec !![α * (β - γ), β * (γ - α); β - γ, γ - α] ![0, 1] 0
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_zero, zero_add, mul_one]; ring
    | 1 =>
      change (γ - α) * ![β, 1] 1 =
        Matrix.mulVec !![α * (β - γ), β * (γ - α); β - γ, γ - α] ![0, 1] 1
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_zero, zero_add, mul_one]
  · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
    use Units.mk0 (β - α) (sub_ne_zero.mpr hαβ.symm)
    ext i
    match i with
    | 0 =>
      change (β - α) * ![γ, 1] 0 =
        Matrix.mulVec !![α * (β - γ), β * (γ - α); β - γ, γ - α] ![1, 1] 0
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_one]; ring
    | 1 =>
      change (β - α) * ![γ, 1] 1 =
        Matrix.mulVec !![α * (β - γ), β * (γ - α); β - γ, γ - α] ![1, 1] 1
      simp only [Matrix.of_apply, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
        Matrix.cons_val_zero, Matrix.cons_val_one, mul_one]; ring

/-- The canonical image of `PGL(2, F_q)` inside `PGL(2, K)` acts 3-transitively on `P¹(F_q)`. -/
theorem pgl_Fq_three_transitive (m : ℕ) (hm : m ≥ 1)
    (a b c : ProjectiveLine p)
    (ha : a ∈ P1_Fq p m) (hb : b ∈ P1_Fq p m) (hc : c ∈ P1_Fq p m)
    (hab : a ≠ b) (hac : a ≠ c) (hbc : b ≠ c) :
    ∃ h : PGL p, h ∈ (pglMap (galoisFieldRingHom (p := p) m)).range ∧
      h • infinity p = a ∧ h • P1point p 0 = b ∧ h • P1point p 1 = c := by
  simp only [P1_Fq, Set.mem_union, Set.mem_singleton_iff, Set.mem_image] at ha hb hc
  ...
  · rcases hb with rfl | ⟨β, hβ, rfl⟩
    ...
    · rcases hc with rfl | ⟨γ, hγ, rfl⟩
      ...
      · exact three_transitive_case4 p m hm α β γ hα hβ hγ
          (fun h ↦ hab (congrArg _ h)) (fun h ↦ hac (congrArg _ h))
          (fun h ↦ hbc (congrArg _ h))

/-- Any element of `PGL(2, K)` that preserves the subset `P¹(F_q)` must belong to the
image of `PGL(2, F_q)`. -/
theorem preserves_P1Fq_in_range (m : ℕ) (hm : m ≥ 1)
    (g : PGL p) (hg : ∀ x ∈ P1_Fq p m, g • x ∈ P1_Fq p m) :
  ...
  obtain ⟨h, h_range, h_fix⟩ : ∃ h : PGL p,
      h ∈ (pglMap (galoisFieldRingHom (p := p) m)).range ∧
      h • infinity p = g • infinity p ∧ h • P1point p 0 = g • P1point p 0 ∧
      h • P1point p 1 = g • P1point p 1 :=
    pgl_Fq_three_transitive p m hm _ _ _ (hg _ (infinity_mem_P1_Fq p m))
      (hg _ (P1point_mem_P1_Fq p m 0 (F_q_zero p m)))
      (hg _ (P1point_mem_P1_Fq p m 1 (F_q_one p m)))
  ...
  convert Subgroup.mul_mem _ h_range
    (show h⁻¹ * g ∈ (pglMap (galoisFieldRingHom p m)).range from ?_) using 1
  ...
  · exact (pgl_fixes_three_points_eq_one p _
      ...
      (by rw [mul_smul]; exact inv_smul_eq_iff.mpr h_fix.2.2.symm)).symm ▸
        MonoidHom.mem_range.mpr ⟨1, map_one _⟩
...
omit h_odd in
theorem F_q_inv (m : ℕ) (x : K p) (hx : x ∈ F_q_in_K p m) :
    x⁻¹ ∈ F_q_in_K p m :=
  (inv_pow x (p ^ m)).trans (congrArg (·⁻¹) hx)

omit h_odd in
theorem F_q_div (m : ℕ) (x y : K p) (hx : x ∈ F_q_in_K p m) (hy : y ∈ F_q_in_K p m) :
    x / y ∈ F_q_in_K p m := by
  rw [div_eq_mul_inv]
  exact F_q_mul_closed p m x y⁻¹ hx (F_q_inv p m y hy)

omit h_odd in
theorem range_preserves_P1Fq (m : ℕ) (hm : m ≥ 1)
    (g : PGL p) (hg : g ∈ (pglMap (galoisFieldRingHom (p := p) m)).range) :
    ∀ x ∈ P1_Fq p m, g • x ∈ P1_Fq p m := by
  rcases hg with ⟨⟨g⟩, rfl⟩
  intro x hx
  simp only [P1_Fq, Set.mem_union, Set.mem_singleton_iff, Set.mem_image] at hx ⊢
  ...
  · let num := galoisFieldRingHom p m (g.1 0 0)
    ...
    · left
      ...
      have hnum : num ≠ 0 := by
        intro heq
        have h_det : num * galoisFieldRingHom p m (g.1 1 1) -
            galoisFieldRingHom p m (g.1 0 1) * den = galoisFieldRingHom p m g.1.det := by
          rw [Matrix.det_fin_two, map_sub, map_mul, map_mul]
        rw [heq, h, zero_mul, mul_zero, sub_zero] at h_det
        exact IsUnit.ne_zero (IsUnit.map (galoisFieldRingHom p m : GaloisField p m →* K p)
          (isUnit_iff_ne_zero.mpr (Matrix.GeneralLinearGroup.det_ne_zero g))) h_det.symm
      use Units.mk0 num hnum
      ext i; match i with
      | 0 =>
        change num * ![1, 0] 0 = Matrix.mulVec (Matrix.GeneralLinearGroup.map
          (galoisFieldRingHom p m) g).val ![1, 0] 0
        simp only [Matrix.mulVec, dotProduct, Fin.sum_univ_two, Matrix.cons_val_zero,
          Matrix.cons_val_one]
        change galoisFieldRingHom p m (g.1 0 0) * 1 =
          galoisFieldRingHom p m (g.1 0 0) * 1 + galoisFieldRingHom p m (g.1 0 1) * 0
        ring
      | 1 =>
        change num * ![1, 0] 1 = Matrix.mulVec (Matrix.GeneralLinearGroup.map
          (galoisFieldRingHom p m) g).val ![1, 0] 1
        simp only [Matrix.mulVec, dotProduct, Fin.sum_univ_two, Matrix.cons_val_zero,
          Matrix.cons_val_one]
        change galoisFieldRingHom p m (g.1 0 0) * 0 =
          galoisFieldRingHom p m (g.1 1 0) * 1 + galoisFieldRingHom p m (g.1 1 1) * 0
        have h_den : galoisFieldRingHom p m (g.1 1 0) * 1 +
            galoisFieldRingHom p m (g.1 1 1) * 0 = den := by
          change galoisFieldRingHom p m (g.1 1 0) * 1 +
            galoisFieldRingHom p m (g.1 1 1) * 0 = galoisFieldRingHom p m (g.1 1 0)
          ring
        rw [h_den, h, mul_zero]
    · right
      refine ⟨num / den, ?_, ?_⟩
      ...
      · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
        ...
        ext i; match i with
        | 0 =>
          change den⁻¹ * Matrix.mulVec (Matrix.GeneralLinearGroup.map
            (galoisFieldRingHom p m) g).val ![1, 0] 0 = ![num / den, 1] 0
          simp only [Matrix.mulVec, dotProduct, Fin.sum_univ_two, Matrix.cons_val_zero,
            Matrix.cons_val_one]
          change den⁻¹ * (galoisFieldRingHom p m (g.1 0 0) * 1 +
            galoisFieldRingHom p m (g.1 0 1) * 0) = num / den
          have h_num : galoisFieldRingHom p m (g.1 0 0) * 1 +
              galoisFieldRingHom p m (g.1 0 1) * 0 = num := by
            change galoisFieldRingHom p m (g.1 0 0) * 1 +
              galoisFieldRingHom p m (g.1 0 1) * 0 = galoisFieldRingHom p m (g.1 0 0)
            ring
          rw [h_num, div_eq_mul_inv, mul_comm]
        | 1 =>
          change den⁻¹ * Matrix.mulVec (Matrix.GeneralLinearGroup.map
            (galoisFieldRingHom p m) g).val ![1, 0] 1 = ![num / den, 1] 1
          simp only [Matrix.mulVec, dotProduct, Fin.sum_univ_two, Matrix.cons_val_zero,
            Matrix.cons_val_one]
          change den⁻¹ * (galoisFieldRingHom p m (g.1 1 0) * 1 +
            galoisFieldRingHom p m (g.1 1 1) * 0) = 1
          have h_den : galoisFieldRingHom p m (g.1 1 0) * 1 +
              galoisFieldRingHom p m (g.1 1 1) * 0 = den := by
            change galoisFieldRingHom p m (g.1 1 0) * 1 +
              galoisFieldRingHom p m (g.1 1 1) * 0 = galoisFieldRingHom p m (g.1 1 0)
  ...
  · let num := galoisFieldRingHom p m (g.1 0 0) * a +
      galoisFieldRingHom p m (g.1 0 1)
    let den := galoisFieldRingHom p m (g.1 1 0) * a +
      galoisFieldRingHom p m (g.1 1 1)
    by_cases h : den = 0
    · left
      erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
      have hnum : num ≠ 0 := by
        intro heq
        have h_det : num * galoisFieldRingHom p m (g.1 1 0) -
            den * galoisFieldRingHom p m (g.1 0 0) = - galoisFieldRingHom p m g.1.det := by
          rw [Matrix.det_fin_two, map_sub, map_mul, map_mul]
          change (galoisFieldRingHom p m (g.1 0 0) * a + galoisFieldRingHom p m (g.1 0 1)) *
            galoisFieldRingHom p m (g.1 1 0) - (galoisFieldRingHom p m (g.1 1 0) * a +
            galoisFieldRingHom p m (g.1 1 1)) * galoisFieldRingHom p m (g.1 0 0) = _
          ring
        rw [heq, h, zero_mul, zero_mul, sub_zero] at h_det
        exact IsUnit.ne_zero (IsUnit.map (galoisFieldRingHom p m : GaloisField p m →* K p)
          (isUnit_iff_ne_zero.mpr (Matrix.GeneralLinearGroup.det_ne_zero g)))
          (neg_eq_zero.mp h_det.symm)
      use Units.mk0 num hnum
      ext i; match i with
      | 0 =>
        change num * ![1, 0] 0 = Matrix.mulVec (Matrix.GeneralLinearGroup.map
          (galoisFieldRingHom p m) g).val ![a, 1] 0
        simp only [Matrix.mulVec, dotProduct, Fin.sum_univ_two, Matrix.cons_val_zero,
          Matrix.cons_val_one]
        change (galoisFieldRingHom p m (g.1 0 0) * a + galoisFieldRingHom p m (g.1 0 1)) * 1 =
          galoisFieldRingHom p m (g.1 0 0) * a + galoisFieldRingHom p m (g.1 0 1) * 1
        ring
      | 1 =>
        change num * ![1, 0] 1 = Matrix.mulVec (Matrix.GeneralLinearGroup.map
          (galoisFieldRingHom p m) g).val ![a, 1] 1
        simp only [Matrix.mulVec, dotProduct, Fin.sum_univ_two, Matrix.cons_val_zero,
          Matrix.cons_val_one]
        change (galoisFieldRingHom p m (g.1 0 0) * a + galoisFieldRingHom p m (g.1 0 1)) * 0 =
          galoisFieldRingHom p m (g.1 1 0) * a + galoisFieldRingHom p m (g.1 1 1) * 1
        have h_den : galoisFieldRingHom p m (g.1 1 0) * a +
            galoisFieldRingHom p m (g.1 1 1) * 1 = den := by
          change galoisFieldRingHom p m (g.1 1 0) * a +
            galoisFieldRingHom p m (g.1 1 1) * 1 = galoisFieldRingHom p m (g.1 1 0) * a +
            galoisFieldRingHom p m (g.1 1 1)
          ring
        rw [h_den, h, mul_zero]
    · right
      refine ⟨num / den, ?_, ?_⟩
      · have hlin : ∀ i, galoisFieldRingHom p m (g.1 i 0) * a +
            galoisFieldRingHom p m (g.1 i 1) ∈ F_q_in_K p m := fun i ↦
          F_q_add_closed p m _ _
            (F_q_mul_closed p m _ _
              ((galoisFieldRingHom_range_eq_F_q p m hm) ▸ Set.mem_range_self _) ha)
            ((galoisFieldRingHom_range_eq_F_q p m hm) ▸ Set.mem_range_self _)
        exact F_q_div p m _ _ (hlin 0) (hlin 1)
      · erw [Projectivization.smul_mk, Projectivization.mk_eq_mk_iff]
        use (Units.mk0 den h)⁻¹
        ext i; match i with
        | 0 =>
          change den⁻¹ * Matrix.mulVec (Matrix.GeneralLinearGroup.map
            (galoisFieldRingHom p m) g).val ![a, 1] 0 = ![num / den, 1] 0
          simp only [Matrix.mulVec, dotProduct, Fin.sum_univ_two, Matrix.cons_val_zero,
            Matrix.cons_val_one]
          change den⁻¹ * (galoisFieldRingHom p m (g.1 0 0) * a +
            galoisFieldRingHom p m (g.1 0 1) * 1) = num / den
          have h_num : galoisFieldRingHom p m (g.1 0 0) * a +
              galoisFieldRingHom p m (g.1 0 1) * 1 = num := by
            change galoisFieldRingHom p m (g.1 0 0) * a +
              galoisFieldRingHom p m (g.1 0 1) * 1 = galoisFieldRingHom p m (g.1 0 0) * a +
              galoisFieldRingHom p m (g.1 0 1)
            ring
          rw [h_num, div_eq_mul_inv, mul_comm]
        | 1 =>
          change den⁻¹ * Matrix.mulVec (Matrix.GeneralLinearGroup.map
            (galoisFieldRingHom p m) g).val ![a, 1] 1 = ![num / den, 1] 1
          simp only [Matrix.mulVec, dotProduct, Fin.sum_univ_two, Matrix.cons_val_zero,
            Matrix.cons_val_one]
          change den⁻¹ * (galoisFieldRingHom p m (g.1 1 0) * a +
            galoisFieldRingHom p m (g.1 1 1) * 1) = 1
          have h_den : galoisFieldRingHom p m (g.1 1 0) * a +
              galoisFieldRingHom p m (g.1 1 1) * 1 = den := by
            change galoisFieldRingHom p m (g.1 1 0) * a +
              galoisFieldRingHom p m (g.1 1 1) * 1 = galoisFieldRingHom p m (g.1 1 0) * a +
...
theorem unique_sylow_fixing_infinity
    ...
    (hQ_fix : ∀ g ∈ (Q : Subgroup G).map (Subgroup.subtype G),
      g • infinity p = infinity p) :
    P = Q :=
  sylow_fixedPoint_injective p G hG_p (
    (eq_sylow_fixedPoint p G hG_p P _
      (fun _ hg ↦ hP_fix _ (Subgroup.mem_map_of_mem _ hg))).symm.trans
    (eq_sylow_fixedPoint p G hG_p Q _
      (fun _ hg ↦ hQ_fix _ (Subgroup.mem_map_of_mem _ hg))))
...
theorem fixes_infinity_normalizes_sylow
  ...
  · intro g_1 hg_1
    ...
    rw [mul_smul, mul_smul, inv_smul_eq_iff.mpr hg_fix.symm,
      hP_fix (k : PGL p) (Subgroup.mem_map.mpr ⟨k, hk_mem, rfl⟩), hg_fix]
  · exact hP_fix
...
theorem orbit_closed_under_translation
  ...
  translation_smul_P1point p b α ▸
    preserves_orbitInfty p G' (translationPGL p b) hb (P1point p α) hα
...
theorem translationSet_scaled_eq_Fq (m : ℕ) (hm : m ≥ 1)
  ...
  · rw [Set.ncard_image_of_injective]
    ...
    · intro x y hxy
      ...
      rw [← one_mul x, ← mul_inv_cancel₀ hv_ne, mul_assoc, hxy,
        ← mul_assoc, mul_inv_cancel₀ hv_ne, one_mul]
    ...
    have h_image : Finset.image (a * ·) S = S :=
      Finset.eq_of_subset_of_card_le
        ...
        (Finset.card_image_of_injective S
          (fun _ _ hxy ↦ mul_left_cancel₀ (fun h ↦ h0 (h ▸ ha)) hxy)).symm.le
    mul_right_cancel₀
      (Finset.prod_ne_zero_iff.mpr (fun b hb hb0 ↦ h0 (hb0 ▸ hb)))
      (calc a ^ n * ∏ b ∈ S, b = ∏ b ∈ S, (a * b) := by
              rw [← hS_card, ← Finset.prod_const, ← Finset.prod_mul_distrib]
        _ = ∏ b ∈ S, b := by
          have h_pi : ∏ x ∈ S, a * x = ∏ x ∈ S.image (fun x ↦ a * x), x :=
            (Finset.prod_image (s := S) (g := fun x ↦ a * x) (f := fun x ↦ x)
              (fun _ _ _ _ hxy ↦ mul_left_cancel₀ (fun h ↦ h0 (h ▸ ha)) hxy)).symm
          rw [h_image] at h_pi
          exact h_pi
...
theorem dilation_param_inv (g : PGL p) (c : K p) (hc : c ≠ 0)
  ...
  fun x ↦ by
    ...
    rw [inv_inv, h1, ← mul_assoc, ← mul_assoc, inv_mul_cancel,
...
theorem translationPGL_injective : Function.Injective (translationPGL p) := fun a b hab ↦ by
  have h_eq := congrArg (fun M ↦ (M.val 0 0 : K p)) <|
    Subgroup.mem_center_iff.mp (QuotientGroup.eq.mp hab)
    (Matrix.GeneralLinearGroup.mkOfDetNeZero !![(1 : K p), 0; 1, 1] (by
      simp only [Matrix.det_fin_two, Matrix.of_apply, Matrix.cons_val_zero,
        Matrix.cons_val_one, mul_one, sub_zero, ne_eq]; exact one_ne_zero))
  have h_coe_1 : ∀ h, (↑(Matrix.GeneralLinearGroup.mkOfDetNeZero !![(1 : K p), 0; 1, 1] h) :
      Matrix (Fin 2) (Fin 2) (K p)) = !![1, 0; 1, 1] := fun _ ↦ rfl
  ...
  simp only [h_coe_1, h_coe_a, h_coe_b, Matrix.inv_def, Units.val_mul,
    Matrix.GeneralLinearGroup.coe_inv, Matrix.mul_apply, Fin.sum_univ_two, Matrix.of_apply,
    Matrix.cons_val_zero, Matrix.cons_val_one, Matrix.det_fin_two, Matrix.adjugate_fin_two,
    Matrix.smul_apply, smul_eq_mul, mul_one, mul_zero, add_zero, sub_zero, neg_mul,
    Ring.inverse_one, one_mul, neg_zero] at h_eq
...
theorem dilation_param_one_is_translation (g : PGL p)
  ...
  intro M
  apply Units.ext
  have h_coe_A : ∀ h, (↑(Matrix.GeneralLinearGroup.mkOfDetNeZero !![a, b; (0 : K p), d] h) :
      Matrix (Fin 2) (Fin 2) (K p)) = !![d, b; (0 : K p), d] := by
    intro _
    change !![a, b; (0 : K p), d] = !![d, b; (0 : K p), d]
    rw [h_ad]
  have h_coe_B : (↑(translationGL p (d⁻¹ * b)) : Matrix (Fin 2) (Fin 2) (K p)) =
      !![(1 : K p), d⁻¹ * b; (0 : K p), (1 : K p)] := rfl
  have h_inv_dd : Ring.inverse (d * d) = d⁻¹ * d⁻¹ := by
    rw [Ring.inverse_eq_inv, mul_inv_rev, mul_comm]
  rw [← Matrix.ext_iff]
  simp only [Fin.forall_fin_two]
  refine ⟨⟨?_, ?_⟩, ⟨?_, ?_⟩⟩
  all_goals
    simp only [h_coe_A, h_coe_B, Units.val_mul, Matrix.GeneralLinearGroup.coe_inv,
      Matrix.mul_apply, Matrix.inv_def, Fin.sum_univ_two, Matrix.of_apply,
      Matrix.cons_val_zero, Matrix.cons_val_one, Matrix.det_fin_two,
      Matrix.adjugate_fin_two, Matrix.smul_apply, smul_eq_mul, mul_zero,
      sub_zero, neg_zero, h_inv_dd, inv_mul_cancel_right₀ hd]
    ring
        ...
        · intro x
          ...
          · have h1x := h₁ x
            ...
            have h1x_simp : g₁ * translationPGL p x = g₁ := by
              rw [← mul_one (g₁ * translationPGL p x), ← inv_mul_cancel g₁,
                ← mul_assoc, h1x, one_mul]
            have htx : translationPGL p x = 1 := by
              rw [← one_mul (translationPGL p x), ← inv_mul_cancel g₁, mul_assoc,
                h1x_simp, inv_mul_cancel]
...
theorem dilationParam_mul_eq (g₁ g₂ : PGL p)
  ...
  have h1 := dilation_param_mul p g₁ g₂ (dilationParam p g₁ hg₁)
    (dilationParam p g₂ hg₂) (dilationParam_action p g₁ hg₁) (dilationParam_action p g₂ hg₂) 1
...
theorem normalizer_element_fixes_infinity
  ...
  rw [h_inf_eq]
  exact normalizer_stabilizes_fixedPoint p G P (sylow_fixedPoint p G hG_p P)
...
theorem dilationParam_scales_translationSet
  ...
  have h_conj : (g : PGL p) * translationPGL p x * (g : PGL p)⁻¹ ∈
      Subgroup.map (Subgroup.subtype G) (P : Subgroup G) := by
    refine ⟨g * y * g⁻¹, (hg y).1 hy, ?_⟩
    change (g : PGL p) * G.subtype y * (g : PGL p)⁻¹ =
      (g : PGL p) * translationPGL p x * (g : PGL p)⁻¹
    rw [hy_eq]
  have h_action := dilationParam_action p (g : PGL p)
    (normalizer_element_fixes_infinity p G hG_p P hP_fix g hg) x
...
noncomputable def normDilationParam
    ...
    (hP_fix : ∀ g ∈ (P : Subgroup G).map (Subgroup.subtype G),
      g • infinity p = infinity p)
    (g : Subgroup.normalizer (P : Set G)) : K p :=
...
theorem normDilationParam_of_P
  ...
  apply (dilationParam_eq_one_iff p (g : PGL p)
    (normalizer_element_fixes_infinity p G hG_p P hP_fix g g.prop)).mpr
  haveI h_map_finite : Finite ((P : Subgroup G).map (Subgroup.subtype G)) :=
    Set.Finite.to_subtype <| Set.Finite.subset
      (Set.toFinite <| Set.range fun x : G ↦ x.val) (Set.image_subset_range G.subtype _)
...
theorem same_normDilationParam_imp_coset
    ...
    (hP_fix : ∀ g ∈ (P : Subgroup G).map (Subgroup.subtype G),
      g • infinity p = infinity p)
    (g₁ g₂ : Subgroup.normalizer (P : Set G))
  ...
  have h_translation : ∃ β : K p, ((g₁ : G) * (g₂ : G)⁻¹ : PGL p) = translationPGL p β :=
    (same_dilation_param_diff_translation p g₁.val g₂.val
      ...
      (fun x ↦ by
        rw [heq]
        exact dilationParam_action p _
          (normalizer_element_fixes_infinity p G hG_p P hP_fix g₂ g₂.prop) x)).imp
  ...
  exact order_one_or_p_in_sylow p G P ((g₁ : G) * (g₂ : G)⁻¹)
    (Subgroup.mul_mem _ g₁.2 (Subgroup.inv_mem _ g₂.2))
...
theorem normDilationParam_P_mul
    ...
    normDilationParam p G hG_p P hP_fix
      ⟨h * g, (Subgroup.normalizer (P : Set G)).mul_mem (Subgroup.le_normalizer hh) g.prop⟩ =
    normDilationParam p G hG_p P hP_fix g := by
  have h_norm : h ∈ Subgroup.normalizer (P : Set G) := Subgroup.le_normalizer hh
...
theorem normDilationParam_image_card
  ...
  have h_fiber_card : ∀ y ∈ S, Set.ncard {g : Subgroup.normalizer (P : Set G) |
      normDilationParam p G hG_p P hP_fix g = y} = Nat.card (P : Subgroup G) := by
    intro y hy
    obtain ⟨g, hg⟩ : ∃ g : Subgroup.normalizer (P : Set G),
      normDilationParam p G hG_p P hP_fix g = y := hy
    calc
      Set.ncard {x : Subgroup.normalizer (P : Set G) | normDilationParam p G hG_p P hP_fix x = y}
        = Set.ncard (Set.image (fun h : (P : Subgroup G) ↦ (⟨h.val * g.val,
          Subgroup.mul_mem _ (Subgroup.le_normalizer h.prop) g.prop⟩ :
            Subgroup.normalizer (P : Set G))) Set.univ) := by
          congr 1
          ext x
          simp only [Set.mem_setOf_eq, Set.image_univ, Set.mem_range]
          constructor
          · intro hx
            exact Exists.intro ⟨(x : G) * (g : G)⁻¹,
              same_normDilationParam_imp_coset p G hG_p P hP_fix x g (hx.trans hg.symm)⟩
              (Subtype.ext (inv_mul_cancel_right (x : G) (g : G)))
          · rintro ⟨h, rfl⟩
            exact (normDilationParam_P_mul p G hG_p P hP_fix g h.val h.prop).trans hg
      _ = Set.ncard (Set.univ : Set (P : Subgroup G)) :=
          Set.ncard_image_of_injective (f := fun h : (P : Subgroup G) ↦
            (⟨h.val * g.val, Subgroup.mul_mem _ (Subgroup.le_normalizer h.prop) g.prop⟩ :
              Subgroup.normalizer (P : Set G))) Set.univ
            (fun h₁ h₂ h_eq ↦ Subtype.ext (mul_right_cancel (Subtype.ext_iff.mp h_eq)))
      _ = Nat.card (P : Subgroup G) := Set.ncard_univ (P : Subgroup G)
  have h_card_S_mul : Set.ncard S * Nat.card (P : Subgroup G) =
      Nat.card (Subgroup.normalizer (P : Set G)) := by
    have h_sum : ∀ s : Finset (K p), (∑ y ∈ s, Set.ncard {g : Subgroup.normalizer (P : Set G) |
      normDilationParam p G hG_p P hP_fix g = y}) =
      Set.ncard {g : Subgroup.normalizer (P : Set G) |
        normDilationParam p G hG_p P hP_fix g ∈ s} := by
      intro s
      induction s using Finset.induction with
      | empty =>
        calc
          (∑ y ∈ (∅ : Finset (K p)), Set.ncard {g : Subgroup.normalizer (P : Set G) |
            normDilationParam p G hG_p P hP_fix g = y})
            = 0 := Finset.sum_empty
          _ = Set.ncard (∅ : Set (Subgroup.normalizer (P : Set G))) :=
              Eq.symm (Set.ncard_empty (Subgroup.normalizer (P : Set G)))
          _ = Set.ncard {g : Subgroup.normalizer (P : Set G) |
                normDilationParam p G hG_p P hP_fix g ∈ (∅ : Finset (K p))} := by
            congr 1
            ext g
            exact ⟨False.elim, Finset.notMem_empty _⟩
      | insert y s hy ih =>
        rw [Finset.sum_insert hy, ih, ← Set.ncard_union_eq]
        · congr 1
          ext g
          simp only [Set.mem_setOf_eq, Finset.mem_insert, Set.mem_union]
    ...
    calc
      ...
      _ = ∑ y ∈ (Set.toFinite S).toFinset, Set.ncard {g : Subgroup.normalizer (P : Set G) |
            normDilationParam p G hG_p P hP_fix g = y} := by
          apply Finset.sum_congr rfl
          intro y hy
          rw [Set.Finite.mem_toFinset] at hy
          exact (h_fiber_card y hy).symm
      _ = Set.ncard {g : Subgroup.normalizer (P : Set G) |
            normDilationParam p G hG_p P hP_fix g ∈ (Set.toFinite S).toFinset} :=
          h_sum (Set.toFinite S).toFinset
      _ = Set.ncard (Set.univ : Set (Subgroup.normalizer (P : Set G))) := by
          congr 1
          ext g
          rw [Set.mem_setOf_eq, Set.Finite.mem_toFinset]
          exact ⟨fun _ ↦ Set.mem_univ g, fun _ ↦ ⟨g, rfl⟩⟩
      _ = Nat.card (Subgroup.normalizer (P : Set G)) :=
          Set.ncard_univ (Subgroup.normalizer (P : Set G))
  exact ((Nat.div_eq_of_eq_mul_left Nat.card_pos h_card_S_mul.symm).symm :
    Set.ncard S = normalizerQuotient p G P).trans
      (z1_eq_pm_minus_one p G m hm hG P (n_p_gt_one_of_pgl_order p G m hm hG))
...
theorem dilation_params_cover
    ...
    ∃ S : Finset (K p),
      ...
      (∀ a ∈ S, a⁻¹ ∈ S) ∧
      (∀ a ∈ S, a ≠ 0) ∧
      (∀ c ∈ S, ∃ g : G, g ∈ Subgroup.normalizer (P : Set G) ∧
        ∀ x ∈ translationSet p (Subgroup.map (Subgroup.subtype G) (P : Subgroup G)),
  ...
  · intro a ha
    ...
    rw [inv_mul_cancel, normDilationParam_of_P p G hG_p P hP_fix 1
      (Subgroup.one_mem (P : Subgroup G))] at h_mul
  ...
  · intro a ha
    ...
    obtain ⟨g_a, h_ga⟩ := ha
    rw [← h_ga]
    exact dilationParam_ne_zero p _
      (normalizer_element_fixes_infinity p G hG_p P hP_fix g_a.val g_a.prop)
  · intro c hc
...
theorem translationSet_card_eq_P
  ...
  have h_iso : (P : Subgroup G) ≃* H :=
    Subgroup.equivMapOfInjective (P : Subgroup G) G.subtype (Subgroup.subtype_injective G)
  ...
  · haveI : Subsingleton H := (Nat.card_eq_one_iff_unique.mp (h_card_eq.trans h_card)).1
    have h_eq_zero : translationSet p H = {0} := Set.ext fun b ↦
      ⟨fun hb ↦ translationPGL_injective p ((congrArg Subtype.val
        (Subsingleton.elim (⟨translationPGL p b, hb⟩ : H) 1)).trans
          (translationPGL_zero p).symm),
  ...
  · haveI : Finite H := Finite.of_equiv (P : Subgroup G) h_iso.toEquiv
    exact (p_subgroup_fixing_infinity_translations p H (P.2.map (Subgroup.subtype G))
      hP_fix).2.trans h_card_eq
...
theorem orbitInfty_ncard
  ...
  have h_stab_eq : MulAction.stabilizer G (infinity p) = Subgroup.normalizer (P : Set G) :=
    Subgroup.ext fun g ↦ ⟨fixes_infinity_normalizes_sylow p G hG_p P hP_fix g,
      normalizer_element_fixes_infinity p G hG_p P hP_fix g⟩
  have h_orbit_eq : orbitInfty p G = MulAction.orbit G (infinity p) :=
    Set.ext fun x ↦ ⟨fun ⟨g, hg, h⟩ ↦ ⟨⟨g, hg⟩, h⟩, fun ⟨g, h⟩ ↦ ⟨g.val, g.prop, h⟩⟩
  rw [h_orbit_eq, ← Nat.card_coe_set_eq,
    Nat.card_congr (MulAction.orbitEquivQuotientStabilizer G (infinity p))]
...
theorem orbit_eq_infty_union_translations
  ...
  have h_fin_H : (translationSet p H).Finite := Set.finite_of_ncard_pos <| by
    rw [translationSet_card_eq_P p G P hP_fix]
    exact Nat.card_pos
  ...
  have h_card_B : Set.ncard ({infinity p} ∪
      P1point p '' ((fun b ↦ α₀ + b) '' translationSet p H)) = p ^ m + 1 := by
    rw [Set.ncard_union_eq h_disj (Set.finite_singleton _)
      (Set.Finite.image _ <| Set.Finite.image _ h_fin_H)]
    rw [Set.ncard_singleton, Set.ncard_image_of_injective _ h_P1_inj,
      Set.ncard_image_of_injective _ h_add_inj, translationSet_card_eq_P p G P hP_fix]
    have := sylow_order_of_pgl_order p G m hm hG P
    omega
  have h_sub : {infinity p} ∪ P1point p '' ((fun b ↦ α₀ + b) '' translationSet p H) ⊆
      orbitInfty p G := by
    ...
    · intro b hb
      ...
      exact hg_eq ▸ g.prop
    · exact hα₀
  have h_card_eq : Set.ncard (orbitInfty p G) =
      Set.ncard ({infinity p} ∪ P1point p '' ((fun b ↦ α₀ + b) '' translationSet p H)) :=
...
theorem orbit_translate_conj (G' : Subgroup (PGL p))
  ...
  rw [orbitInfty_conj_of_fixes_infty p G' (translationPGL p (-α₀))
    (translation_smul_infinity p (-α₀))]
...
theorem orbit_infty_eq_P1Fq_core
  ...
  obtain ⟨α₀, hα₀⟩ : ∃ α₀ : K p, P1point p α₀ ∈ orbitInfty p G := by
    ...
    have h_np_eq_one : Fintype.card (Sylow p G) = 1 :=
      (orbitInfty_ncard p G hG_p P hP_fix).symm.trans h_card_one
    exact lt_irrefl 1 <| h_np_eq_one ▸ n_p_gt_one_of_pgl_order p G m hm hG
  have h_stable_translation : ∀ c : K p, c ^ (p ^ m - 1) = 1 → c ≠ 0 →
      ∀ x, x ∈ translationSet p H → c * x ∈ translationSet p H :=
    fun c hc hc_ne ↦ by
      obtain ⟨S, hS_card, hS_subgroup, hS_mul, hS_inv, hS_ne_zero, hS_dilation⟩ :=
        dilation_params_cover p G m hm hG P hP_fix
      exact (hS_dilation c (finite_subgroup_eq_roots_of_unity p S (p ^ m - 1)
        ...
        hS_card hS_mul hS_ne_zero c hc hc_ne)).choose_spec.2
  obtain ⟨V, rfl, hV_card, hV_add, hV_zero, hV_neg, hV_stable⟩ : ∃ V : Set (K p),
      V = translationSet p H ∧ Set.ncard V = p ^ m ∧ (∀ x y, x ∈ V → y ∈ V → x + y ∈ V) ∧
      (0 : K p) ∈ V ∧ (∀ x, x ∈ V → -x ∈ V) ∧
      (∀ c : K p, c ^ (p ^ m - 1) = 1 → c ≠ 0 → ∀ x, x ∈ V → c * x ∈ V) :=
    ⟨translationSet p H, rfl, (translationSet_card_eq_P p G P hP_fix).trans
      (sylow_order_of_pgl_order p G m hm hG P), translationSet_add p H,
      translationSet_zero p H, translationSet_neg p H, h_stable_translation⟩
  obtain ⟨v₀, hv₀_mem, hv₀_ne⟩ : ∃ v₀ : K p, v₀ ∈ translationSet p H ∧ v₀ ≠ 0 := by
    ...
    exact lt_irrefl 1 <| h_pow_eq_one ▸ hp_gt_one.trans_le
      ((pow_one p).symm.trans_le (Nat.pow_le_pow_right hp_gt_one.le hm))
  use dilationPGL p v₀⁻¹ (inv_ne_zero hv₀_ne) * translationPGL p (-α₀)
  refine ⟨?_, ?_⟩
  · exact (mul_smul (dilationPGL p v₀⁻¹ (inv_ne_zero hv₀_ne))
      (translationPGL p (-α₀)) (infinity p)).trans <|
      (congrArg (fun x ↦ dilationPGL p v₀⁻¹ (inv_ne_zero hv₀_ne) • x)
        (translation_smul_infinity p (-α₀))).trans <|
      dilation_smul_infinity p v₀⁻¹ (inv_ne_zero hv₀_ne)
  · have h_map_eq : G.map (MulEquiv.toMonoidHom (MulAut.conj
        (dilationPGL p v₀⁻¹ (inv_ne_zero hv₀_ne) * translationPGL p (-α₀)))) =
      (G.map (MulEquiv.toMonoidHom (MulAut.conj (translationPGL p (-α₀))))).map
        (MulEquiv.toMonoidHom (MulAut.conj (dilationPGL p v₀⁻¹ (inv_ne_zero hv₀_ne)))) := by
      rw [Subgroup.map_map]
      congr 1
      ext x
    ...
    have h_orbit := orbit_translate_conj p G α₀ (translationSet p H)
      (orbit_eq_infty_union_translations p G m hm hG hG_p P hP_fix α₀ hα₀)
    have h_orbit_dilation := orbit_dilation_conj p _ v₀⁻¹ (inv_ne_zero hv₀_ne)
      (translationSet p H) h_orbit
    have h_orbit_dilation_eq := translationSet_scaled_eq_Fq p m hm
      (translationSet p H) hV_card hV_zero hV_stable v₀ hv₀_mem hv₀_ne
...
theorem exists_conj_sylow_fixing_infty
  ...
  calc g' • infinity p
    ...
    _ = k₀ • ↑x • k₀⁻¹ • infinity p :=
      (mul_smul (k₀ * ↑x) k₀⁻¹ (infinity p)).trans (mul_smul k₀ (↑x) (k₀⁻¹ • infinity p))
    _ = k₀ • ↑x • α := congrArg (fun y ↦ k₀ • ↑x • y) (hk₀ ▸ inv_smul_smul k₀ α)
    _ = k₀ • α := congrArg (fun y ↦ k₀ • y) (hα ↑x (Subgroup.mem_map_of_mem _ hx_in_P))
    _ = infinity p := hk₀

/-- A subgroup of `PGL(2, K)` with the correct order and Sylow `p`-subgroup size
has an orbit of infinity exactly equal to `P¹(F_q)` under appropriate conjugation. -/
theorem orbit_infty_eq_P1Fq
  ...
  obtain ⟨k₀, P₁, hP₁⟩ := exists_conj_sylow_fixing_infty p G
  ...
  have h_map_eq : G₁.map (MulEquiv.toMonoidHom (MulAut.conj g)) =
      G.map (MulEquiv.toMonoidHom (MulAut.conj (g * k₀))) := by
    rw [Subgroup.map_map]
    congr 1
    ext x
...
theorem pgl_conjugate_to_standard
  ...
  use g
  have h_le : G.map (MulEquiv.toMonoidHom (MulAut.conj g)) ≤
      (pglMap (galoisFieldRingHom (p := p) m)).range :=
    fun h hh ↦ preserves_P1Fq_in_range p m hm h
      (fun x hx ↦ hg ▸ preserves_orbitInfty p _ h hh x (hg ▸ hx))
  have h_card_Gmap : Nat.card (G.map (MulEquiv.toMonoidHom (MulAut.conj g))) = Nat.card G :=
    (Nat.card_congr (conjEquiv p G g).toEquiv).symm
  have h_card_range : Nat.card ((pglMap (galoisFieldRingHom (p := p) m)).range) =
      Nat.card G :=
    (Nat.card_congr (Equiv.ofInjective _
      (pglMap_injective (galoisFieldRingHom (p := p) m) (RingHom.injective _)))).symm.trans
      (by rw [card_PGL2, Fintype.card_eq_nat_card.trans (GaloisField.card p m (ne_of_gt hm)),
        ← pow_mul', ← hG])
  have h_card_eq : Nat.card (G.map (MulEquiv.toMonoidHom (MulAut.conj g))) =
      Nat.card ((pglMap (galoisFieldRingHom (p := p) m)).range) :=
    h_card_Gmap.trans h_card_range.symm
  by_contra h_neq
  have h_ssubset : (G.map (MulEquiv.toMonoidHom (MulAut.conj g)) : Set (PGL p)) ⊂
      ((pglMap (galoisFieldRingHom (p := p) m)).range : Set (PGL p)) :=
    Set.ssubset_iff_subset_ne.mpr ⟨h_le,
      fun h_set_eq ↦ h_neq (Subgroup.ext (Set.ext_iff.mp h_set_eq))⟩
  have h_lt : Nat.card (G.map (MulEquiv.toMonoidHom (MulAut.conj g))) <
      Nat.card ((pglMap (galoisFieldRingHom (p := p) m)).range) :=
    Set.ncard_lt_ncard h_ssubset (Set.finite_range _)
  exact (ne_of_lt h_lt) h_card_eq

/-- Any two finite subgroups of `PGL(2, K)` of order `|PGL(2, p^m)|` are conjugate. -/
theorem pgl_subgroups_conjugate
  ...
  have h_map_comp : MulEquiv.toMonoidHom (MulAut.conj (g₂⁻¹ * g₁)) =
      (MulEquiv.toMonoidHom (MulAut.conj g₂).symm).comp
        (MulEquiv.toMonoidHom (MulAut.conj g₁)) := by
    ext x
    simp only [MonoidHom.comp_apply, MulEquiv.coe_toMonoidHom,
      MulAut.conj_apply, MulAut.conj_symm_apply]
    group
  have h_symm_comp : (MulEquiv.toMonoidHom (MulAut.conj g₂).symm).comp
      (MulEquiv.toMonoidHom (MulAut.conj g₂)) = MonoidHom.id _ :=
    MonoidHom.ext fun x ↦ (MulAut.conj g₂).symm_apply_apply x
  calc G.map (MulEquiv.toMonoidHom (MulAut.conj (g₂⁻¹ * g₁)))
    _ = (G.map (MulEquiv.toMonoidHom (MulAut.conj g₁))).map
        (MulEquiv.toMonoidHom (MulAut.conj g₂).symm) := by
          rw [h_map_comp, ← Subgroup.map_map]
    _ = ((pglMap (galoisFieldRingHom (p := p) m)).range).map
        (MulEquiv.toMonoidHom (MulAut.conj g₂).symm) := by rw [hg₁]
    _ = (H.map (MulEquiv.toMonoidHom (MulAut.conj g₂))).map
        (MulEquiv.toMonoidHom (MulAut.conj g₂).symm) := by rw [← hg₂]
