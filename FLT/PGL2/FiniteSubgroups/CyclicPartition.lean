...
This file formalises the *cyclic partition* approach to
recognising finite groups: given a group whose non-identity
elements are partitioned into conjugacy classes of cyclic
subgroups with prescribed orders and normaliser indices,
the group is identified up to isomorphism.

## Main definitions

- `CyclicPartitionConfig`: a record of a cyclic subgroup
  order `d` and normaliser multiplier `f`.
- `HasCyclicPartition G configs`: the proposition that `G`
  admits a cyclic partition matching the given list of
  configurations.

## Main results

- `dihedralRecognition`: a group with a normal cyclic
  subgroup of index 2 where all outside elements have
  order 2 is isomorphic to a dihedral group.
- `dihedral_of_hasCyclicPartition_odd`: recognition of
  `D_n` (odd `n ≥ 3`) from its cyclic partition.
- `dihedral_of_hasCyclicPartition_even`: recognition of
  `D_n` (even `n ≥ 4`) from its cyclic partition.
- `iso_A4_of_hasCyclicPartition`: recognition of `A₄`
  from its cyclic partition with `|G| = 12`.
- `iso_S4_of_hasCyclicPartition`: recognition of `S₄`
  from its cyclic partition with `|G| = 24`.
...
/-- Configuration data for one conjugacy class of cyclic
subgroups: `d` is the subgroup order and `f` is the
normaliser-to-subgroup ratio. -/
structure CyclicPartitionConfig where
  d : ℕ
  f : ℕ

/-- A group `G` has a cyclic partition matching `configs` if its
non-identity elements are partitioned into conjugacy classes of
cyclic subgroups whose orders and normaliser indices match. -/
def HasCyclicPartition (G : Type*) [Group G] [Finite G]
  ...
  ∃ (H : List (Subgroup G)),
    ...
    (∀ (i : Fin configs.length),
      ...
      IsCyclic Hi ∧
      Nat.card Hi = ci.d ∧
      Nat.card (Subgroup.normalizer (Hi : Set G)) = ci.f * ci.d) ∧
...
theorem isCyclic_of_hasCyclicPartition (G : Type*) [Group G] [Finite G] (N : ℕ)
    (hN : Nat.card G = N)
...
theorem dihedralRecognition (G : Type*) [Group G] [Finite G] (n : ℕ) (hn : n ≥ 2)
    (H : Subgroup G) [hH_normal : H.Normal]
  ...
  have hb_pow_mul_a : ∀ (x : ℕ), b ^ x * a = a * (b ^ x)⁻¹ := by
    intro x
    induction x with
    | zero => simp only [pow_zero, inv_one, one_mul, mul_one]
    | succ x ih =>
  ...
  have h_coset : ∀ g : G, g ∉ H → ∃ h ∈ H, g = a * h := by
    ...
    have hgx : g * x ∈ H := by
      rcases hx g with ⟨h1, _⟩ | ⟨h1, _⟩
      · exact h1
      · exact absurd h1 hg
    have hax : a * x ∈ H := by
      rcases hx a with ⟨h1, _⟩ | ⟨h1, _⟩
      · exact h1
      · exact absurd h1 ha
    exact ⟨a⁻¹ * g,
        Subgroup.Normal.mem_comm hH_normal (show g * a⁻¹ ∈ H from
...
lemma extractDihedralStructure (G : Type*) [Group G] [Finite G] (n : ℕ)
    (hN : Nat.card G = 2 * n) (hn : n ≥ 2)
    (H₁ : Subgroup G)
    (hH₁_card : Nat.card H₁ = n)
    (hH₁_norm : Nat.card (Subgroup.normalizer (H₁ : Set G)) = 2 * n)
...
theorem dihedral_of_hasCyclicPartition_odd (G : Type*) [Group G] [Finite G] (n : ℕ)
    (hn_ge : n ≥ 3)
    (hN : Nat.card G = 2 * n)
...
theorem dihedral_of_hasCyclicPartition_even (G : Type*) [Group G] [Finite G] (n : ℕ)
    (hn_ge : n ≥ 4)
    (hN : Nat.card G = 2 * n)
...
theorem sylow_inf_eq_bot_of_prime_card (G : Type*) [Group G] [Finite G]
...
theorem sylow3_toPermHom_ker_eq_bot_of_card_12 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 12) (h_sylow : Nat.card (Sylow 3 G) = 4) :
    (MulAction.toPermHom G (Sylow 3 G)).ker = ⊥ := by
  have h_norm_card : ∀ P : Sylow 3 G,
      Nat.card (Subgroup.normalizer (P : Set G)) = 3 := fun P ↦ by
    have h_mul := Subgroup.index_mul_card
      (Subgroup.normalizer (P : Set G))
    change (Subgroup.normalizer (P : Set G)).index * _ = _ at h_mul
    rw [← Sylow.card_eq_index_normalizer, h_sylow, hN] at h_mul
    omega
  have h_P_card : ∀ P : Sylow 3 G, Nat.card (P : Subgroup G) = 3 := fun P ↦ by
    have h_dvd :=
      Subgroup.card_dvd_of_le (show (P : Set G) ≤
        Subgroup.normalizer (P : Set G) from Subgroup.le_normalizer)
    rw [h_norm_card P] at h_dvd
    cases (Nat.dvd_prime Nat.prime_three).mp h_dvd with
    | inl h_one =>
      have h_norm_top : (Subgroup.normalizer (P : Set G)) = ⊤ := by
        ext x
        refine ⟨fun _ ↦ Subgroup.mem_top x, fun _ ↦ by
          rw [Subgroup.mem_set_normalizer_iff, ← Sylow.coe_coe P, Subgroup.card_eq_one.mp h_one]
          exact fun y ↦ ⟨
            fun hy ↦ Subgroup.mem_bot.mpr (conj_eq_one_iff.mpr (Subgroup.mem_bot.mp hy)),
            fun hy ↦ Subgroup.mem_bot.mpr (conj_eq_one_iff.mp (Subgroup.mem_bot.mp hy))
          ⟩⟩
      have h_mul := Subgroup.index_mul_card
        (Subgroup.normalizer (P : Set G) : Subgroup G)
      rw [h_norm_card P, h_norm_top, Subgroup.index_top, one_mul, hN] at h_mul
      omega
    | inr h_three =>
      exact h_three
  have h_norm_eq : ∀ P : Sylow 3 G,
      (Subgroup.normalizer (P : Set G)) = P := by
    intro P
    have h_set_eq : ((P : Subgroup G) : Set G) =
...
lemma iso_A4_of_card_12_sylow3_4 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 12) (h_sylow : Nat.card (Sylow 3 G) = 4) :
  ...
  let iso_perm : Equiv.Perm (Sylow 3 G) ≃* Equiv.Perm (Fin 4) := {
    ...
    map_mul' := fun _ _ ↦ Equiv.ext fun _ ↦ by simp only [Equiv.toFun_as_coe,
      Equiv.permCongr_apply, Equiv.Perm.mul_apply, Equiv.symm_apply_apply]
  }
...
lemma card_sylow3_of_non_normal_order3 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 12)
  ...
  · exfalso
    ...
    have hHP : H = (P : Subgroup G) := by
          ...
          change Nat.card (P : Subgroup G) ≤ Nat.card H
          rw [hk, hk1, pow_one]; omega
    exact hH_not_normal (hHP ▸ Sylow.normal_of_subsingleton P)
  · exact h_four

theorem iso_A4_of_hasCyclicPartition (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 12)
...
lemma card_sylow3_of_hasCyclicPartition_S4 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 24)
  ...
  change Nat.card (Subgroup.normalizer (H₁ : Set G)) = 6 at hH₁_norm
  ...
  have hH₁_eq_Q : H₁ = Q.toSubgroup := SetLike.ext' <|
    Set.eq_of_subset_of_ncard_le hQ_le (by change Nat.card Q.toSubgroup ≤ Nat.card H₁; rw [hQ_card,
        hH₁_card])
  have h_idx := Subgroup.index_mul_card (Subgroup.normalizer (Q.toSubgroup : Set G))
  change (Subgroup.normalizer (Q : Set G)).index * _ = _ at h_idx
  rw [← Sylow.card_eq_index_normalizer Q, ← hH₁_eq_Q, hH₁_norm, hN] at h_idx
  omega

lemma center_eq_bot_of_hasCyclicPartition (G : Type*) [Group G] [Finite G] (N : ℕ)
    (hN : Nat.card G = N)
  ...
  have h_norm_top : Subgroup.normalizer (Hi : Set G) = ⊤ := by
  ...
  have h_card_norm : Nat.card (Subgroup.normalizer (Hi : Set G)) = N := by
    rw [h_norm_top, ← hN]
...
lemma normal_order2_le_center (G : Type*) [Group G] [Finite G]
    (K : Subgroup G) [hK_normal : K.Normal] (hK : Nat.card K = 2) :
  ...
  have hk_center : k ∈ Subgroup.center G := by
    ...
    rcases h_cases (g * k * g⁻¹)
        (Subgroup.Normal.conj_mem hK_normal k k_sub.property g) with h1 | hk_eq
...
lemma card_sylow_eq_one_of_normal (G : Type*) [Group G] [Finite G]
  ...
  exact ⟨P, fun Q ↦ by
    ...
    change g ∈ Subgroup.normalizer ((P : Subgroup G) : Set G)
    rw [← hP, Subgroup.normalizer_eq_top]
    exact Subgroup.mem_top g⟩

lemma not_normal_order6_of_card_sylow3_4 (G : Type*) [Group G] [Finite G]
  ...
  obtain ⟨Q, hQ⟩ := Fintype.card_eq_one_iff.mp <| show Fintype.card (Sylow 3 K) = 1 by
    have h_dvd : Nat.card (Sylow 3 K) ∣ 6 := by
      ...
      exact Subgroup.card_quotient_dvd_card (Subgroup.normalizer
        (Classical.arbitrary (Sylow 3 K) : Set K))
    ...
    omega
  have hQ_normal : (Q : Subgroup K).Normal := ⟨fun n hn g ↦
    (Sylow.smul_eq_iff_mem_normalizer.mp (show g • Q = Q from hQ (g • Q))) n |>.mp hn⟩
  haveI : (Q : Subgroup K).Characteristic := Sylow.characteristic_of_normal Q hQ_normal
  have hQG_normal : (Subgroup.map K.subtype (Q : Subgroup K)).Normal :=
  ...
  haveI : (P : Subgroup G).Normal :=
    (SetLike.ext' <| Set.eq_of_subset_of_ncard_le hP h_card_eq.ge : QG = ↑P) ▸ hQG_normal
  have h_sylow_one : Nat.card (Sylow 3 G) = 1 := by
    rw [Nat.card_eq_fintype_card, Fintype.card_eq_one_iff]
    exact ⟨P, fun R ↦ by
      obtain ⟨g, rfl⟩ := MulAction.exists_smul_eq G P R
      exact Sylow.smul_eq_iff_mem_normalizer.mpr
        ((P : Subgroup G).normalizer_eq_top.symm ▸ Subgroup.mem_top g)⟩
  omega

lemma toPermHom_injective_of_card_24 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 24)
  ...
  have hK_dvd : Nat.card K ∣ 6 :=
    (show Nat.card (Subgroup.normalizer (P : Set G)) = 6 from
      (show 4 * Nat.card (Subgroup.normalizer (P : Set G)) = 24 →
        Nat.card (Subgroup.normalizer (P : Set G)) = 6 by intros; omega)
        (show 4 * Nat.card (Subgroup.normalizer (P : Set G)) = 24 by
          rw [← h_sylow, Sylow.card_eq_index_normalizer P, ← hN]
          exact (Subgroup.normalizer (P : Set G)).index_mul_card)
...
theorem iso_S4_of_hasCyclicPartition (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 24)
    (h : HasCyclicPartition G [{ d := 2, f := 2 }, { d := 3, f := 2 }, { d := 4, f := 2 }]) :
...
lemma orderOf_mem_of_hasCyclicPartition_A5 (G : Type*) [Group G] [Finite G]
...
lemma card_sylow5_of_hasCyclicPartition_A5 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 60)
    (h : HasCyclicPartition G [{ d := 2, f := 2 }, { d := 3, f := 2 }, { d := 5, f := 2 }]) :
  ...
  change (Subgroup.normalizer ((P : Subgroup G) : Set G)).index = _
  rw [hP_eq]
  have h_mul := (Subgroup.normalizer (H₂ : Set G)).index_mul_card
  rw [hH₂_norm_card, hN] at h_mul
  exact Nat.eq_of_mul_eq_mul_right (by norm_num) h_mul

lemma card_sylow3_of_hasCyclicPartition_A5 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 60)
    (h : HasCyclicPartition G [{ d := 2, f := 2 }, { d := 3, f := 2 }, { d := 5, f := 2 }]) :
...
lemma sylow_le_normal_of_coprime_index (G : Type*) [Group G] [Finite G]
  ...
  rcases n with _ | n
  · intro x hx
    have h_sub : Subsingleton ↥(P.map (QuotientGroup.mk' N)) :=
      (Nat.card_eq_one_iff_unique.mp (by rw [hn, pow_zero])).1
...
lemma card_elements_orderOf_prime (G : Type*) [Group G] [Finite G]
    (p : ℕ) [hp : Fact (Nat.Prime p)]
...
lemma mem_of_orderOf_prime_sylow_le (G : Type*) [Group G] [Finite G]
...
lemma exists_orderOf_15_of_card_15 (H : Type*) [Group H] [Finite H]
...
lemma card_ge_45_of_orders_3_and_5 (G : Type*) [Group G] [Finite G]
    (hG : Nat.card G = 60)
...
lemma not_normal_of_card_lt (G : Type*) [Group G] [Finite G]
    (p : ℕ) [Fact (Nat.Prime p)]
...
lemma isSimple_of_hasCyclicPartition_A5 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 60)
    (h : HasCyclicPartition G [{ d := 2, f := 2 }, { d := 3, f := 2 }, { d := 5, f := 2 }]) :
    IsSimpleGroup G := by
  ...
  exact IsSimpleGroup.mk (fun N hN_normal ↦ by
    ...
    · rcases h31_59 with ⟨h31, _⟩
      ...
      have h_lt : k < 2 := by
        by_contra! h
        have h_mul_le := Nat.mul_le_mul h31 h
        rw [← hk] at h_mul_le
        exact absurd h_mul_le (by norm_num)
...
lemma exists_normal_complement_of_sylow2_15 (G : Type*) [Group G] [Finite G]
  ...
  have h_centralizer :
    ...
    fun x hx y hy ↦
      haveI := Fintype.ofFinite (Subgroup.normalizer (P : Set G))
      (Subtype.ext_iff.mp ((IsPGroup.isMulCommutative_of_card_eq_prime_sq (show Nat.card
        (Subgroup.normalizer (P : Set G)) = 2 ^ 2 by
          have h_card_eq := Subgroup.card_eq_card_quotient_mul_card_subgroup
            (Subgroup.normalizer (P : Set G))
          change _ = Nat.card (G ⧸ Subgroup.normalizer (P : Set G)) * _ at h_card_eq
          rw [← Nat.card_congr (Sylow.equivQuotientNormalizer P), h_n2, hN] at h_card_eq; omega
...
lemma card_sylow2_ne_one_of_simple_60 (G : Type*) [Group G] [Finite G]
...
lemma card_sylow2_ne_three_of_simple_60 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 60) (hSimple : IsSimpleGroup G) :
...
lemma card_sylow2_eq_five_of_simple_60 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 60) (hSimple : IsSimpleGroup G) :
...
lemma toPermHom_injective_of_simple_60 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 60) (hSimple : IsSimpleGroup G) :
...
lemma iso_A5_of_simple_60 (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 60) (hSimple : IsSimpleGroup G) :
...
theorem iso_A5_of_hasCyclicPartition (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 60)
    (h : HasCyclicPartition G [{ d := 2, f := 2 }, { d := 3, f := 2 }, { d := 5, f := 2 }]) :
