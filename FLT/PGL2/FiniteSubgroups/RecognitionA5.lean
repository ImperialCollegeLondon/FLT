  ...
  two_fixed_points_contradicts_order_p p (g^2) x y
    (by
      rw [orderOf_pow', hg_order, Nat.gcd_comm, Nat.gcd_rec,
          (Nat.Prime.eq_two_or_odd (Fact.out : Nat.Prime p) |>
            Or.resolve_left <| ne_of_gt h_odd.out),
...
theorem no_commute_p_coprime (g h : PGL p) (hh_ne : h ≠ 1)
    ...
    (hcomm : g * h = h * g) :
    False := by
  obtain ⟨x, y, hxy, h_fixed⟩ := Set.ncard_eq_two.mp
    (coprime_order_two_fixed_points p h hh_ne hh_fin hh_coprime)
  ...
  match (H_fix (g • x)).mp
          (commute_preserves_fixedPoint p g h x hcomm ((H_fix x).mpr (Or.inl rfl))),
        (H_fix (g • y)).mp
          (commute_preserves_fixedPoint p g h y hcomm ((H_fix y).mpr (Or.inr rfl))) with
  | Or.inl hgx, Or.inl hgy => exact absurd (MulAction.injective g (hgx.trans hgy.symm)) hxy
  | Or.inl hgx, Or.inr hgy =>
    exact two_fixed_points_contradicts_order_p p g x y hg_order hxy hgx hgy
...
theorem element_dichotomy (G : Subgroup (PGL p)) [Finite G]
  ...
  have hn : orderOf (g : PGL p) ≠ 0 := (isOfFinOrder_iff_pow_eq_one.mpr ⟨orderOf g,
    orderOf_pos _, by
      rw [← Subgroup.coe_pow, pow_orderOf_eq_one, Subgroup.coe_one]⟩).orderOf_pos.ne'
  ...
  · by_cases hb_one : b = 1
    ...
    · have hgb_ord : orderOf ((g : PGL p) ^ b) = p := by
        rw [orderOf_pow' _ hb_pos.ne', hab, pow_one]
        rw [Nat.gcd_eq_right ⟨p, mul_comm p b⟩, Nat.mul_div_cancel _ hb_pos]
      have hgp_ord : orderOf ((g : PGL p) ^ p) = b := by
        rw [orderOf_pow' _ (Fact.out : Nat.Prime p).ne_zero, hab, pow_one]
        rw [Nat.gcd_eq_right ⟨b, rfl⟩, mul_comm p b, Nat.mul_div_cancel _ (Nat.Prime.pos Fact.out)]
      exfalso
  ...
  · have hgb_ord : orderOf ((g : PGL p) ^ b) = p ^ (a + 2) := by
      rw [orderOf_pow' _ hb_pos.ne', hab]
      rw [Nat.gcd_eq_right ⟨p ^ (a + 2), mul_comm (p ^ (a + 2)) b⟩, Nat.mul_div_cancel _ hb_pos]
    haveI : Finite (Subgroup.zpowers ((g : PGL p) ^ b)) :=
      Nat.finite_of_card_ne_zero (by
        rw [Nat.card_zpowers, hgb_ord]; exact (pow_pos (Nat.Prime.pos Fact.out) _).ne')
    ...
    have hpgrp : IsPGroup p (Subgroup.zpowers ((g : PGL p) ^ b)) := fun ⟨x, hx⟩ ↦
      ⟨a + 2, Subtype.ext (orderOf_dvd_iff_pow_eq_one.mp
        (hgb_ord ▸ orderOf_dvd_of_mem_zpowers hx))⟩
    ...
    have h_p_lt_pa : p < p ^ (a + 2) := by
      calc p = p ^ 1 := (pow_one p).symm
           _ < p ^ (a + 2) := Nat.pow_lt_pow_right (Fact.out : Nat.Prime p).one_lt
                                (Nat.succ_lt_succ (Nat.zero_lt_succ a))
...
lemma order_3_centralizes_order_5 {G : Type*} [Group G] [Finite G]
  ...
  · have hq_ord : orderOf q = 5 := by
      have h1 : orderOf q ∣ 5 :=
        hQ_card ▸ (Nat.card_zpowers q).symm ▸ Subgroup.card_dvd_of_le (Subgroup.zpowers_le.mpr hq)
    ...
    obtain ⟨a, ha⟩ : ∃ a : ℤ, g * q * g⁻¹ = q^a := by
      ...
      use b
      have ha' : q = (x : G) ^ a :=
        (congrArg Subtype.val ha).symm.trans (Subgroup.coe_zpow Q x a)
      have hb' : g * (x : G) * g⁻¹ = (x : G) ^ b :=
        (congrArg Subtype.val hb).symm.trans (Subgroup.coe_zpow Q x b)
      calc
    ...
    have h_pow_conj : ∀ n : ℕ, g^n * q * (g^n)⁻¹ = q^(a^n) := by
      ...
      | succ n ih =>
          calc
            g^(n + 1) * q * (g^(n + 1))⁻¹ = g * (g^n * q * (g^n)⁻¹) * g⁻¹ := by
    ...
    obtain ⟨k, hk⟩ : ∃ k : ℤ, a = 5 * k + 1 := by
      ...
      have hM : a^3 - 1 = 5 * (25 * (a / 5)^3 + 15 * (a / 5)^2 * (a % 5) +
          3 * (a / 5) * (a % 5)^2) + ((a % 5)^3 - 1) := by
    ...
    calc
      ...
      _ = q * g := by
        rw [show q ^ (5 : ℤ) = 1 by exact_mod_cast (hq_ord ▸ pow_orderOf_eq_one q),
            one_zpow, one_mul]
...
lemma sylow_5_options (G : Type*) [Group G] [Finite G]
  ...
  have h_div12 : Fintype.card (Sylow 5 G) ∣ 12 := by
    ...
    rw [← Nat.card_eq_fintype_card, Sylow.card_eq_index_normalizer P, ← h_idx]
    exact Subgroup.index_dvd_of_le (Subgroup.le_normalizer (H := (P : Subgroup G)))
  obtain ⟨k, hk⟩ : ∃ k, Fintype.card (Sylow 5 G) = 5 * k + 1 :=
    ⟨Fintype.card (Sylow 5 G) / 5, by
      have : Fintype.card (Sylow 5 G) % 5 = 1 := by
        rw [← Nat.card_eq_fintype_card]; exact card_sylow_modEq_one 5 G
      omega⟩
...
lemma order_30_n3_ne_10_of_n5_eq_6 {G : Type*} [Group G] [Finite G]
  ...
  let S5 (P : Sylow 5 G) : Finset G := Finset.filter
    (fun x ↦ x ∈ (P : Subgroup G) ∧ orderOf x = 5) Finset.univ
  let S3 (Q : Sylow 3 G) : Finset G := Finset.filter
    (fun x ↦ x ∈ (Q : Subgroup G) ∧ orderOf x = 3) Finset.univ
  ...
  have hS5_card : ∀ P, (S5 P).card = 4 := fun P ↦ by
    have h_eq : S5 P = (Finset.filter (fun x ↦ x ∈ (P : Subgroup G)) Finset.univ).erase 1 := by
      ext x; simp only [S5, Finset.mem_filter_univ, Finset.mem_erase]
      constructor
      · rintro ⟨hxP, hx5⟩
        exact ⟨fun h ↦ absurd (hx5.symm.trans (orderOf_eq_one_iff.mpr h)) (by norm_num), hxP⟩
      · rintro ⟨hx1, hxP⟩
        have h_div : orderOf x ∣ 5 := hP_card P ▸ (Nat.card_zpowers x).symm ▸
          Subgroup.card_dvd_of_le (Subgroup.zpowers_le.mpr hxP)
        exact ⟨hxP, (Nat.dvd_prime (Nat.prime_five)).mp h_div |>.resolve_left
          (mt orderOf_eq_one_iff.mp hx1)⟩
    rw [h_eq, Finset.card_erase_of_mem
      (Finset.mem_filter.mpr ⟨Finset.mem_univ 1, Subgroup.one_mem _⟩)]
    rw [← Fintype.card_subtype (fun x ↦ x ∈ (P : Subgroup G)),
        show Fintype.card { x // x ∈ (P : Subgroup G) } = Fintype.card P from rfl]
  ...
  have hS3_card : ∀ Q, (S3 Q).card = 2 := fun Q ↦ by
    have h_eq : S3 Q = (Finset.filter (fun x ↦ x ∈ (Q : Subgroup G)) Finset.univ).erase 1 := by
      ext x; simp only [S3, Finset.mem_filter_univ, Finset.mem_erase]
      constructor
      · rintro ⟨hxQ, hx3⟩
        exact ⟨fun h ↦ absurd (hx3.symm.trans (orderOf_eq_one_iff.mpr h)) (by norm_num), hxQ⟩
      · rintro ⟨hx1, hxQ⟩
        have h_div : orderOf x ∣ 3 := hQ_card Q ▸ (Nat.card_zpowers x).symm ▸
          Subgroup.card_dvd_of_le (Subgroup.zpowers_le.mpr hxQ)
        exact ⟨hxQ, (Nat.dvd_prime (Nat.prime_three)).mp h_div |>.resolve_left
          (mt orderOf_eq_one_iff.mp hx1)⟩
    rw [h_eq, Finset.card_erase_of_mem
      (Finset.mem_filter.mpr ⟨Finset.mem_univ 1, Subgroup.one_mem _⟩)]
    rw [← Fintype.card_subtype (fun x ↦ x ∈ (Q : Subgroup G)),
        show Fintype.card { x // x ∈ (Q : Subgroup G) } = Fintype.card Q from rfl]
  ...
  have hF5_bound : 24 ≤ F5.card := by
    ...
    have hU5_card : U5.card = 24 := by
      have h_disj : ∀ P Q : Sylow 5 G, P ≠ Q → Disjoint (S5 P) (S5 Q) := fun P Q hPQ ↦ by
        ...
        simp only [S5, Finset.mem_filter_univ] at hxP hxQ
        have hzP : Subgroup.zpowers x = P.toSubgroup :=
          SetLike.ext' <| Set.eq_of_subset_of_ncard_le (Subgroup.zpowers_le.mpr hxP.1)
            ((Nat.card_zpowers x).trans (hxP.2.trans (hP_card P).symm)).ge
        have hzQ : Subgroup.zpowers x = Q.toSubgroup :=
          SetLike.ext' <| Set.eq_of_subset_of_ncard_le (Subgroup.zpowers_le.mpr hxQ.1)
            ((Nat.card_zpowers x).trans (hxQ.2.trans (hP_card Q).symm)).ge
        exact hPQ (by ext y; exact Subgroup.ext_iff.mp (hzP.symm.trans hzQ) y)
      rw [Finset.card_biUnion (fun P _ Q _ hPQ ↦ h_disj P Q hPQ),
          Finset.sum_congr rfl (fun P _ ↦ hS5_card P), Finset.sum_const, Finset.card_univ, hn5]
      rfl
  ...
  have hF3_bound : 20 ≤ F3.card := by
    ...
    have hU3_card : U3.card = 20 := by
      have h_disj : ∀ Q1 Q2 : Sylow 3 G, Q1 ≠ Q2 → Disjoint (S3 Q1) (S3 Q2) := fun Q1 Q2 hQQ ↦ by
        ...
        simp only [S3, Finset.mem_filter_univ] at hxQ1 hxQ2
        have hzQ1 : Subgroup.zpowers x = Q1.toSubgroup :=
          SetLike.ext' <| Set.eq_of_subset_of_ncard_le (Subgroup.zpowers_le.mpr hxQ1.1)
            ((Nat.card_zpowers x).trans (hxQ1.2.trans (hQ_card Q1).symm)).ge
        have hzQ2 : Subgroup.zpowers x = Q2.toSubgroup :=
          SetLike.ext' <| Set.eq_of_subset_of_ncard_le (Subgroup.zpowers_le.mpr hxQ2.1)
            ((Nat.card_zpowers x).trans (hxQ2.2.trans (hQ_card Q2).symm)).ge
        exact hQQ (by ext y; exact Subgroup.ext_iff.mp (hzQ1.symm.trans hzQ2) y)
      rw [Finset.card_biUnion (fun Q1 _ Q2 _ hQQ ↦ h_disj Q1 Q2 hQQ),
          Finset.sum_congr rfl (fun Q _ ↦ hS3_card Q), Finset.sum_const, Finset.card_univ, hn3]
      rfl
  ...
  have hF1_card : F1.card = 1 := by
    rw [show F1 = {1} from Finset.ext (fun x ↦ by
      simp only [F1, Finset.mem_filter_univ, Finset.mem_singleton, orderOf_eq_one_iff])]
    exact Finset.card_singleton 1
  have h_disj_53 : Disjoint F5 F3 := by
    rw [Finset.disjoint_left]; intro x hx5 hx3
    exact absurd ((Finset.mem_filter.mp hx5).2.symm.trans (Finset.mem_filter.mp hx3).2)
      (by norm_num)
  have h_disj_53_1 : Disjoint (F5 ∪ F3) F1 := by
    rw [Finset.disjoint_union_left]
    constructor
    · rw [Finset.disjoint_left]; intro x hx5 hx1
      exact absurd ((Finset.mem_filter.mp hx5).2.symm.trans (Finset.mem_filter.mp hx1).2)
        (by norm_num)
    · rw [Finset.disjoint_left]; intro x hx3 hx1
      exact absurd ((Finset.mem_filter.mp hx3).2.symm.trans (Finset.mem_filter.mp hx1).2)
        (by norm_num)
...
lemma card_sup_of_coprime_normal {G : Type*} [Group G]
  ...
  · rintro ⟨⟨hx, hhx⟩, ⟨kx, hkx⟩⟩ ⟨⟨hy, hhy⟩, ⟨ky, hky⟩⟩ hxy
    ...
    have h_one : hx⁻¹ * hy = 1 :=
      Subgroup.mem_bot.mp <| (Subgroup.disjoint_of_coprime_natCard hcop).eq_bot ▸
        (⟨H.mul_mem (H.inv_mem hhx) hhy,
          h_eq.symm ▸ K.mul_mem hkx (K.inv_mem hky)⟩ : hx⁻¹ * hy ∈ H ⊓ K)
    exact Prod.ext (Subtype.ext (inv_mul_eq_one.mp h_one))
      (Subtype.ext (mul_inv_eq_one.mp (h_eq.symm.trans h_one)))
  · rintro ⟨x, hx⟩
    obtain ⟨h, hh, k, hk, rfl⟩ : ∃ h ∈ H, ∃ k ∈ K, x = h * k := by
      ...
      · rename_i x_val y_val hx_mem hy_mem hx_ih hy_ih
        ...
        refine ⟨h1 * (k1 * h2 * k1⁻¹), H.mul_mem hh1 (hH.conj_mem _ hh2 k1),
          k1 * k2, K.mul_mem hk1 hk2, ?_⟩
        group
      · rename_i x_val hx_mem hx_ih
        obtain ⟨h, hh, k, hk, rfl⟩ := hx_ih
        refine ⟨k⁻¹ * h⁻¹ * k, (by rw [inv_inv] : k⁻¹ * h⁻¹ * k⁻¹⁻¹ = k⁻¹ * h⁻¹ * k) ▸
          hH.conj_mem _ (H.inv_mem hh) k⁻¹, k⁻¹, K.inv_mem hk, ?_⟩
        group
...
lemma order_30_unique_sylow_5 {G : Type*} [Group G] [Finite G]
  ...
  have hn3 : Fintype.card (Sylow 3 G) = 1 := by
    have h_cases : Fintype.card (Sylow 3 G) = 1 ∨ Fintype.card (Sylow 3 G) = 10 := by
      ...
      obtain ⟨k, hk⟩ : ∃ k, Fintype.card (Sylow 3 G) = 3 * k + 1 :=
        ⟨Fintype.card (Sylow 3 G) / 3, by
          have : Fintype.card (Sylow 3 G) % 3 = 1 := by
            rw [← Nat.card_eq_fintype_card]; exact card_sylow_modEq_one 3 G
          omega⟩
  ...
  obtain ⟨N₃, hN₃⟩ : ∃ N₃ : Subgroup G, N₃.Normal ∧ Nat.card N₃ = 3 :=
    ...
    ⟨N₃.toSubgroup,
     Subgroup.Normal.mk (fun n h_mem g ↦ hN₃ (g • N₃) ▸ Subgroup.mem_map_of_mem _ h_mem),
     N₃.card_eq_multiplicity.trans (by rw [hn, factorization_30_3, pow_one])⟩
  obtain ⟨P₅, hP₅⟩ : ∃ P₅ : Subgroup G, Nat.card P₅ = 5 := by
    exact Sylow.exists_subgroup_card_pow_prime 5
      (show 5 ^ 1 ∣ Nat.card G from hn.symm ▸ by norm_num)
  have h_card_sup : Nat.card (N₃ ⊔ P₅ : Subgroup G) = 15 :=
    (card_sup_of_coprime_normal N₃ P₅ hN₃.1 (by rw [hN₃.2, hP₅]; norm_num)).trans
      (by rw [hN₃.2, hP₅])
  haveI h_normal_sup : (N₃ ⊔ P₅ : Subgroup G).Normal :=
    Subgroup.normal_of_index_eq_two (Nat.eq_of_mul_eq_mul_right (show 0 < 15 by norm_num)
      (by rw [← h_card_sup, Subgroup.index_mul_card, hn, h_card_sup]))
  obtain ⟨P₅', hP₅'⟩ : ∃ P₅' : Sylow 5 (↥(N₃ ⊔ P₅)),
      (P₅'.toSubgroup.map (Subgroup.subtype (N₃ ⊔ P₅))).Normal :=
    sylow_normal_of_unique_in_normal (N₃ ⊔ P₅) (order_15_unique_sylow_5 h_card_sup)
  have h_is_maximal : ∀ {Q : Subgroup G}, IsPGroup 5 Q →
      (P₅'.toSubgroup.map (N₃ ⊔ P₅).subtype : Subgroup G) ≤ Q →
      Q = P₅'.toSubgroup.map (N₃ ⊔ P₅).subtype :=
    fun {Q} hQ hQ' ↦ SetLike.ext' (Eq.symm <| Set.eq_of_subset_of_ncard_le hQ' (by
      have hP5'_card : Nat.card (P₅'.toSubgroup.map (N₃ ⊔ P₅).subtype) = 5 :=
        (Subgroup.card_subtype _ _).trans
          (P₅'.card_eq_multiplicity.trans (by rw [h_card_sup, factorization_15_5]; rfl))
      have ⟨k, hk⟩ := IsPGroup.exists_card_eq hQ
      exact ((le_antisymm (by
        have : k ≤ 1 := by
          by_contra h
          exact absurd (dvd_trans (pow_dvd_pow 5 (not_le.mp h))
            (hk ▸ hn ▸ Subgroup.card_subgroup_dvd_card Q)) (by norm_num)
        interval_cases k <;> rw [hk] <;> norm_num) (by
        rw [← hP5'_card]
        exact Nat.card_mono (Set.toFinite (Q : Set G))
          (show (_ : Set G) ⊆ _ from hQ')) : Nat.card Q = 5).trans
            hP5'_card.symm).le
    ))
  ...
  exact h_contra (Fintype.card_eq_one_iff.mpr
...
lemma order_30_unique_sylow_3 {G : Type*} [Group G] [Finite G]
  ...
  obtain ⟨N₅, hN₅⟩ : ∃ N₅ : Subgroup G, N₅.Normal ∧ Nat.card N₅ = 5 :=
    ...
    ⟨N₅.toSubgroup,
     Subgroup.Normal.mk (fun n h_mem g ↦ hN₅ (g • N₅) ▸ Subgroup.mem_map_of_mem _ h_mem),
     N₅.card_eq_multiplicity.trans (by rw [hn, factorization_30_5, pow_one])⟩
  obtain ⟨P₃, hP₃⟩ : ∃ P₃ : Subgroup G, Nat.card P₃ = 3 := by
    exact Sylow.exists_subgroup_card_pow_prime 3
      (show 3 ^ 1 ∣ Nat.card G from hn.symm ▸ by norm_num)
  have h_card_sup : Nat.card (N₅ ⊔ P₃ : Subgroup G) = 15 :=
    (card_sup_of_coprime_normal N₅ P₃ hN₅.1 (by rw [hN₅.2, hP₃]; norm_num)).trans
      (by rw [hN₅.2, hP₃])
  haveI h_normal_sup : (N₅ ⊔ P₃ : Subgroup G).Normal :=
    Subgroup.normal_of_index_eq_two (Nat.eq_of_mul_eq_mul_right (show 0 < 15 by norm_num)
      (by rw [← h_card_sup, Subgroup.index_mul_card, hn, h_card_sup]))
  obtain ⟨P₃', hP₃'⟩ : ∃ P₃' : Sylow 3 (↥(N₅ ⊔ P₃)),
      (P₃'.toSubgroup.map (Subgroup.subtype (N₅ ⊔ P₃))).Normal :=
    sylow_normal_of_unique_in_normal (N₅ ⊔ P₃) (order_15_unique_sylow_3 h_card_sup)
  have h_is_maximal : ∀ {Q : Subgroup G}, IsPGroup 3 Q →
      (P₃'.toSubgroup.map (N₅ ⊔ P₃).subtype : Subgroup G) ≤ Q →
      Q = P₃'.toSubgroup.map (N₅ ⊔ P₃).subtype :=
    fun {Q} hQ hQ' ↦ SetLike.ext' (Eq.symm <| Set.eq_of_subset_of_ncard_le hQ' (by
      have hP3'_card : Nat.card (P₃'.toSubgroup.map (N₅ ⊔ P₃).subtype) = 3 :=
        (Subgroup.card_subtype _ _).trans
          (P₃'.card_eq_multiplicity.trans (by rw [h_card_sup, factorization_15_3]; rfl))
      have ⟨k, hk⟩ := IsPGroup.exists_card_eq hQ
      exact ((le_antisymm (by
        have : k ≤ 1 := by
          by_contra h
          exact absurd (dvd_trans (pow_dvd_pow 3 (not_le.mp h))
            (hk ▸ hn ▸ Subgroup.card_subgroup_dvd_card Q)) (by norm_num)
        interval_cases k <;> rw [hk] <;> norm_num) (by
        rw [← hP3'_card]
        exact Nat.card_mono (Set.toFinite (Q : Set G))
          (show (_ : Set G) ⊆ _ from hQ')) : Nat.card Q = 3).trans
            hP3'_card.symm).le
    ))
...
lemma no_normal_30_with_all_sylow3 {G : Type*} [Group G] [Finite G]
  ...
  have h_unique : ∀ P : Sylow 3 G, P.toSubgroup = P₃.toSubgroup.map N.subtype := fun P ↦ by
    have hP_card : Nat.card (P.toSubgroup.subgroupOf N) = 3 :=
      (Nat.card_congr ⟨fun (x : P.toSubgroup.subgroupOf N) ↦ (⟨x.1.1, x.2⟩ : P.toSubgroup),
        fun x ↦ (⟨⟨x.1, hP P x.2⟩, x.2⟩ : P.toSubgroup.subgroupOf N), fun _ ↦ rfl, fun _ ↦ rfl⟩ :
        Nat.card _ = Nat.card P.toSubgroup).trans
          (P.card_eq_multiplicity.trans (by rw [hn, factorization_60_3, pow_one]))
    obtain ⟨Q, hQ⟩ := IsPGroup.exists_le_sylow (p := 3) (fun x ↦ ⟨1, by
      rw [pow_one]
      exact orderOf_dvd_iff_pow_eq_one.mp (hP_card ▸
        (show orderOf x ∣ Nat.card (Subgroup.zpowers x) by rw [Nat.card_zpowers]).trans
        (Subgroup.card_subgroup_dvd_card (Subgroup.zpowers x)))⟩)
    exact (Subgroup.map_subgroupOf_eq_of_le (hP P)).symm.trans <|
      congrArg (Subgroup.map N.subtype) <|
        (SetLike.ext' <| Set.eq_of_subset_of_ncard_le hQ ((Q.card_eq_multiplicity.trans
          (by rw [hN30, factorization_30_3, pow_one]) : Nat.card Q.toSubgroup = 3).trans
            hP_card.symm).le).trans (congrArg Sylow.toSubgroup (hP₃ Q))
...
lemma no_normal_30_with_all_sylow5 {G : Type*} [Group G] [Finite G]
  ...
  have h_unique : ∀ P : Sylow 5 G, P.toSubgroup = P₅.toSubgroup.map N.subtype := fun P ↦ by
    ...
    exact (Subgroup.map_subgroupOf_eq_of_le (hP P)).symm.trans <|
      congrArg (Subgroup.map N.subtype) <|
        (SetLike.ext' <| Set.eq_of_subset_of_ncard_le hQ ((Q.card_eq_multiplicity.trans
          (by rw [hN30, factorization_30_5, pow_one]) : Nat.card Q.toSubgroup = 5).trans
            hP_card.symm).le).trans (congrArg Sylow.toSubgroup (hP₅ Q))
...
lemma normal_with_element_contains_all_sylow {G : Type*} [Group G] [Finite G]
  ...
  obtain ⟨P₀, h_le⟩ := IsPGroup.exists_le_sylow (IsPGroup.of_card
    ((Nat.card_zpowers g).trans (hg.trans (pow_one q).symm)))
...
lemma sylow_prime_pairwise_disjoint {G : Type*} [Group G] [Finite G]
  ...
  have h_dvd := hS P ▸ Subgroup.card_dvd_of_le
    (inf_le_left : (P : Subgroup G) ⊓ (Q : Subgroup G) ≤ P)
...
lemma card_ge_of_contains_all_sylow {G : Type*} [Group G] [Finite G]
  ...
  have h_disj : Set.PairwiseDisjoint (↑(Finset.univ : Finset (Sylow q G))) U :=
    fun P _ Q _ hPQ ↦ Finset.disjoint_left.mpr fun x hxP hxQ ↦
      (Finset.mem_sdiff.mp hxP).2 <| Finset.mem_singleton.mpr <| Subgroup.mem_bot.mp <|
        sylow_prime_pairwise_disjoint hS P Q hPQ ▸
          (⟨Set.mem_toFinset.mp (Finset.mem_sdiff.mp hxP).1,
            Set.mem_toFinset.mp (Finset.mem_sdiff.mp hxQ).1⟩ : x ∈ P.toSubgroup ⊓ Q.toSubgroup)
  have h_card : ∀ P : Sylow q G, (U P).card = q - 1 := fun P ↦ by
    have h_sub : ({1} : Finset G) ⊆ (P : Set G).toFinset :=
      Finset.singleton_subset_iff.mpr <| Set.mem_toFinset.mpr <| Subgroup.one_mem P.toSubgroup
  ...
  have h_eq : (insert 1 (Finset.univ.biUnion U)).card = n_sylow * (q - 1) + 1 := by
    rw [Finset.card_insert_of_notMem, Finset.card_biUnion h_disj,
        Finset.sum_congr rfl (fun P _ ↦ h_card P), Finset.sum_const, Finset.card_univ,
        hn_sylow, nsmul_eq_mul, Nat.cast_id]
    exact fun h ↦ let ⟨_, _, hP⟩ := Finset.mem_biUnion.mp h
                  (Finset.mem_sdiff.mp hP).2 (Finset.mem_singleton.mpr rfl)
...
lemma normal_divides_4_eq_bot {G : Type*} [Group G] [Finite G]
  ...
  have hGN_card : Nat.card (G ⧸ N) = 15 ∨ Nat.card (G ⧸ N) = 30 := by
    have hc : Nat.card (G ⧸ N) * Nat.card N = 60 := by
      rw [← hn, Subgroup.card_eq_card_quotient_mul_card_subgroup N]
    have h2 : Nat.card N ≤ 4 := Nat.le_of_dvd (Nat.zero_lt_succ 3) hN_div
    have h_pos : 1 ≤ Nat.card N := Nat.pos_of_ne_zero fun h0 ↦
      Nat.succ_ne_zero 3 (eq_zero_of_zero_dvd (h0 ▸ hN_div))
    interval_cases h : Nat.card N
    · exact absurd (eq_bot_iff.mpr fun x hx ↦ orderOf_eq_one_iff.mp <| Nat.dvd_one.mp <| h ▸
        ((Nat.card_zpowers x).symm ▸ Subgroup.card_dvd_of_le (Subgroup.zpowers_le.mpr hx) :
          orderOf x ∣ Nat.card N)) hN_ne_bot
    · right; exact Nat.eq_of_mul_eq_mul_right (Nat.zero_lt_succ 1)
        ((h ▸ hc : Nat.card (G ⧸ N) * 2 = 60).trans (by rfl))
    · exact absurd (h ▸ hN_div : 3 ∣ 4) (by norm_num)
    · left; exact Nat.eq_of_mul_eq_mul_right (Nat.zero_lt_succ 3)
        ((h ▸ hc : Nat.card (G ⧸ N) * 4 = 60).trans (by rfl))
  obtain ⟨Q, hQ⟩ := Fintype.card_eq_one_iff.mp
    (hGN_card.elim order_15_unique_sylow_3 order_30_unique_sylow_3)
  have hQ_card : Nat.card Q.toSubgroup = 3 :=
    Sylow.card_eq_multiplicity Q |>.trans (by
      rcases hGN_card with h | h <;> rw [h] <;>
        simp only [factorization_15_3, factorization_30_3, pow_one])
  ...
  have hM_card : Nat.card M ≤ 12 := by
    ...
    have eq_idx : Q.toSubgroup.index > 0 := Nat.pos_of_ne_zero fun h0 ↦ by
      rcases hGN_card with h | h <;> exact absurd (show Nat.card (G ⧸ N) = 0 by
        rw [← h_idx, h0, mul_zero] |>.symm ▸ h) (by norm_num)
    have h4 : Nat.card M = 3 * Nat.card N := Nat.eq_of_mul_eq_mul_right eq_idx <| by
      rw [← Q.toSubgroup.index_comap_of_surjective (QuotientGroup.mk'_surjective N),
  ...
  have hP_le : ∀ P : Sylow 3 G, (P : Subgroup G) ≤ M := fun P ↦ by
    obtain ⟨Q', hQ'⟩ := IsPGroup.exists_le_sylow
      (IsPGroup.map P.isPGroup' (QuotientGroup.mk' N))
    exact Subgroup.map_le_iff_le_comap.mp (hQ Q' ▸ hQ')
  exact absurd ((card_ge_of_contains_all_sylow M hP_card hP_le 10 hn3).trans hM_card : 21 ≤ 12)
    (by norm_num)
...
lemma sylow3_in_normal_forces_large {G : Type*} [Group G] [Finite G]
  ...
  obtain ⟨k, hk⟩ := hN_card
  have : k ≤ 2 := by
    have := (Nat.mul_le_mul_right k
      (card_ge_of_contains_all_sylow N hS3 hN 10 hn3)).trans (le_of_eq hk.symm)
    omega
...
lemma sylow5_in_normal_forces_large {G : Type*} [Group G] [Finite G]
  ...
  have : k ≤ 2 := by
    have := (Nat.mul_le_mul_right k
      (card_ge_of_contains_all_sylow N hS5 hN 6 hn5)).trans (le_of_eq hk.symm)
    omega
  interval_cases k <;> omega
...
lemma is_simple_60_aux {G : Type*} [Group G] [Finite G] [Nontrivial G]
  ...
  · left
    ...
    · obtain ⟨g, hgN, hg⟩ := cauchy_element_in_subgroup N hN_div_3
      ...
      exact no_normal_30_with_all_sylow3 hn N
        (sylow3_in_normal_forces_large hn3 hS3 N hN_all3 h_dvd hN_top) hn3 hN_all3
    · by_cases hN_div_5 : 5 ∣ Nat.card N
      · obtain ⟨g, hgN, hg⟩ := cauchy_element_in_subgroup N hN_div_5
        ...
        exact no_normal_30_with_all_sylow5 hn N
          (sylow5_in_normal_forces_large hn5 hS5 N hN_all5 h_dvd hN_top) hn5 hN_all5
...
theorem is_simple_60 (G : Subgroup (PGL p))
  ...
  { eq_bot_or_eq_top_of_normal := fun N hN ↦
      is_simple_60_aux hn (hp3 ▸ hn3)
        ((sylow_5_options G hn).resolve_left (sylow_5_not_one p G hp3 hn hn3)) N }
...
lemma simple_60_n2_ne_15 (G : Type*) [Group G] [Finite G]
  ...
  have h_cent : Subgroup.normalizer (P : Set G) ≤ Subgroup.centralizer (P : Set G) := by
    have h_norm_idx : (Subgroup.normalizer (P : Set G)).index = 15 := by
      rw [← Sylow.card_eq_index_normalizer P, Nat.card_eq_fintype_card, hn2]
    have h_norm_card : Nat.card (Subgroup.normalizer (P : Set G)) = 4 :=
      Nat.eq_of_mul_eq_mul_right (show 0 < 15 by norm_num) (
        (congrArg (Nat.card (Subgroup.normalizer (P : Set G)) * ·) h_norm_idx.symm).trans
          ((Subgroup.card_mul_index _).trans (hn.trans (rfl : 60 = 4 * 15))))
    have h_norm_eq : (P : Subgroup G) = Subgroup.normalizer (P : Set G) :=
      SetLike.ext' (Set.eq_of_subset_of_ncard_le Subgroup.le_normalizer
        ((Nat.card_coe_set_eq ((Subgroup.normalizer (P : Set G)) : Set G)).symm.trans
...
lemma simple_60_n2_eq_5 (G : Type*) [Group G] [Finite G]
  ...
  have h_cases : Fintype.card (Sylow 2 G) = 1 ∨ Fintype.card (Sylow 2 G) = 3 ∨
    ...
    obtain ⟨k, hk⟩ : ∃ k, Fintype.card (Sylow 2 G) = 2 * k + 1 :=
      ⟨Fintype.card (Sylow 2 G) / 2, by
        have : Fintype.card (Sylow 2 G) % 2 = 1 := by
          rw [← Nat.card_eq_fintype_card]; exact card_sylow_modEq_one 2 G
        omega⟩
...
theorem simple_60_is_A5 (G : Type*) [Group G] [Finite G]
  ...
  have h_iso_range : G ≃* f.range :=
    MulEquiv.ofBijective f.rangeRestrict
      ⟨fun x y h ↦ (simple_group_faithful_action_on_sylow hs
        (by rw [simple_60_n2_eq_5 G hn hs]; norm_num)) (congrArg Subtype.val h),
  ...
  have h_index : f.range.index = 2 := by
    ...
    exact Nat.eq_of_mul_eq_mul_right (show 0 < 60 by norm_num) (
      (congrArg (f.range.index * ·)
        (hn.symm.trans (Nat.card_congr h_iso_range.toEquiv))).trans
        ((Subgroup.index_mul_card _).trans
          (h_perm_card.trans (rfl : 120 = 2 * 60))))
  exact ⟨(h_iso_range.trans (MulEquiv.subgroupCongr
    (Equiv.Perm.eq_alternatingGroup_of_index_eq_two h_index))).trans
