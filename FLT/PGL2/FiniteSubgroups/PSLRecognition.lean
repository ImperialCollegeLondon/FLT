...
This file proves that a finite subgroup of `PGL(2, K)` whose order
equals `|PSL(2, p^m)|` is conjugate to a subgroup of the image of
`PSL(2, F_q)` inside `PGL(2, K)`. The argument recovers the subfield
`F_q` from the translation parameters of a Sylow `p`-subgroup:
their dilation parameters exhaust the squares in `F_q`, and a
sum-of-two-squares argument then shows that the translation set,
scaled by any nonzero element, coincides with `F_q` itself. It
follows that the orbit of infinity is the projective line
`P^1(F_q)`, which forces the conjugated subgroup into the range
of the canonical map `PGL(2, F_q) → PGL(2, K)`.

## Main results

- `sum_of_two_squares_Fq`: every element in a finite field is a sum of two squares.
- `additive_subgroup_eq_F_q_from_squares`: an additive subgroup stable under squaring is
                                           the whole field.
- `sylow_order_of_psl_order`: deduces the size of a Sylow `p`-subgroup.
- `n_p_eq_psl`: the number of Sylow `p`-subgroups matches the expected number `q + 1`.
- `translationSet_scaled_eq_Fq_psl`: the translation parameter set equals the subfield `F_q`.
- `orbit_infty_eq_P1Fq_psl`: the orbit of infinity under the subgroup is the
...
/-- Every element in a finite field `F_q` (where `q = p^m ≥ 5`) can be expressed as
the sum of two squares in `F_q`. -/
theorem sum_of_two_squares_Fq (m : ℕ) (hm : m ≥ 1) (hpm : p ^ m ≥ 5)
  ...
  obtain ⟨a', ha'⟩ : ∃ a' : GaloisField p m, galoisFieldRingHom p m a' = a := by
    ...
    obtain ⟨s, hs⟩ := Polynomial.splits_iff_exists_multiset.mp
      (instIsSplittingFieldZModGaloisFieldHSubPolynomialHPowNatX p m).1
  ...
  obtain ⟨b', c', hbc'⟩ : ∃ b' c' : GaloisField p m, b'^2 + c'^2 = a' := by
    have hp2 : (Polynomial.X ^ 2 : Polynomial (GaloisField p m)).degree = 2 :=
      Polynomial.degree_X_pow 2
    ...
    obtain ⟨b', c', hbc'⟩ := FiniteField.exists_root_sum_quadratic hp2 hq2
      (by apply Nat.odd_iff.mp; rw [h_card]
          exact Odd.pow ((Fact.out : Nat.Prime p).odd_of_ne_two (ne_of_gt (Fact.out : p > 2))))
    simp only [Polynomial.eval_sub, Polynomial.eval_pow, Polynomial.eval_X,
      Polynomial.eval_C] at hbc'
...
theorem sq_is_half_root_or_zero (m : ℕ) (b : K p) (hb : b ∈ F_q_in_K p m) :
  ...
  · refine Or.inr ⟨pow_ne_zero 2 hb0, ?_⟩
    have h_even : 2 ∣ p ^ m - 1 := by
      ...
      have h_odd : ¬ Even p := fun h ↦
        ne_of_gt (Fact.out : p > 2) ((Nat.Prime.even_iff Fact.out).mp h)
...
/-- An additive subgroup of `F_q` containing 1 and stable under multiplication by
all squares is the entire field `F_q`. -/
...
/-- If a finite subgroup `G` of `PGL(2, K)` has order equal to `|PSL(2, p^m)|`,
then its Sylow `p`-subgroup has order `p^m`. -/
theorem sylow_order_of_psl_order (G : Subgroup (PGL p)) [Finite G]
  ...
  have h_even : 2 ∣ p ^ (2 * m) - 1 := by
    rw [← even_iff_two_dvd, Nat.even_sub (Nat.one_le_pow _ _ (Fact.out : Nat.Prime p).pos)]
    exact iff_of_false (fun h ↦ ne_of_gt (Fact.out : p > 2)
      ((Nat.Prime.even_iff Fact.out).mp (Nat.even_pow.mp h).1)) (by norm_num)
  have h_two : (2 : ℕ).factorization p = 0 := Nat.factorization_eq_zero_of_not_dvd fun h ↦
    not_le_of_gt Fact.out (Nat.le_of_dvd (by norm_num) h)
...
theorem n_p_gt_one_of_psl_order (G : Subgroup (PGL p)) [Finite G]
  ...
  obtain ⟨P, hP⟩ : ∃ P : Sylow p G, ∀ Q : Sylow p G, Q = P :=
    Fintype.card_eq_one_iff.mp (by
      have := Fintype.card_pos_iff.mpr ⟨Classical.arbitrary (Sylow p G)⟩; omega)
  haveI h_normal : (P : Subgroup G).Normal :=
    ⟨fun n hn g ↦ hP (g • P) ▸ Subgroup.mem_map_of_mem _ hn⟩
  have h_even : 2 ∣ p ^ (2 * m) - 1 := by
    rw [← even_iff_two_dvd, Nat.even_sub (Nat.one_le_pow _ _ (Fact.out : Nat.Prime p).pos)]
    exact iff_of_false (fun h ↦ ne_of_gt (Fact.out : p > 2)
      ((Nat.Prime.even_iff Fact.out).mp (Nat.even_pow.mp h).1)) (by norm_num)
  have h_hole : p ∣ Nat.card G := dvd_trans (dvd_pow_self p (by omega : m ≠ 0))
    (by rw [hn, Nat.mul_div_assoc _ h_even]; exact dvd_mul_right (p ^ m) _)
  have hP_divides : (p ^ (2 * m) - 1) / 2 ∣ p ^ m - 1 := by
    rw [← sylow_order_of_psl_order p G m hm hn P]
    rw [show (p ^ (2 * m) - 1) / 2 = (Nat.card G) / (Nat.card P) from by
      rw [hn, sylow_order_of_psl_order p G m hm hn P, Nat.mul_div_assoc _ h_even,
        Nat.mul_div_cancel_left _ (pow_pos (Fact.out : Nat.Prime p).pos _)]]
    convert normalizer_complement_divides_main p G P h_hole using 1
    erw [Subgroup.normalizer_eq_top_iff.mpr h_normal]
...
/-- The number of Sylow `p`-subgroups of `G` is exactly `p^m + 1`. -/
theorem n_p_eq_psl (G : Subgroup (PGL p)) [Finite G]
  ...
  have hG_p : p ∣ Nat.card G := by
    rw [hn]
    exact Nat.dvd_div_of_mul_dvd
      (mul_comm p 2 ▸ Nat.mul_dvd_mul (dvd_pow_self p (ne_of_gt hm)) h_even)
  ...
  have hk_one : k * p ^ m + 1 ∣ (p ^ m - 1) * (p ^ m + 1) := by
    have h_coprime : Nat.Coprime (k * p ^ m + 1) (p ^ m) := by
      change Nat.gcd (k * p ^ m + 1) (p ^ m) = 1; norm_num
    ...
    have h_divG : Nat.card G ∣ p ^ m * (p ^ (2 * m) - 1) :=
      hn ▸ Nat.div_dvd_of_dvd (dvd_mul_of_dvd_right h_even _)
    refine h_sq ▸ h_coprime.dvd_of_dvd_mul_left (dvd_trans ?_ h_divG)
    rw [← hk, ← Nat.card_eq_fintype_card,
      Nat.card_congr (Sylow.equivQuotientNormalizer (Classical.arbitrary (Sylow p G)))]
    exact Subgroup.card_quotient_dvd_card _
  have hp_gt_one : 1 < p ^ m :=
    one_lt_pow₀ (Nat.lt_trans (by norm_num : 1 < 2) hp_gt_two) (ne_of_gt hm)
  rcases k with _ | _ | k
...
theorem z1_eq_psl (G : Subgroup (PGL p)) [Finite G]
  ...
  have h_orbit_stabilizer : Nat.card (Subgroup.normalizer (P : Set G)) *
      Fintype.card (Sylow p G) = Nat.card G := by
    rw [mul_comm, ← Nat.card_eq_fintype_card, Sylow.card_eq_index_normalizer P,
      Subgroup.index_mul_card]
  have h_card_normalizer : Nat.card (Subgroup.normalizer (P : Set G)) =
      (p ^ m * (p ^ (2 * m) - 1)) / (2 * (p ^ m + 1)) := by
    have h1 : Nat.card (Subgroup.normalizer (P : Set G)) * (p ^ m + 1) =
        p ^ m * (p ^ (2 * m) - 1) / 2 := by
  ...
  change Nat.card (Subgroup.normalizer (P : Set G)) / Nat.card P = (p ^ m - 1) / 2
...
theorem normDilationParam_image_card_eq_normalizerQuotient
  ...
  let f : ↥(Subgroup.normalizer (P : Set G)) →* (K p)ˣ :=
    MonoidHom.mk' (fun g ↦ Units.mk0 (normDilationParam p G hG_p P hP_fix g)
      (dilationParam_ne_zero p _
        (normalizer_element_fixes_infinity p G hG_p P hP_fix g g.property)))
      (fun a b ↦ Units.ext (normDilationParam_mul p G hG_p P hP_fix a b))
  have h_ker : f.ker = (P : Subgroup G).comap
      (Subgroup.subtype (Subgroup.normalizer (P : Set G))) := by
    ext g
    ...
    have h_coset := same_normDilationParam_imp_coset p G hG_p P hP_fix g 1 <|
      (congr_arg Units.val hg).trans (dilationParam_one p
        (normalizer_element_fixes_infinity p G hG_p P hP_fix 1 (Subgroup.one_mem _))).symm
  ...
  change Nat.card (Set.range f) = Nat.card (Subgroup.normalizer (P : Set G)) / Nat.card (P : Set G)
  rw [← Subgroup.index_mul_card f.ker, Subgroup.index_ker f]
  have h_card_ker : Nat.card f.ker = Nat.card (P : Subgroup G) := by
    rw [h_ker]
    exact Nat.card_congr ⟨fun x ↦ ⟨x.val.val, x.property⟩, fun x ↦
      ⟨⟨x.val, Subgroup.le_normalizer x.property⟩, x.property⟩, fun _ ↦ rfl, fun _ ↦ rfl⟩
  rw [h_card_ker]
...
theorem normalizer_dilation_params_cover_nq_roots
  ...
  have hc_mem : c ∈ hS_finite.toFinset := by
    refine finite_subgroup_eq_roots_of_unity _ _ _ ?_ ?_ ?_ ?_ c hc hc_ne_zero
    · exact Nat.div_pos (Nat.le_of_dvd Nat.card_pos
        (Subgroup.card_dvd_of_le Subgroup.le_normalizer)) Nat.card_pos
    ...
    · simp only [Set.Finite.mem_toFinset]
      rintro _ ⟨g, rfl⟩
      exact dilationParam_ne_zero p g
        (normalizer_element_fixes_infinity p G hG_p P hP_fix g g.property)
...
/-- Up to scaling, the translation parameter set is exactly the subfield `F_q`. -/
...
theorem orbit_eq_infty_union_translations_psl
  ...
  have h_card_eq : (orbitInfty p G).ncard = ({infinity p} ∪
      P1point p '' ((fun b ↦ α₀ + b) '' translationSet p (Subgroup.map G.subtype P))).ncard := by
    rw [orbitInfty_ncard p G hG_p P hP_fix,
      n_p_eq_psl p G m hm hn (n_p_gt_one_of_psl_order p G m hm hpm hn), add_comm]
    have hd : Disjoint {infinity p}
        (P1point p '' ((fun b ↦ α₀ + b) '' translationSet p (Subgroup.map G.subtype P))) := by
    ...
    haveI : Finite ↥(P1point p ''
        ((fun b ↦ α₀ + b) '' translationSet p (Subgroup.map G.subtype P))) :=
      (Set.Finite.image _ (Set.Finite.image _ (Set.finite_of_ncard_pos
        (by rw [translationSet_card_eq_P p G P hP_fix]; exact Nat.card_pos)))).to_subtype
    rw [Set.ncard_union_eq hd]
    · rw [Set.ncard_image_of_injective]
      · rw [Set.ncard_image_of_injective]
        · rw [Set.ncard_singleton, translationSet_card_eq_P p G P hP_fix,
            sylow_order_of_psl_order p G m hm hn P]
        · intro x y hxy
  ...
  have h_orbit_finite : (orbitInfty p G).Finite :=
    Set.finite_of_ncard_pos (by rw [orbitInfty_ncard p G hG_p P hP_fix,
      n_p_eq_psl p G m hm hn (n_p_gt_one_of_psl_order p G m hm hpm hn)]; positivity)
  exact (Set.eq_of_subset_of_ncard_le (orbit_superset_of_translations p G
    (translationSet p (Subgroup.map (Subgroup.subtype G) (P : Subgroup G)))
    (fun _ ⟨g, _, hg'⟩ ↦ hg'.symm ▸ Subtype.mem g) α₀ hα₀) h_card_eq.le h_orbit_finite).symm
...
theorem orbit_infty_eq_P1Fq_psl_core
  ...
  have h_card : Set.ncard (orbitInfty p G) = Fintype.card (Sylow p G) :=
    orbitInfty_ncard p G hG_p P hP_fix
  obtain ⟨α₀, hα₀⟩ : ∃ α₀ : K p, P1point p α₀ ∈ orbitInfty p G := by
    obtain ⟨x, hx, hx_ne⟩ := Set.exists_ne_of_one_lt_ncard
      (h_card.symm ▸ n_p_gt_one_of_psl_order p G m hm hpm hn) (infinity p)
  ...
  have hV_card : Set.ncard V = p ^ m := by
    rw [hV_def, translationSet_card_eq_P p G P hP_fix, sylow_order_of_psl_order p G m hm hn P]
  obtain ⟨v₀, hv₀_mem, hv₀_ne⟩ : ∃ v₀ ∈ V, v₀ ≠ 0 := by
    by_contra! h
    have hV_eq : V = {0} := Set.eq_singleton_iff_unique_mem.mpr
      ⟨hV_def.symm ▸ translationSet_zero p H, h⟩
  ...
  have h_trans_orbit : orbitInfty p (G.map (MulEquiv.toMonoidHom (MulAut.conj g_trans))) =
      {infinity p} ∪ P1point p '' V :=
    orbit_translate_conj p G α₀ V h_orbit_V
  have h_map_eq : G.map (MulEquiv.toMonoidHom (MulAut.conj g₀)) =
      (G.map (MulEquiv.toMonoidHom (MulAut.conj g_trans))).map
        (MulEquiv.toMonoidHom (MulAut.conj g_dil)) := by
    rw [Subgroup.map_map]; congr 1; ext x
    simp only [g₀, MonoidHom.comp_apply, MulEquiv.coe_toMonoidHom, MulAut.conj_apply]; group
  rw [P1_Fq, h_map_eq, ← h_Fq]
  exact orbit_dilation_conj p _ v₀⁻¹ (inv_ne_zero hv₀_ne) V h_trans_orbit

/-- The orbit of infinity under `G` is precisely the standard projective line `P^1(F_q)`. -/
theorem orbit_infty_eq_P1Fq_psl
  ...
  have h_even : 2 ∣ p ^ (2 * m) - 1 := by
    rw [← even_iff_two_dvd,
      Nat.even_sub (Nat.one_le_pow _ _ (Nat.le_trans (by norm_num : 1 ≤ 2) (Fact.out : p > 2).le))]
    have h_odd : ¬ Even p := fun h ↦
      ne_of_gt (Fact.out : p > 2) ((Nat.Prime.even_iff Fact.out).mp h)
    exact iff_of_false (fun h ↦ h_odd (Nat.even_pow.mp h).1) (by norm_num)
  have hG_p : p ∣ Nat.card G := by
...
/-- A subgroup of `PGL(2, K)` with the correct order and Sylow structure
acts faithfully on `P^1(F_q)` and can be identified with `PSL(2, q)`. -/
