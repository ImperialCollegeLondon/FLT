...
lemma isSmooth_directSum {I : Type*} {V : I → Type*} [(i : I) → AddCommMonoid (V i)]
    ...
    IsSmooth (Representation.directSum ρ) := by classical
  ...
  have hset : {g : G | ∀ i : I, ((ρ i) g) (v i) = v i}
    ...
    · intro h_stab i
      ...
      · rw [DFinsupp.notMem_support_iff] at h_supp; rw [h_supp, map_zero]
  rw [hset]; exact isOpen_biInter_finset fun i _ => (h i).smooth (v i)
...
lemma isSmoothVector_tmul {ρ : Representation k G V} {ρ' : Representation k G V'} {v : V} {v' : V'}
  ...
  have h_sub : (stabilizer ρ v) ⊓ (stabilizer ρ' v') ≤ (stabilizer (ρ.tprod ρ') (v ⊗ₜ[k] v')) := by
