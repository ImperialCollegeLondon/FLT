...
scoped instance {G : Type*} [Group G] [Finite G] : Fintype G := Fintype.ofFinite _
...
instance : SMul (PGL p) (ProjectiveLine p) where
  smul := Quotient.lift (fun g x ↦ g • x) (by
    ...
    | h v hv =>
      change g₁ • Projectivization.mk (K p) v hv = g₂ • Projectivization.mk (K p) v hv
      ...
      have h_val : (g₁⁻¹ * g₂).val = algebraMap (K p) _ (↑u : K p) := congr_arg Units.val hu.symm
      rw [h_val, Algebra.algebraMap_eq_smul_one, Matrix.smul_mulVec]
      exact congrArg (fun x => ↑u • x) (Matrix.one_mulVec v).symm)

instance : MulAction (PGL p) (ProjectiveLine p) where
  one_smul x := Projectivization.instMulAction.one_smul x
  mul_smul g₁ g₂ x := by
...
def pglMap {F L : Type*} [Field F] [Field L] (f : F →+* L) : PGLOf F →* PGLOf L :=
  QuotientGroup.map (Subgroup.center (GL (Fin 2) F)) (Subgroup.center (GL (Fin 2) L))
    (Matrix.GeneralLinearGroup.map f) (by
      ...
      let E1 := Matrix.GeneralLinearGroup.mkOfDetNeZero
        (!![1, 1; 0, 1] : Matrix (Fin 2) (Fin 2) F) (by norm_num [Matrix.det_fin_two])
      ...
      have h10 : x.val 1 0 = 0 := add_right_cancel (by
        ...
        exact h_eq)
      let E2 := Matrix.GeneralLinearGroup.mkOfDetNeZero
        (!![1, 0; 1, 1] : Matrix (Fin 2) (Fin 2) F) (by norm_num [Matrix.det_fin_two])
      ...
      intro g
      apply Units.ext
      change g.val * (Matrix.GeneralLinearGroup.map f x).val =
        (Matrix.GeneralLinearGroup.map f x).val * g.val
      ext i j
      rw [Matrix.mul_apply, Fin.sum_univ_two, Matrix.mul_apply, Fin.sum_univ_two]
      match i, j with
      | 0, 0 =>
        change g.val 0 0 * f (x.val 0 0) + g.val 0 1 * f (x.val 1 0) =
          f (x.val 0 0) * g.val 0 0 + f (x.val 0 1) * g.val 1 0
        rw [h10, h01, map_zero, mul_zero, zero_mul, add_zero, add_zero, mul_comm]
      | 0, 1 =>
        change g.val 0 0 * f (x.val 0 1) + g.val 0 1 * f (x.val 1 1) =
          f (x.val 0 0) * g.val 0 1 + f (x.val 0 1) * g.val 1 1
        rw [h01, map_zero, mul_zero, zero_mul, zero_add, add_zero, h00, mul_comm]
      | 1, 0 =>
        change g.val 1 0 * f (x.val 0 0) + g.val 1 1 * f (x.val 1 0) =
          f (x.val 1 0) * g.val 0 0 + f (x.val 1 1) * g.val 1 0
        rw [h10, map_zero, mul_zero, zero_mul, add_zero, zero_add, h00, mul_comm]
      | 1, 1 =>
        change g.val 1 0 * f (x.val 0 1) + g.val 1 1 * f (x.val 1 1) =
          f (x.val 1 0) * g.val 0 1 + f (x.val 1 1) * g.val 1 1
...
theorem finite_of_closure_finite (S : Set (K p)) (hS : S.Finite) : Finite (Subfield.closure S) := by
  ...
  haveI : Finite (IntermediateField.adjoin (ZMod p) (s : Set (K p))) :=
    Module.finite_iff_finite.mp (IntermediateField.finiteDimensional_adjoin
      (fun x _ ↦ isAlgebraic_iff_isIntegral.mp <|
        (AlgebraicClosure.isAlgebraic (ZMod p)).isAlgebraic x))
  let f : Subfield.closure (s : Set (K p)) → IntermediateField.adjoin (ZMod p) (s : Set (K p)) :=
    fun x ↦ ⟨x.val, Subfield.closure_le.mpr (fun _ hx ↦
      IntermediateField.subset_adjoin (ZMod p) (s : Set (K p)) hx) x.property⟩
...
theorem exists_finite_subfield_conjugate (G : Subgroup (PGL p)) [Finite G] :
  ...
  let L : Subfield (K p) := Subfield.closure
    (⋃ g ∈ S, Set.range (fun ij : Fin 2 × Fin 2 ↦ g.val ij.1 ij.2))
  ...
  have h_range : f x ∈ (Matrix.GeneralLinearGroup.map (Subfield.subtype L)).range := by
    ...
    let y_val : Matrix (Fin 2) (Fin 2) L :=
      Matrix.of fun i j ↦ ⟨(f x).val i j, hL_mem (f x) hf_in i j⟩
  ...
  · rintro ⟨c, hc⟩
    ...
    have hc_ne : c ≠ 0 := fun h ↦ x.rep_nonzero <| by
      calc x.rep = (g⁻¹.val * g.val).mulVec x.rep := by
            ...
            exact (Matrix.one_mulVec x.rep).symm
        _ = g⁻¹.val.mulVec (g.val.mulVec x.rep) := by rw [Matrix.mulVec_mulVec]; rfl
        _ = 0 := by rw [hc, h, zero_smul, Matrix.mulVec_zero]
    exact ⟨Units.mk0 c hc_ne, hc.symm⟩

open Classical in
theorem card_eigenvalues (g : GL (Fin 2) (K p)) :
    Set.ncard {x : K p | (g.val - x • 1).det = 0} =
      if (g.val.trace) ^ 2 = 4 * g.val.det then 1 else 2 := by
  let T := g.val.trace
  let D := g.val.det
  have h_det : ∀ x : K p, (g.val - x • 1).det = x^2 - T * x + D := fun x ↦ by
    rw [Matrix.det_fin_two]
    change (g.val - x • 1) 0 0 * (g.val - x • 1) 1 1 -
      (g.val - x • 1) 0 1 * (g.val - x • 1) 1 0 = x^2 - g.val.trace * x + g.val.det
    rw [Matrix.trace_fin_two, Matrix.det_fin_two]
    change (g.val 0 0 - x * 1) * (g.val 1 1 - x * 1) -
      (g.val 0 1 - x * 0) * (g.val 1 0 - x * 0) = x^2 -
        (g.val 0 0 + g.val 1 1) * x + (g.val 0 0 * g.val 1 1 - g.val 0 1 * g.val 1 0)
    ring
  rw [show {x : K p | (g.val - x • 1).det = 0} = {x | x^2 - T * x + D = 0} from
    Set.ext (fun z ↦ by change (g.val - z • 1).det = 0 ↔ z^2 - T * z + D = 0; rw [h_det z])]
  obtain ⟨x, hx⟩ : ∃ x : K p, x^2 - T * x + D = 0 := by
    ...
    exact ⟨root, by
      have h_eval : f.eval root = root^2 - T * root + D := by
        change (Polynomial.X^2 - Polynomial.C T * Polynomial.X + Polynomial.C D).eval root =
          root^2 - T * root + D
...
theorem eigenvector_isMultiple_of_basis (g : GL (Fin 2) (K p)) (x y : K p) (v w u : Fin 2 → K p)
  ...
  obtain ⟨c, hc_eq⟩ := (Submodule.mem_span_range_iff_exists_fun (K p)).mp <|
    (Submodule.eq_top_of_finrank_eq (by
      rw [finrank_span_eq_card h_lin, Module.finrank_pi])).symm ▸ Submodule.mem_top
  let a := c 0; let b := c 1
  ...
  have h_zero : (a * (x - z)) • v + (b * (y - z)) • w = 0 := by
    ...
    calc a * (x - z) * v i + b * (y - z) * w i
      ...
      _ = (a • (g.val.mulVec v) + b • (g.val.mulVec w)) i - z * (a • v + b • w) i := by
        rw [← hvx, ← hwy]
      _ = (g.val.mulVec (a • v) + g.val.mulVec (b • w)) i - z * (a • v + b • w) i := by
  ...
  have h_c_zero := Fintype.linearIndependent_iff.mp h_lin ![a * (x - z), b * (y - z)] (by
    rw [Fin.sum_univ_two]; exact h_zero)
...
theorem fixedPoints_eq_pair (g : GL (Fin 2) (K p)) (x y : K p) (v w : Fin 2 → K p)
  ...
  constructor
  · intro hP
    rcases eigenvector_isMultiple_of_basis p g x y v w P.rep h_ne hv hw hvx hwy
      ((fixedPoints_lift p g P).mp hP) with ⟨c, hc⟩ | ⟨c, hc⟩
    · left
...
theorem ncard_fixedPoints_eq_two_of_ne_eigenvalues (g : GL (Fin 2) (K p)) (x y : K p)
  ...
  rw [Matrix.sub_mulVec, sub_eq_zero, Matrix.smul_mulVec] at hv hw
  have hv := hv.trans (congrArg (fun z => x • z) (Matrix.one_mulVec v))
  have hw := hw.trans (congrArg (fun z => y • z) (Matrix.one_mulVec w))
...
theorem ncard_fixedPoints_eq_two_of_discrim_ne_zero (g : GL (Fin 2) (K p))
  ...
  have hf_ne_zero : f.degree ≠ 0 := fun h ↦ by
    have h1 : f.coeff 2 = 1 := by
      change (Polynomial.X^2 - Polynomial.C g.val.trace * Polynomial.X +
        Polynomial.C g.val.det).coeff 2 = 1
      simp only [Polynomial.coeff_add, Polynomial.coeff_sub, Polynomial.coeff_X_pow,
        Polynomial.coeff_C_mul, Polynomial.coeff_X, Polynomial.coeff_C]
  ...
  have hx : x^2 - g.val.trace * x + g.val.det = 0 := by
    have h_eval : f.eval x = x^2 - g.val.trace * x + g.val.det := by
      change (Polynomial.X^2 - Polynomial.C g.val.trace * Polynomial.X +
          Polynomial.C g.val.det).eval x = x^2 - g.val.trace * x + g.val.det
      simp only [Polynomial.eval_add, Polynomial.eval_sub, Polynomial.eval_pow,
        Polynomial.eval_mul, Polynomial.eval_X, Polynomial.eval_C]
    rw [← h_eval]
    exact hx_root
  ...
  have expand (t : K p) : (g.val - t • 1).det = t^2 - g.val.trace * t + g.val.det := by
    ...
    change (g.val 0 0 - t * 1) * (g.val 1 1 - t * 1) - (g.val 0 1 - t * 0) * (g.val 1 0 - t * 0) =
      t^2 - (g.val 0 0 + g.val 1 1) * t + (g.val 0 0 * g.val 1 1 - g.val 0 1 * g.val 1 0)
    ring
  exact ncard_fixedPoints_eq_two_of_ne_eigenvalues p g x y h_ne
    (by rw [expand, hx]) (by rw [expand, hy])
...
theorem ncard_fixedPoints_eq_one_of_discrim_zero (g : GL (Fin 2) (K p)) (hg : ¬ ∃ c : K p,
  ...
  have hv_eq : g.val.mulVec v = lambda • v := by
    ...
    exact hv.trans (congrArg (fun z => lambda • z) (Matrix.one_mulVec v))
  ...
  have h_ker_lt : Module.finrank (K p) ker < 2 := by
    ...
    have h_top : ker = ⊤ := by
      ...
      have h_dim : Module.finrank (K p) (Fin 2 → K p) = 2 := by
    ...
    have hA_zero : A = 0 := by
      ...
      match i, j with
      | 0, 0 =>
        change A 0 0 * (1 : K p) + A 0 1 * (0 : K p) = 0 at h_eval
        rw [mul_zero, add_zero, mul_one] at h_eval; exact h_eval
      | 0, 1 =>
        change A 0 0 * (0 : K p) + A 0 1 * (1 : K p) = 0 at h_eval
        rw [mul_zero, zero_add, mul_one] at h_eval; exact h_eval
      | 1, 0 =>
        change A 1 0 * (1 : K p) + A 1 1 * (0 : K p) = 0 at h_eval
        rw [mul_zero, add_zero, mul_one] at h_eval; exact h_eval
      | 1, 1 =>
        change A 1 0 * (0 : K p) + A 1 1 * (1 : K p) = 0 at h_eval
        rw [mul_zero, zero_add, mul_one] at h_eval; exact h_eval
  ...
  have h_kernel : ∀ w : Fin 2 → K p, g.val.mulVec w = lambda • w → ∃ c : K p, w = c • v := by
    ...
    have hw_ker : w ∈ ker := by
      ...
      exact (congrArg (fun z => g.val.mulVec w - lambda • z) (Matrix.one_mulVec w)).trans <| by
        rw [hw, sub_self]
  ...
  · intro hP
    ...
    have h_det_c : (g.val - c • 1).det = 0 := by
      ...
      exact ⟨P.rep, P.rep_nonzero, by
        rw [Matrix.sub_mulVec, sub_eq_zero, Matrix.smul_mulVec]
        exact hc.trans (congrArg (fun z => c • z) (Matrix.one_mulVec P.rep).symm)⟩
...
theorem orderOf_coprime_of_discrim_ne_zero (g : GL (Fin 2) (K p))
  ...
  have hlambda1 : lambda1^2 - g.val.trace * lambda1 + g.val.det = 0 := by
    have h_eval : f.eval lambda1 = lambda1^2 - g.val.trace * lambda1 + g.val.det := by
      change (Polynomial.X^2 - Polynomial.C g.val.trace * Polynomial.X +
        Polynomial.C g.val.det).eval lambda1 = lambda1^2 - g.val.trace * lambda1 + g.val.det
      simp only [Polynomial.eval_add, Polynomial.eval_sub, Polynomial.eval_pow,
        Polynomial.eval_mul, Polynomial.eval_X, Polynomial.eval_C]
    rw [← h_eval]
    exact h_root
  ...
  have expand (t : K p) : (g.val - t • 1).det = t^2 - g.val.trace * t + g.val.det := by
    rw [Matrix.det_fin_two, Matrix.trace_fin_two, Matrix.det_fin_two]
    change (g.val 0 0 - t * 1) * (g.val 1 1 - t * 1) -
      (g.val 0 1 - t * 0) * (g.val 1 0 - t * 0) = t^2 -
      (g.val 0 0 + g.val 1 1) * t + (g.val 0 0 * g.val 1 1 - g.val 0 1 * g.val 1 0)
    ring
  ...
  have hv1_eq : g.val.mulVec v1 = lambda1 • v1 := by
    apply sub_eq_zero.mp
    calc g.val.mulVec v1 - lambda1 • v1 =
        g.val.mulVec v1 - lambda1 • (1 : Matrix (Fin 2) (Fin 2) (K p)).mulVec v1 :=
          congrArg (fun z => g.val.mulVec v1 - lambda1 • z) (Matrix.one_mulVec v1).symm
      _ = g.val.mulVec v1 - (lambda1 • 1 : Matrix (Fin 2) (Fin 2) (K p)).mulVec v1 := by
  ...
  obtain ⟨v2, hv2_ne, hv2⟩ := Matrix.exists_mulVec_eq_zero_iff.mpr <|
    show (g.val - lambda2 • 1).det = 0 by
      have hlambda2_eq_zero : lambda2^2 - g.val.trace * lambda2 + g.val.det = 0 := by
        calc lambda2^2 - g.val.trace * lambda2 + g.val.det =
            (g.val.trace - lambda1)^2 - g.val.trace * (g.val.trace - lambda1) + g.val.det := rfl
  ...
  have hv2_eq : g.val.mulVec v2 = lambda2 • v2 := by
    apply sub_eq_zero.mp
    calc g.val.mulVec v2 - lambda2 • v2 =
        g.val.mulVec v2 - lambda2 • (1 : Matrix (Fin 2) (Fin 2) (K p)).mulVec v2 :=
          congrArg (fun z => g.val.mulVec v2 - lambda2 • z) (Matrix.one_mulVec v2).symm
      _ = g.val.mulVec v2 - (lambda2 • 1 : Matrix (Fin 2) (Fin 2) (K p)).mulVec v2 := by
        rw [← Matrix.smul_mulVec]
  ...
  have h_P_det_ne_zero : (Matrix.of fun i (j : Fin 2) ↦ if j = 0 then v1 i else v2 i).det ≠ 0 := by
    ...
    have h_det : v1 0 * v2 1 - v2 0 * v1 1 = 0 := by
      have h_det_fin : (Matrix.of fun i (j : Fin 2) ↦ if j = 0 then v1 i else v2 i).det =
          v1 0 * v2 1 - v2 0 * v1 1 := by
        rw [Matrix.det_fin_two]
        change (if (0 : Fin 2) = 0 then v1 0 else v2 0) *
          (if (1 : Fin 2) = 0 then v1 1 else v2 1) -
          (if (1 : Fin 2) = 0 then v1 0 else v2 0) *
          (if (0 : Fin 2) = 0 then v1 1 else v2 1) = _
        rw [if_pos rfl, if_neg (by norm_num : (1 : Fin 2) ≠ 0)]
        rw [if_neg (by norm_num : (1 : Fin 2) ≠ 0), if_pos rfl]
    ...
    · refine ⟨fun i ↦ if i = 0 then v2 1 else -v1 1, ?_, ?_⟩
      · ext j
        ...
        | 0 =>
          ...
          rw [Fin.sum_univ_two]
          change (if (0 : Fin 2) = 0 then v2 1 else -v1 1) * v1 0 +
            (if (1 : Fin 2) = 0 then v2 1 else -v1 1) * v2 0 = 0
          rw [if_pos rfl, if_neg (by norm_num : (1 : Fin 2) ≠ 0), h, h']
          rw [mul_zero, mul_zero, add_zero]
        | 1 =>
          change ∑ i : Fin 2, (if i = 0 then v2 1 else -v1 1) * (![v1, v2] i 1) = 0
          rw [Fin.sum_univ_two]
          change (if (0 : Fin 2) = 0 then v2 1 else -v1 1) * v1 1 +
            (if (1 : Fin 2) = 0 then v2 1 else -v1 1) * v2 1 = 0
          rw [if_pos rfl, if_neg (by norm_num : (1 : Fin 2) ≠ 0)]
          ring
    ...
    · refine ⟨fun i ↦ if i = 0 then -v2 0 else v1 0, ?_, ?_⟩
      · ext j
        ...
        | 0 =>
          ...
          rw [Fin.sum_univ_two]
          change (if (0 : Fin 2) = 0 then -v2 0 else v1 0) * v1 0 +
            (if (1 : Fin 2) = 0 then -v2 0 else v1 0) * v2 0 = 0
          rw [if_pos rfl, if_neg (by norm_num : (1 : Fin 2) ≠ 0)]
          ring
        | 1 =>
          change ∑ i : Fin 2, (if i = 0 then -v2 0 else v1 0) * (![v1, v2] i 1) = 0
          rw [Fin.sum_univ_two]
          change (if (0 : Fin 2) = 0 then -v2 0 else v1 0) * v1 1 +
            (if (1 : Fin 2) = 0 then -v2 0 else v1 0) * v2 1 = 0
          rw [if_pos rfl, if_neg (by norm_num : (1 : Fin 2) ≠ 0)]
  ...
  have h_diag_det_ne_zero : (Matrix.diagonal ![lambda1, lambda2]).det ≠ 0 := by
    ...
    have hd1 : (Matrix.diagonal ![lambda1, lambda2]).det = lambda1 * lambda2 := by
      rw [Matrix.det_fin_two]
      change (Matrix.diagonal ![lambda1, lambda2] 0 0) *
        (Matrix.diagonal ![lambda1, lambda2] 1 1) -
        (Matrix.diagonal ![lambda1, lambda2] 0 1) *
        (Matrix.diagonal ![lambda1, lambda2] 1 0) = lambda1 * lambda2
      change lambda1 * lambda2 - 0 * 0 = lambda1 * lambda2
      ring
    rw [hd1] at hz
    rw [hlambda2.2] at hz
    exact g.isUnit.map Matrix.detMonoidHom |>.ne_zero hz
  let Y := Matrix.GeneralLinearGroup.mkOfDetNeZero (Matrix.diagonal ![lambda1, lambda2])
    h_diag_det_ne_zero
  obtain ⟨P, hP⟩ : ∃ P : GL (Fin 2) (K p), g = P * Y * P⁻¹ := by
    ...
    rw [← Units.mul_inv_cancel_right g.val P, h_diag]
    rfl
  have h_order_eq : orderOf (QuotientGroup.mk g : PGL p) =
      orderOf (QuotientGroup.mk Y : PGL p) := by
    have h_semi : SemiconjBy (QuotientGroup.mk P : PGL p) (QuotientGroup.mk Y : PGL p)
        (QuotientGroup.mk g : PGL p) := by
  ...
  have h_order_eq2 : orderOf (QuotientGroup.mk Y : PGL p) = orderOf (lambda1 / lambda2 : K p) := by
    ...
    intro n
    rw [show (QuotientGroup.mk Y : PGL p) ^ n = QuotientGroup.mk (Y ^ n) from rfl]
    rw [QuotientGroup.eq_one_iff, Subgroup.mem_center_iff]
    have hl2_ne : lambda2 ≠ 0 := by
      intro hc
      exact g.isUnit.map Matrix.detMonoidHom |>.ne_zero (show g.val.det = 0 by
        rw [← hlambda2.2, hc, mul_zero])
    constructor
    · intro h
      let E := Matrix.GeneralLinearGroup.mkOfDetNeZero (Matrix.of fun (i j : Fin 2) ↦
        if i = 0 then if j = 0 then (1 : K p) else 0 else if j = 0 then 1 else 1) (by
        intro hE
        have h_det : (Matrix.of fun (i j : Fin 2) ↦ if i = 0 then if j = 0 then (1 : K p)
          else 0 else if j = 0 then 1 else 1).det = 1 := by
          rw [Matrix.det_fin_two]
      ...
      have hl_eq : lambda1^n = lambda2^n := by
        ...
        change (E.val * (Matrix.diagonal ![lambda1, lambda2])^n) 1 0 =
          ((Matrix.diagonal ![lambda1, lambda2])^n * E.val) 1 0 at hE
        rw [Matrix.diagonal_pow] at hE
        change (E.val * Matrix.diagonal ![lambda1^n, lambda2^n]) 1 0 =
          (Matrix.diagonal ![lambda1^n, lambda2^n] * E.val) 1 0 at hE
    ...
    · intro h
      ...
      intro E
      apply Units.ext
      change E.val * (Matrix.diagonal ![lambda1, lambda2])^n =
        (Matrix.diagonal ![lambda1, lambda2])^n * E.val
      rw [Matrix.diagonal_pow]
      ext i j
      match i, j with
      | 0, 0 =>
        change ∑ k : Fin 2, E.val 0 k * Matrix.diagonal ![lambda1^n, lambda2^n] k 0 =
          ∑ k : Fin 2, Matrix.diagonal ![lambda1^n, lambda2^n] 0 k * E.val k 0
        rw [Fin.sum_univ_two, Fin.sum_univ_two]
        change E.val 0 0 * lambda1^n + E.val 0 1 * 0 = lambda1^n * E.val 0 0 + 0 * E.val 1 0
        ring
      | 0, 1 =>
        change ∑ k : Fin 2, E.val 0 k * Matrix.diagonal ![lambda1^n, lambda2^n] k 1 =
          ∑ k : Fin 2, Matrix.diagonal ![lambda1^n, lambda2^n] 0 k * E.val k 1
        rw [Fin.sum_univ_two, Fin.sum_univ_two]
        change E.val 0 0 * 0 + E.val 0 1 * lambda2^n = lambda1^n * E.val 0 1 + 0 * E.val 1 1
        rw [hl_eq]; ring
      | 1, 0 =>
        change ∑ k : Fin 2, E.val 1 k * Matrix.diagonal ![lambda1^n, lambda2^n] k 0 =
          ∑ k : Fin 2, Matrix.diagonal ![lambda1^n, lambda2^n] 1 k * E.val k 0
        rw [Fin.sum_univ_two, Fin.sum_univ_two]
        change E.val 1 0 * lambda1^n + E.val 1 1 * 0 = 0 * E.val 0 0 + lambda2^n * E.val 1 0
        rw [hl_eq]; ring
      | 1, 1 =>
        change ∑ k : Fin 2, E.val 1 k * Matrix.diagonal ![lambda1^n, lambda2^n] k 1 =
          ∑ k : Fin 2, Matrix.diagonal ![lambda1^n, lambda2^n] 1 k * E.val k 1
        rw [Fin.sum_univ_two, Fin.sum_univ_two]
  ...
  have h_coprime : ¬(p ∣ orderOf (lambda1 / lambda2 : K p)) := by
    ...
    have hk_pos : 0 < k := by
      have h_opos: 0 < orderOf (lambda1 / lambda2 : K p) :=
        h_order_eq2 ▸ h_order_eq ▸ h_fin.orderOf_pos
      by_contra h_k0
...
theorem orderOf_eq_prime_of_discrim_zero (g : GL (Fin 2) (K p)) (hg : ¬ ∃ c : K p, g.val = c • 1)
  ...
  have h_two_ne : (2 : K p) ≠ 0 :=
    (NeZero.of_not_dvd (R := K p) (fun h ↦
      absurd (Nat.le_of_dvd (by norm_num) h) (not_le.mpr h_odd.out))).out
  ...
  have h_N_entry (i j : Fin 2) : 2 * N i j = 2 * g.val i j -
      (g.val 0 0 + g.val 1 1) * (1 : Matrix (Fin 2) (Fin 2) (K p)) i j := by
    simp only [N, Matrix.sub_apply, Matrix.smul_apply, smul_eq_mul]
    calc 2 * (g.val i j - eig * (1 : Matrix _ _ _) i j) =
        2 * g.val i j - (2 * eig) * (1 : Matrix _ _ _) i j := by ring
      _ = 2 * g.val i j - (g.val 0 0 + g.val 1 1) * (1 : Matrix _ _ _) i j := by rw [h2_eig]
  have hN2 : N ^ 2 = 0 := by
    ext i j
    ...
    have hX : 4 * (N i 0 * N 0 j + N i 1 * N 1 j) = 0 := by
      ...
      | 0, 0 =>
        calc 4 * (N 0 0 * N 0 0 + N 0 1 * N 1 0)
          ...
          _ = (2 * g.val 0 0 - (g.val 0 0 + g.val 1 1) * 1) *
                (2 * g.val 0 0 - (g.val 0 0 + g.val 1 1) * 1) +
              (2 * g.val 0 1 - (g.val 0 0 + g.val 1 1) * 0) *
                (2 * g.val 1 0 - (g.val 0 0 + g.val 1 1) * 0) := by
            rw [h_N_entry 0 0, h_N_entry 0 1, h_N_entry 1 0]; rfl
          _ = (g.val 0 0 + g.val 1 1)^2 -
              4 * (g.val 0 0 * g.val 1 1 - g.val 0 1 * g.val 1 0) := by ring
          _ = 4 * (g.val 0 0 * g.val 1 1 - g.val 0 1 * g.val 1 0) -
                4 * (g.val 0 0 * g.val 1 1 - g.val 0 1 * g.val 1 0) := by rw [h_d]
          _ = 0 := by ring
      | 0, 1 =>
        calc 4 * (N 0 0 * N 0 1 + N 0 1 * N 1 1)
          ...
          _ = (2 * g.val 0 0 - (g.val 0 0 + g.val 1 1) * 1) *
                (2 * g.val 0 1 - (g.val 0 0 + g.val 1 1) * 0) +
              (2 * g.val 0 1 - (g.val 0 0 + g.val 1 1) * 0) *
                (2 * g.val 1 1 - (g.val 0 0 + g.val 1 1) * 1) := by
            rw [h_N_entry 0 0, h_N_entry 0 1, h_N_entry 1 1]; rfl
          _ = 0 := by ring
      | 1, 0 =>
        calc 4 * (N 1 0 * N 0 0 + N 1 1 * N 1 0)
          _ = (2 * N 1 0) * (2 * N 0 0) + (2 * N 1 1) * (2 * N 1 0) := by ring
          _ = (2 * g.val 1 0 - (g.val 0 0 + g.val 1 1) * 0) *
                (2 * g.val 0 0 - (g.val 0 0 + g.val 1 1) * 1) +
              (2 * g.val 1 1 - (g.val 0 0 + g.val 1 1) * 1) *
                (2 * g.val 1 0 - (g.val 0 0 + g.val 1 1) * 0) := by
            rw [h_N_entry 1 0, h_N_entry 0 0, h_N_entry 1 1]; rfl
          _ = 0 := by ring
      | 1, 1 =>
        calc 4 * (N 1 0 * N 0 1 + N 1 1 * N 1 1)
          ...
          _ = (2 * g.val 1 0 - (g.val 0 0 + g.val 1 1) * 0) *
              ...
              (2 * g.val 1 1 - (g.val 0 0 + g.val 1 1) * 1) *
                (2 * g.val 1 1 - (g.val 0 0 + g.val 1 1) * 1) := by
            rw [h_N_entry 1 0, h_N_entry 0 1, h_N_entry 1 1]; rfl
          _ = (g.val 0 0 + g.val 1 1)^2 -
              4 * (g.val 0 0 * g.val 1 1 - g.val 0 1 * g.val 1 0) := by ring
          _ = 4 * (g.val 0 0 * g.val 1 1 - g.val 0 1 * g.val 1 0) -
                4 * (g.val 0 0 * g.val 1 1 - g.val 0 1 * g.val 1 0) := by rw [h_d]
  ...
  have h_char : g.val ^ p = eig ^ p • 1 := by
    have hgN : g.val = eig • 1 + N := by
      ext i j
      change g.val i j = (eig • (1 : Matrix _ _ _)) i j +
        (g.val i j - (eig • (1 : Matrix _ _ _)) i j)
      ring
  ...
  have h_pow_one : (QuotientGroup.mk g : PGL p) ^ p = 1 := by
    ...
    rw [Units.val_pow_eq_pow_val, h_char, Matrix.mul_smul, Matrix.smul_mul,
...
theorem orderOf_eq_prime_of_prime_pow (g : PGL p) (k : ℕ) (hk : k ≥ 1)
  ...
  · have h_coprime := orderOf_coprime_of_discrim_ne_zero p g' h_discr
    ...
    have h_gcd_eq_p : Nat.gcd p (p ^ k) = p :=
      Nat.gcd_eq_left (dvd_pow_self p (by exact ne_of_gt hk))
...
theorem fixesInfinity_iff_upperTriangular (g : PGL p) :
    g • (infinity p) = (infinity p) ↔ ∃ (a b d : K p) (h : a * d ≠ 0),
      g = QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![a, b; 0, d] (by
        rw [Matrix.det_fin_two]; change a * d - b * 0 ≠ 0; rw [mul_zero, sub_zero]; exact h)) := by
  ...
  · rintro ⟨a, b, d, h_det, rfl⟩
    have hz_mul : (Matrix.GeneralLinearGroup.mkOfDetNeZero !![a, b; 0, d] (by
      rw [Matrix.det_fin_two]; change a * d - b * 0 ≠ 0; rw [mul_zero, sub_zero]; exact h_det)
        ).val.mulVec ![1, 0] ≠ 0 := by
      intro h_zero
      have hM_det : !![a, b; 0, d].det = 0 :=
        Matrix.exists_mulVec_eq_zero_iff.mp ⟨![1, 0], hz_one, h_zero⟩
...
theorem upper_triangular_ratio_unique
    ...
    (heq : (QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![a₁, b₁; 0, d₁] (by
        rw [Matrix.det_fin_two]; change a₁ * d₁ - b₁ * 0 ≠ 0; rw [mul_zero, sub_zero]; exact h₁)) :
          PGL p) =
  ...
  let B := Matrix.GeneralLinearGroup.mkOfDetNeZero !![a₂, b₂; 0, d₂] (by
    ...
    exact h₂)
  obtain ⟨c, hc⟩ :=
    Matrix.GeneralLinearGroup.mem_center_iff_val_mem_range_scalar.mp (QuotientGroup.eq.mp heq)
  have hB : B.val = A.val * Matrix.scalar (Fin 2) c := by
    rw [hc, ← Units.val_mul, mul_inv_cancel_left]
  have ha2 : a₂ = a₁ * c := by
  ...
  calc a₁ * d₁⁻¹
    _ = a₁ * d₁⁻¹ * (c * c⁻¹) := by
      rw [mul_inv_cancel₀ (fun hc ↦ h₂ (by rw [ha2, hc, mul_zero, zero_mul])), mul_one]
    _ = (a₁ * c) * (d₁⁻¹ * c⁻¹) := by ring
...
theorem upper_triangular_mul_ratio
    ...
    (QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![a₁, b₁; 0, d₁] (by
     ...
     QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero
        !![a₁ * a₂, a₁ * b₂ + b₁ * d₂; 0, d₁ * d₂]
      (by rw [Matrix.det_fin_two]; change (a₁ * a₂) * (d₁ * d₂) - (a₁ * b₂ + b₁ * d₂) * 0 ≠ 0;
          rw [mul_zero, sub_zero];
          rw [show (a₁ * a₂) * (d₁ * d₂) = (a₁ * d₁) * (a₂ * d₂) by ring];
...
theorem ratio_one_imp_unipotent
    ...
    orderOf (QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![a, b; 0, d]
      (by rw [Matrix.det_fin_two]; change a * d - b * 0 ≠ 0
          rw [mul_zero, sub_zero]; exact h)) : PGL p) = 1 ∨
    orderOf (QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![a, b; 0, d]
      (by rw [Matrix.det_fin_two]; change a * d - b * 0 ≠ 0
          rw [mul_zero, sub_zero]; exact h)) : PGL p) = p := by
  have ha : a ≠ 0 := fun ha_eq ↦ h (by rw [ha_eq, zero_mul])
  have h_eq : a = d := eq_of_div_eq_one hr
  let Y := Matrix.GeneralLinearGroup.mkOfDetNeZero !![a, b; 0, d] (by
    rw [Matrix.det_fin_two]; change a * d - b * 0 ≠ 0; rw [mul_zero, sub_zero]; exact h)
  let X := Matrix.GeneralLinearGroup.mkOfDetNeZero !![1, b * a⁻¹; 0, 1] (by
    rw [Matrix.det_fin_two]; change 1 * 1 - (b * a⁻¹) * 0 ≠ 0
    rw [mul_zero, sub_zero, mul_one]; exact one_ne_zero)
  have h_simplified : (QuotientGroup.mk Y : PGL p) = QuotientGroup.mk X := by
    ...
    rw [Units.val_mul, h_mat, Units.inv_mul_cancel_left]
  rw [h_simplified]
  by_cases hb : b * a⁻¹ = 0
  · left
    have hX_one : X = 1 := by
  ...
  · right
    ...
    · rintro ⟨c, hc⟩
      have h01 : X.val 0 1 = (c • (1 : Matrix (Fin 2) (Fin 2) (K p))) 0 1 :=
        congr_fun (congr_fun hc 0) 1
...
theorem isUnipotent_of_fixesInfinity_orderOf (g : PGL p) (h_fix : g • (infinity p) = (infinity p))
    (h_order : orderOf g = p) :
  ∃ x : K p, g = QuotientGroup.mk (Matrix.GeneralLinearGroup.mkOfDetNeZero !![1, x; 0, 1] (by
      rw [Matrix.det_fin_two]; change (1 : K p) * 1 - x * 0 ≠ 0;
      rw [mul_one, mul_zero, sub_zero]; exact one_ne_zero)) := by
  ...
  · use b / a
    ...
    have hAB : A.val = a • B.val := by
      ...
      | 0, 1 =>
        ...
        rw [← mul_assoc, mul_comm a b, mul_assoc,
          mul_inv_cancel₀ (mul_ne_zero_iff.mp h_det).1, mul_one]
    ...
    have h_center : (B⁻¹ * A).val = a • 1 :=
      calc (B⁻¹ * A).val = (B⁻¹).val * A.val := rfl
        ...
        _ = a • 1 := by
          rw [show (B⁻¹).val * B.val = 1 from congr_arg Units.val (inv_mul_cancel B)]
  ...
  · let A := Matrix.GeneralLinearGroup.mkOfDetNeZero
    ...
    have hg_ne_one : g ≠ 1 := fun hc ↦
      (Fact.out : Nat.Prime p).ne_one (h_order.symm.trans <|
        (congr_arg orderOf hc).trans orderOf_one)
...
theorem exists_smul_eq_infinity (x : ProjectiveLine p) :
  ...
  set v := x.rep
  obtain ⟨A, hAdet, hAv⟩ : ∃ A : Matrix _ _ (K p), A.det ≠ 0 ∧ A.mulVec v = ![1, 0] := by
    by_cases hv0 : v 0 = 0
    · have hv1 : v 1 ≠ 0 := fun h ↦
        x.rep_nonzero (funext fun i ↦ match i with | 0 => hv0 | 1 => h)
  ...
  rw [← Projectivization.mk_rep x]
  exact (Projectivization.mk_eq_mk_iff (K p) _ _ _ _).mpr ⟨1, by
    rw [one_smul]; exact hAv.symm⟩
...
theorem commute_of_orderOf_prime_fixesInfinity (g h : PGL p) (hg_order : orderOf g = p)
  ...
  congr 1
  apply Units.ext
  change (!![1, x; 0, 1] * !![1, y; 0, 1] : Matrix (Fin 2) (Fin 2) _) =
      (!![1, y; 0, 1] * !![1, x; 0, 1] : Matrix (Fin 2) (Fin 2) _)
  ext i j
  rw [Matrix.mul_apply, Fin.sum_univ_two, Matrix.mul_apply, Fin.sum_univ_two]
  match i, j with
...
theorem commute_of_orderOf_prime_common_fixedPoint (g h : PGL p) (x : ProjectiveLine p)
  ...
  have order_conj_eq : ∀ y : PGL p, orderOf (k * y * k⁻¹) = orderOf y := fun y ↦
    SemiconjBy.orderOf_eq k⁻¹
        (show SemiconjBy k⁻¹ (k * y * k⁻¹) y by
          change k⁻¹ * (k * y * k⁻¹) = y * k⁻¹; group)
...
theorem card_center_GL2 :
  ...
  have h_center : ∀ g : GL (Fin 2) F,
      g ∈ Subgroup.center (GL (Fin 2) F) ↔
        ∃ c : F, c ≠ 0 ∧ (g : Matrix (Fin 2) (Fin 2) F) = Matrix.diagonal ![c, c] := by
    intro g
    constructor
    · intro hg
      have h_comm : ∀ h : Matrix (Fin 2) (Fin 2) F, h.det ≠ 0 →
          (g : Matrix (Fin 2) (Fin 2) F) * h = h * (g : Matrix (Fin 2) (Fin 2) F) := fun h hh ↦
        congrArg Units.val
          (Subgroup.mem_center_iff.mp hg (Matrix.GeneralLinearGroup.mkOfDetNeZero h hh)).symm
      have eq_01 : (g : Matrix (Fin 2) (Fin 2) F) 0 1 = 0 := by
        ...
        change (g : Matrix (Fin 2) (Fin 2) F) 0 0 * 1 + (g : Matrix (Fin 2) (Fin 2) F) 0 1 * 1 =
          1 * (g : Matrix (Fin 2) (Fin 2) F) 0 0 + 0 * (g : Matrix (Fin 2) (Fin 2) F) 1 0 at h
      ...
      have eq_10 : (g : Matrix (Fin 2) (Fin 2) F) 1 0 = 0 := by
        ...
        simp only [Matrix.mul_apply, Fin.sum_univ_two] at h
        change (g : Matrix (Fin 2) (Fin 2) F) 1 0 * 1 + (g : Matrix (Fin 2) (Fin 2) F) 1 1 * 1 =
          0 * (g : Matrix (Fin 2) (Fin 2) F) 0 1 + 1 * (g : Matrix (Fin 2) (Fin 2) F) 1 1 at h
        simp only [mul_one, one_mul, zero_mul] at h
      ...
      have eq_11 : (g : Matrix (Fin 2) (Fin 2) F) 1 1 = (g : Matrix (Fin 2) (Fin 2) F) 0 0 := by
        ...
        simp only [Matrix.mul_apply, Fin.sum_univ_two] at h
        change (g : Matrix (Fin 2) (Fin 2) F) 1 0 * 1 + (g : Matrix (Fin 2) (Fin 2) F) 1 1 * 1 =
          1 * (g : Matrix (Fin 2) (Fin 2) F) 0 0 + 1 * (g : Matrix (Fin 2) (Fin 2) F) 1 0 at h
    ...
    · rintro ⟨c, _, hg⟩
      ...
      match i, j with
      | 0, 0 =>
        change (h : Matrix (Fin 2) (Fin 2) F) 0 0 * c + (h : Matrix (Fin 2) (Fin 2) F) 0 1 * 0 =
          c * (h : Matrix (Fin 2) (Fin 2) F) 0 0 + 0 * (h : Matrix (Fin 2) (Fin 2) F) 1 0
        rw [mul_zero, add_zero, zero_mul, add_zero, mul_comm]
      | 0, 1 =>
        change (h : Matrix (Fin 2) (Fin 2) F) 0 0 * 0 + (h : Matrix (Fin 2) (Fin 2) F) 0 1 * c =
          c * (h : Matrix (Fin 2) (Fin 2) F) 0 1 + 0 * (h : Matrix (Fin 2) (Fin 2) F) 1 1
        rw [mul_zero, zero_add, zero_mul, add_zero, mul_comm]
      | 1, 0 =>
        change (h : Matrix (Fin 2) (Fin 2) F) 1 0 * c + (h : Matrix (Fin 2) (Fin 2) F) 1 1 * 0 =
          0 * (h : Matrix (Fin 2) (Fin 2) F) 0 0 + c * (h : Matrix (Fin 2) (Fin 2) F) 1 0
        rw [mul_zero, add_zero, zero_mul, zero_add, mul_comm]
      | 1, 1 =>
        change (h : Matrix (Fin 2) (Fin 2) F) 1 0 * 0 + (h : Matrix (Fin 2) (Fin 2) F) 1 1 * c =
          0 * (h : Matrix (Fin 2) (Fin 2) F) 0 1 + c * (h : Matrix (Fin 2) (Fin 2) F) 1 1
        rw [mul_zero, zero_add, zero_mul, zero_add, mul_comm]
  let e : Subgroup.center (GL (Fin 2) F) ≃ {c : F // c ≠ 0} := {
    ...
    left_inv := fun g ↦ Subtype.ext (Units.ext (by
      ...
      change Matrix.diagonal ![(g.1 : Matrix (Fin 2) (Fin 2) F) 0 0,
        (g.1 : Matrix (Fin 2) (Fin 2) F) 0 0] = _
      rw [hg]
      rfl))
