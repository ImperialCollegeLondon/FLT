...
noncomputable section NatClassEquation

open Classical in
lemma card_conjugates_eq_normalizer_index {G' : Type*} [Group G'] [Finite G']
    (H : Subgroup G') :
    Nat.card {K : Subgroup G' | ∃ g : G', K = H.map (MulAut.conj g).toMonoidHom} =
    (Subgroup.normalizer (H : Set G')).index := by
  rw [Subgroup.index_eq_card, Nat.card_congr]
  symm
  ...
  · obtain ⟨g, hg⟩ := a.property
    have hn := QuotientGroup.out_eq' (QuotientGroup.mk g : G' ⧸ (Subgroup.normalizer (H : Set G')))
    rw [QuotientGroup.eq] at hn
    have h_conj : H.map (MulAut.conj g).toMonoidHom =
        H.map (MulAut.conj (g : G' ⧸ (Subgroup.normalizer (H : Set G'))).out).toMonoidHom := by
      ext x
      simp only [Subgroup.mem_map, MulAut.conj_apply, MulEquiv.coe_toMonoidHom]
      constructor
      · rintro ⟨y, hy, rfl⟩
        have key : ((g : G' ⧸ (Subgroup.normalizer (H : Set G'))).out⁻¹ * g) * y *
            ((g : G' ⧸ (Subgroup.normalizer (H : Set G'))).out⁻¹ * g)⁻¹ =
            (g : G' ⧸ (Subgroup.normalizer (H : Set G'))).out⁻¹ * (g * y * g⁻¹) *
            (g : G' ⧸ (Subgroup.normalizer (H : Set G'))).out := by group
        exact ⟨_, key ▸
          (Subgroup.mem_normalizer_iff.mp hn y).mp hy, by group⟩
      · rintro ⟨y, hy, rfl⟩
        have key2 : ((g : G' ⧸ (Subgroup.normalizer (H : Set G'))).out⁻¹ * g) *
            (((g : G' ⧸ (Subgroup.normalizer (H : Set G'))).out⁻¹ * g)⁻¹ * y *
            ((g : G' ⧸ (Subgroup.normalizer (H : Set G'))).out⁻¹ * g)) *
            ((g : G' ⧸ (Subgroup.normalizer (H : Set G'))).out⁻¹ * g)⁻¹ = y := by group
        exact ⟨_, (Subgroup.mem_normalizer_iff.mp hn _).mpr
          (key2.symm ▸ hy), by group⟩
    exact ⟨g, Subtype.ext (by rw [hg, h_conj])⟩

lemma conjClass_unique_index {G' : Type*} [Group G']
...
open Classical in
lemma card_conjNonidentity {G' : Type*} [Group G'] [Finite G']
    (H : Subgroup G') (g : G') :
  ...
  have h_eq : conjNonidentity H g = K.carrier.toFinset.erase 1 := by
    ext x
    simp only [conjNonidentity, Finset.mem_filter_univ, Finset.mem_erase, Set.mem_toFinset]
    rfl
  rw [h_eq, Finset.card_erase_of_mem]
  · rw [Set.toFinset_card, ← Nat.card_eq_fintype_card]
    exact congrArg (fun x ↦ x - 1)
      (Nat.card_congr (MulEquiv.subgroupMap (MulAut.conj g) H).toEquiv.symm)
  · exact Set.mem_toFinset.mpr (Subgroup.one_mem K)

open Classical in
lemma card_conjClassElements {G' : Type*} [Group G'] [Finite G']
    (H : Subgroup G')
    (hdisjoint : ∀ K L : Subgroup G',
      (∃ g : G', K = H.map (MulAut.conj g).toMonoidHom) →
      (∃ h : G', L = H.map (MulAut.conj h).toMonoidHom) →
      K ≠ L → K ⊓ L = ⊥) :
    (conjClassElements H).card = (Subgroup.normalizer (H : Set G')).index * (Nat.card H - 1) := by
  ...
  rw [show conjClassElements H = Finset.biUnion S
      ...
      ext x
      simp only [conjClassElements, S, Finset.mem_filter_univ, Finset.mem_biUnion]
      constructor
      · rintro ⟨hx, g, hg⟩
    ...
    Finset.card_biUnion (fun K hK L hL hKL ↦ by
      simp only [Finset.disjoint_left, Finset.mem_filter_univ]
      rintro x ⟨hx1, hxK⟩ ⟨_, hxL⟩
      exact hx1 ((Subgroup.ext_iff.mp (hdisjoint K L
        ((Finset.mem_filter_univ K).mp hK)
        ((Finset.mem_filter_univ L).mp hL) hKL) x).mp ⟨hxK, hxL⟩))]
  trans ∑ K ∈ S, (Nat.card H - 1)
  · refine Finset.sum_congr rfl fun K hK ↦ ?_
    simp only [S, Finset.mem_filter_univ] at hK
    obtain ⟨g, rfl⟩ := hK
    exact card_conjNonidentity H g
  · rw [Finset.sum_const, smul_eq_mul, ← h_num]
    congr 1
    rw [Nat.card_eq_fintype_card]
    exact Fintype.card_of_subtype S (fun _ ↦ by
...
lemma natClassEquation {G' : Type*} [Group G'] [Finite G']
    ...
    (hinj : ∀ i j : Fin r, (∃ g : G',
      (H i).map (MulAut.conj g).toMonoidHom = H j) → i = j) :
    Nat.card G' - 1 =
      ∑ i : Fin r, (Subgroup.normalizer (H i : Set G')).index * (Nat.card (H i) - 1) := by
  have h_total_non_identity :
    ...
    · refine congr_arg Finset.card (Finset.ext fun x ↦ ?_)
      simp only [conjClassElements, Finset.mem_filter_univ, Finset.mem_biUnion]
      constructor
      · intro hx
        rcases hcover x hx with ⟨K, ⟨i, g, rfl⟩, hxK⟩
        exact ⟨i, Finset.mem_univ i, hx, g, hxK⟩
      · rintro ⟨_, _, hx, _, _⟩
        exact hx
    · intro i _ j _ hij
      simp only [conjClassElements, Finset.disjoint_left, Finset.mem_filter_univ]
  ...
  · have h_fil_neq_one: Finset.univ.filter (fun x : G' ↦ x ≠ 1) = Finset.univ.erase 1 := by
      ext x
      simp only [Finset.mem_filter_univ, Finset.mem_erase]
      exact ⟨fun h ↦ ⟨h, Finset.mem_univ x⟩, fun h ↦ h.1⟩
    rw [h_fil_neq_one, Finset.card_erase_of_mem (Finset.mem_univ 1),
...
end NatClassEquation

noncomputable section TameClassification
...
lemma fixedPoints_determined (G : Subgroup (PGL p)) [Finite G]
    (hG_tame : ¬ (p : ℕ) ∣ Nat.card G)
    (g : PGL p) (hg : g ∈ G) (hg_ne : g ≠ 1)
...
omit h_odd in
lemma commute_of_fixedPair (G_sub : Subgroup (PGL p)) [Finite G_sub]
  ...
  have h_comm_u : ∀ u : Fin 2 → K p,
    ...
    exact (Matrix.mulVec_mulVec _ _ _).symm.trans (Eq.trans (by
      simp only [Matrix.mulVec_add, Matrix.mulVec_smul, hc, hd, he, hf, smul_smul]
...
lemma pairStabilizer_isCyclic (G : Subgroup (PGL p)) [Finite G]
  ...
  have hf_mul : ∀ g₁ g₂ : pairStabilizer p G x y, f (g₁ * g₂) = f g₁ * f g₂ := by
    ...
    · change (QuotientGroup.mk (f g₁) : PGL p) * (QuotientGroup.mk (f g₂) : PGL p) = (g₁ : PGL p) *
      ...
      rw [hf_mk, hf_mk]
    · rw [Units.val_mul]
      exact (Matrix.mulVec_mulVec y.rep _ _).symm.trans (by rw [hf_y, hf_y])
  have hc_mul : ∀ g₁ g₂ : pairStabilizer p G x y, c (g₁ * g₂) = c g₁ * c g₂ := by
    intro g₁ g₂
    have h : (c (g₁ * g₂) - c g₁ * c g₂) • x.rep = 0 := by
      rw [sub_smul, sub_eq_zero, ← hc_x, hf_mul, Units.val_mul]
      exact (Matrix.mulVec_mulVec x.rep _ _).symm.trans <| by
        rw [hc_x, Matrix.mulVec_smul, hc_x, smul_smul, mul_comm]
  ...
  have hf_one : f 1 = 1 := by
    ...
    · rw [Units.val_one]
      exact Matrix.one_mulVec y.rep
  have h_c_one : c 1 = 1 := by
    have h : (1 - c 1) • x.rep = 0 := by
      rw [sub_smul, one_smul, ← Matrix.one_mulVec x.rep, ← Units.val_one, ← hf_one]
      exact (congrArg (fun z => z - c 1 • z) (hc_x 1)).trans <| by
        rw [smul_smul, ← hc_mul 1 1, mul_one, sub_self]
...
lemma disjoint_fixedPairs (G : Subgroup (PGL p)) [Finite G]
    (hG_tame : ¬ (p : ℕ) ∣ Nat.card G)
...
lemma normalizer_permutes_pair (G : Subgroup (PGL p)) [Finite G]
    (hG_tame : ¬ (p : ℕ) ∣ Nat.card G)
    (x y : ProjectiveLine p) (hxy : x ≠ y)
    (n : Subgroup.normalizer (pairStabilizer p G x y : Set ↥G))
    (hne : ∃ g : pairStabilizer p G x y, g ≠ 1) :
    (n.1.val • x = x ∧ n.1.val • y = y) ∨
    (n.1.val • x = y ∧ n.1.val • y = x) := by
  obtain ⟨g, hg_ne⟩ := hne
  have h_fix_set : fixedPoints p g.1.val = {x, y} :=
    fixedPoints_determined p G hG_tame g.1.val g.1.property
      (fun h ↦ hg_ne (Subtype.ext (Subtype.ext h))) x y hxy g.2.1 g.2.2
  have hn_mem := (Subgroup.mem_normalizer_iff.mp n.property g.1).mp g.property
  have h_prop {z : ProjectiveLine p} (hz : ((n.1 * g.1 * n.1⁻¹ : ↥G) : PGL p) • z = z) :
      n.1.val⁻¹ • z ∈ ({x, y} : Set (ProjectiveLine p)) := by
    rw [← h_fix_set]
    change g.1.val • (n.1.val⁻¹ • z) = n.1.val⁻¹ • z
    rw [Subgroup.coe_mul, Subgroup.coe_mul, Subgroup.coe_inv, mul_smul, mul_smul] at hz
    rw [← one_smul (PGL p) (g.1.val • (n.1.val⁻¹ • z)), ← inv_mul_cancel n.1.val,
        mul_smul, hz]
  have hnx := h_prop hn_mem.1
  have hny := h_prop hn_mem.2
  rw [Set.mem_insert_iff, Set.mem_singleton_iff] at hnx hny
  have h_eq_smul {z w : ProjectiveLine p} (h : n.1.val⁻¹ • z = w) : n.1.val • w = z := by
    rw [← h, ← mul_smul, mul_inv_cancel n.1.val, one_smul]
...
open Classical in
lemma normalizer_pairStabilizer_index_le_two (G : Subgroup (PGL p)) [Finite G]
    (hG_tame : ¬ (p : ℕ) ∣ Nat.card G)
    (x y : ProjectiveLine p) (hxy : x ≠ y)
    (hne : ∃ g : pairStabilizer p G x y, g ≠ 1) :
    ((pairStabilizer p G x y).subgroupOf
      (Subgroup.normalizer (pairStabilizer p G x y : Set ↥G))).index ≤ 2 := by
  let N := Subgroup.normalizer (pairStabilizer p G x y : Set ↥G)
  let H_sub := (pairStabilizer p G x y).subgroupOf N
  have h_perm (n : N) : (n.1.val • x = x ∧ n.1.val • y = y) ∨ (n.1.val • x = y ∧ n.1.val • y = x) :=
    normalizer_permutes_pair p G hG_tame x y hxy n hne
  by_cases hk : ∃ k : N, (k.1.val • x = y ∧ k.1.val • y = x)
  · obtain ⟨k, hk_swap⟩ := hk
    let Q := N ⧸ H_sub
    have h_all : ∀ q : Q, q = 1 ∨ q = QuotientGroup.mk k := by
      ...
      · apply Or.inr
        ...
        change (n.1.val⁻¹ * k.1.val) • x = x ∧ (n.1.val⁻¹ * k.1.val) • y = y
...
lemma mem_pairStabilizer_of_ne_one (G : Subgroup (PGL p)) [Finite G]
    (hG_tame : ¬ (p : ℕ) ∣ Nat.card G)
    (g : G) (hg_ne : g ≠ 1) :
...
lemma conjugate_pairStabilizer_disjoint (G : Subgroup (PGL p)) [Finite G]
    (hG_tame : ¬ (p : ℕ) ∣ Nat.card G)
    (H₁ H₂ : Subgroup G)
...
lemma normalizer_card_eq_index_mul {G : Type*} [Group G] [Finite G]
    (H : Subgroup G) :
    Nat.card (Subgroup.normalizer (H : Set G)) =
      (H.subgroupOf (Subgroup.normalizer (H : Set G))).index * Nat.card H := by
  have h_congr : Nat.card (H.subgroupOf (Subgroup.normalizer (H : Set G))) = Nat.card H :=
    Nat.card_congr
      ⟨fun x ↦ ⟨x.val.val, x.property⟩,
       fun x ↦ ⟨⟨x.val, Subgroup.le_normalizer x.property⟩, x.property⟩,
       fun _ ↦ rfl,
       fun _ ↦ rfl⟩
  rw [← h_congr]
...
open Classical in
lemma finQuotientReps {α : Type*} [Finite α] (s : Setoid α) :
    ∃ (r : ℕ) (reps : Fin r → α),
      (∀ x : α, ∃ i, s.r x (reps i)) ∧
...
omit h_odd in
lemma orbitPartitionConstruction (G : Subgroup (PGL p)) [Finite G] :
...
lemma orbitPartitionData (G : Subgroup (PGL p)) [Finite G]
    (hG_tame : ¬ (p : ℕ) ∣ Nat.card G)
    ...
    ∃ (r : ℕ) (d f : Fin r → ℕ)
      ...
      (∀ i, Nat.card (Subgroup.normalizer (H i : Set ↥G)) = f i * d i) ∧
      (∀ K L : Subgroup ↥G,
        (∃ (i : Fin r) (g : ↥G), K = (H i).map (MulAut.conj g).toMonoidHom) →
        (∃ (j : Fin r) (h : ↥G), L = (H j).map (MulAut.conj h).toMonoidHom) →
        K ≠ L → K ⊓ L = ⊥) ∧
  ...
  obtain ⟨r, xrep, yrep, hxy, hnt, horbit, hinj⟩ :=
    orbitPartitionConstruction p G
  let H : Fin r → Subgroup ↥G := fun i ↦ pairStabilizer p G (xrep i) (yrep i)
  let d : Fin r → ℕ := fun i ↦ Nat.card (H i)
  let f : Fin r → ℕ := fun i ↦ (H i).subgroupOf (Subgroup.normalizer (H i : Set ↥G)) |>.index
  have hd_ge : ∀ i, d i ≥ 2 := fun i ↦ by
    change Nat.card (H i) ≥ 2; rw [Nat.card_eq_fintype_card]; exact Fintype.one_lt_card
  ...
  refine ⟨r, d, f, H,
    ...
    fun i ↦ by
      ...
      have hz : f i ≠ 0 :=
        Subgroup.index_ne_zero_of_finite (H := (H i).subgroupOf
          (Subgroup.normalizer (H i : Set ↥G)))
      omega,
    fun i ↦ by
  ...
  have hnat' : Nat.card ↥G - 1 = ∑ i : Fin r, (Nat.card ↥G / (f i * d i)) * (d i - 1) := by
    rw [natClassEquation r H hdisjoint hcover (fun i j ⟨g, hg⟩ ↦ hinj i j ⟨g, hg⟩)]
    congr 1; ext i; congr 1
    have h_idx := Subgroup.index_mul_card (Subgroup.normalizer (H i : Set ↥G))
...
lemma r2Solutions (d f : Fin 2 → ℕ) (n : ℕ) (hn : n ≥ 2)
  ...
  · rw [hf0, hf1] at hsum'; simp only [Nat.cast_one, div_one, one_mul] at hsum'
    have h_leq : (1 : ℚ) / d 0 + 1 / d 1 ≤ 1 :=
      calc _ ≤ 1 / (2 : ℚ) + 1 / 2 := by
            gcongr
            · exact Nat.cast_le.mpr (hd 0)
            · exact Nat.cast_le.mpr (hd 1)
        _ = 1 := by norm_num
    ...
    have h_d0_le : d 0 ≤ 3 := by
      ...
      have h_leq : (1 : ℚ) / d 0 + 1 / (2 * (d 1 : ℚ)) ≤ 1 / 2 := by
        calc (1 : ℚ) / d 0 + 1 / (2 * (d 1 : ℚ)) ≤ 1 / 4 + 1 / 4 := by
              gcongr
              · exact_mod_cast show d 0 ≥ 4 by omega
              · exact_mod_cast show 2 * d 1 ≥ 4 by have := hd 1; omega
          _ = 1 / 2 := by norm_num
      linarith only [h_inv_sum, h_leq,
        div_pos (by norm_num : (0:ℚ) < 1) (by positivity : (0:ℚ) < n)]
    have hd0_ge : d 0 ≥ 2 := hd 0
    interval_cases h0 : d 0
    · left; refine ⟨1, 0, by norm_num, hf1, hf0, h0, ?_⟩
      exact_mod_cast h_n_of (2 * (d 1 : ℚ)) (by have := hd 1; positivity) <| by
        linarith only [h_inv_sum]
    · have h1 : d 1 = 2 := by
        have : d 1 ≤ 2 := by
          by_contra! h
      ...
      exact_mod_cast h_n_of 12 (by norm_num) <| by
        rw [h1_q] at h_inv_sum; linarith only [h_inv_sum]
  · rw [hf0, hf1] at hsum'; simp only [Nat.cast_one, Nat.cast_two, div_one, one_mul] at hsum'
    ...
    have h_d1_le : d 1 ≤ 3 := by
      ...
      have h_leq : (1 : ℚ) / (2 * (d 0 : ℚ)) + 1 / d 1 ≤ 1 / 2 := by
        calc (1 : ℚ) / (2 * (d 0 : ℚ)) + 1 / d 1 ≤ 1 / 4 + 1 / 4 := by
              gcongr
              · exact_mod_cast show 2 * d 0 ≥ 4 by have := hd 0; omega
    ...
    · left; refine ⟨0, 1, by norm_num, hf0, hf1, h1, ?_⟩
      exact_mod_cast h_n_of (2 * (d 0 : ℚ)) (by have := hd 0; positivity) <| by
        linarith only [h_inv_sum]
    · have h0 : d 0 = 2 := by
        have : d 0 ≤ 2 := by
          by_contra! h
      ...
      exact_mod_cast h_n_of 12 (by norm_num) <| by
        rw [h0_q] at h_inv_sum; linarith only [h_inv_sum]
...
lemma r3SolutionsUnsorted (d : Fin 3 → ℕ) (n : ℕ) (hn : n ≥ 2) (hd : ∀ i, d i ≥ 2)
  ...
  have hsum' : (1 : ℚ) / d (σ 0) + 1 / d (σ 1) + 1 / d (σ 2) = 1 + 2 / n := by
    have h_sum_comp := Equiv.sum_comp σ (fun i ↦ (1 : ℚ) / d i)
    rw [Fin.sum_univ_three] at h_sum_comp
    exact h_sum_comp.trans hsum
  have hd₀ : d (σ 0) = 2 := by
    ...
    have h_leq : (1 : ℚ) / d (σ 0) + 1 / d (σ 1) + 1 / d (σ 2) ≤ 1 :=
      calc (1 : ℚ) / d (σ 0) + 1 / d (σ 1) + 1 / d (σ 2)
        _ ≤ 1 / (d (σ 0) : ℚ) + 1 / (d (σ 0) : ℚ) + 1 / (d (σ 0) : ℚ) := by
          gcongr
          · exact Nat.cast_le.mpr hσ.1
          · exact Nat.cast_le.mpr (hσ.1.trans hσ.2)
        _ = 3 / (d (σ 0) : ℚ) := by ring
...
lemma dihedral2_of_hasCyclicPartition (G : Type*) [Group G] [Finite G]
    (hN : Nat.card G = 4)
  ...
  letI : Module (ZMod 2) (Additive G) := AddCommGroup.zmodModule fun x ↦ by
    rw [two_nsmul]; change x.toMul * x.toMul = 1; exact h_sq x.toMul
  letI : Module (ZMod 2) (Additive (DihedralGroup 2)) := AddCommGroup.zmodModule fun x ↦ by
    rw [two_nsmul]; change x.toMul * x.toMul = 1
...
lemma r2CaseOdd (G : Type*) [Group G] [Finite G]
  ...
  have h_zpowers_g : Subgroup.zpowers g.1 = H_a := by
    ext y; exact ⟨fun h ↦ let ⟨n,
      ...
      fun hy ↦ let ⟨n, hn⟩ := Subgroup.mem_zpowers_iff.mp (hg_gen ⟨y, hy⟩)
        have h_pow : (g.1 ^ n) = y := congr_arg Subtype.val hn; Subgroup.mem_zpowers_iff.mpr ⟨n,
            h_pow⟩⟩
  have hg_ord : orderOf g.1 = d := by
    calc orderOf g.1 = orderOf g := orderOf_injective H_a.subtype Subtype.coe_injective g
  ...
  have hz_ord : orderOf z = 2 := by
    calc orderOf z = orderOf g.1 / Nat.gcd (orderOf g.1) k := orderOf_pow g.1
  ...
  have hz_unique : ∀ x ∈ H_a, orderOf x = 2 → x = z := fun x _ hx_ord ↦ by
    ...
    have h_g_2n : g.1 ^ ((2 : ℤ) * n) = 1 :=
      calc g.1 ^ ((2 : ℤ) * n)
          = (g.1 ^ n) ^ (2 : ℤ) := by
            rw [mul_comm]; exact zpow_mul g.1 n 2
        _ = x ^ (2 : ℤ) := by rw [hn_eq]
        _ = 1 := by
    ...
    have h_x_zpow : x = z ^ c := calc x = g.1 ^ n := hn_eq.symm
      _ = (g.1 ^ (k : ℤ)) ^ c := by rw [hc_eq, zpow_mul]
    ...
    · exfalso
      ...
      rw [h_ord_one] at hx_ord; exact absurd hx_ord (by norm_num)
  ...
  have hz_in_b : z ∈ H_b := by
    ...
    have hy_not_one : y_sub.1 ≠ 1 :=
      fun h ↦ hy_neq (Subtype.ext h)
    have hz_not_one : z ≠ 1 := fun h ↦ by
      rw [h, orderOf_one] at hz_ord
      exact absurd hz_ord (by norm_num)
    have hz_norm_b : z ∈ Subgroup.normalizer (H_b : Set G) :=
      Subgroup.mem_normalizer_iff.mpr fun w ↦ by
        have hw : w * z = z * w := Subgroup.mem_center_iff.mp hz_center w
        rw [show z * w * z⁻¹ = w by rw [← hw]; group]
    let S : Finset (Subgroup.normalizer (H_b : Set G)) := insert ⟨1, Subgroup.one_mem _⟩
      (insert ⟨y_sub, Subgroup.le_normalizer y_sub.property⟩ {⟨z, hz_norm_b⟩})
      ...
      rw [Finset.card_insert_of_notMem (by
        ...
        Finset.card_insert_of_notMem (by
            ...
            exact fun h ↦ hz_not_b <| by
              have h_eq : y_sub.1 = z := Subtype.ext_iff.mp h
              rw [← h_eq]; exact y_sub.property),
        Finset.card_singleton]
    have h_bound : S.card ≤ Fintype.card (Subgroup.normalizer (H_b : Set G)) :=
...
lemma r3D233Contradiction (G : Type*) [Group G] [Finite G]
    (hn : Nat.card G = 12)
    (H : Subgroup G) (hH_card : Nat.card H = 3)
    (hH_norm : Nat.card (Subgroup.normalizer (H : Set G)) = 6) : False := by
  have h_pgroup : IsPGroup 3 H := fun x ↦ ⟨1, by
    change x ^ 3 = 1
    rw [← hH_card, Nat.card_eq_fintype_card]
    exact pow_card_eq_one⟩
  obtain ⟨P, hP_le⟩ := h_pgroup.exists_le_sylow
  have h_eq : (P : Set G) = (H : Set G) := (Set.eq_of_subset_of_ncard_le hP_le <| by
    change Nat.card ↑P ≤ Nat.card H
    rw [hH_card, P.card_eq_multiplicity, hn,
        Nat.factorization_eq_one (m := 4) rfl Nat.prime_three (by norm_num), pow_one]).symm
  have h_idx : (Subgroup.normalizer (H : Set G)).index = 2 := by
    have h_mul := Subgroup.index_mul_card (Subgroup.normalizer (H : Set G))
...
lemma buildPartition_r1 (G : Type*) [Group G] [Finite G]
    (H : Subgroup G)
    (hcyc : IsCyclic H) (hcard : Nat.card H = Nat.card G)
    (hnorm : Nat.card (Subgroup.normalizer (H : Set G)) = 1 * Nat.card G)
...
lemma buildPartition_r2_dihedral (G : Type*) [Group G] [Finite G]
    (Ha Hb : Subgroup G) (da : ℕ)
    (ha_cyc : IsCyclic Ha) (hb_cyc : IsCyclic Hb)
    (ha_card : Nat.card Ha = da) (hb_card : Nat.card Hb = 2)
    (ha_norm : Nat.card (Subgroup.normalizer (Ha : Set G)) = 2 * da)
    (hb_norm : Nat.card (Subgroup.normalizer (Hb : Set G)) = 1 * 2)
    (hdisj : ∀ K L : Subgroup G,
...
lemma buildPartition_r2_A4 (G : Type*) [Group G] [Finite G]
    (Ha Hb : Subgroup G)
    (ha_cyc : IsCyclic Ha) (hb_cyc : IsCyclic Hb)
    (ha_card : Nat.card Ha = 2) (hb_card : Nat.card Hb = 3)
    (ha_norm : Nat.card (Subgroup.normalizer (Ha : Set G)) = 4)
    (hb_norm : Nat.card (Subgroup.normalizer (Hb : Set G)) = 3)
    (hdisj : ∀ K L : Subgroup G,
      (∃ (i : Fin 2) (g : G), K = (![Ha, Hb] i).map (MulAut.conj g).toMonoidHom) →
      (∃ (j : Fin 2) (h : G), L = (![Ha, Hb] j).map (MulAut.conj h).toMonoidHom) →
      K ≠ L → K ⊓ L = ⊥)
...
lemma buildPartition_r3_evenDihedral (G : Type*) [Group G] [Finite G]
    ...
    (h0_norm : Nat.card (Subgroup.normalizer (H0 : Set G)) = 4)
    (h1_norm : Nat.card (Subgroup.normalizer (H1 : Set G)) = 4)
    (h2_norm : Nat.card (Subgroup.normalizer (H2 : Set G)) = 2 * k)
    (hdisj : ∀ K L : Subgroup G,
  ...
  · intro x hx
    obtain ⟨K_sub, ⟨i, g, rfl⟩, hxK⟩ := hcover x hx
    match i with
    | 0 => exact ⟨_, ⟨⟨1, by change (1 : ℕ) < 3; omega⟩, g, H0, rfl, rfl⟩, hxK⟩
    | 1 => exact ⟨_, ⟨⟨2, by change (2 : ℕ) < 3; omega⟩, g, H1, rfl, rfl⟩, hxK⟩
    | 2 => exact ⟨_, ⟨⟨0, by change (0 : ℕ) < 3; omega⟩, g, H2, rfl, rfl⟩, hxK⟩

lemma buildPartition_r3_exceptional (G : Type*) [Group G] [Finite G]
    (H0 H1 H2 : Subgroup G) (d0 d1 d2 : ℕ)
    (h0_cyc : IsCyclic H0) (h1_cyc : IsCyclic H1) (h2_cyc : IsCyclic H2)
    (h0_card : Nat.card H0 = d0) (h1_card : Nat.card H1 = d1) (h2_card : Nat.card H2 = d2)
    (h0_norm : Nat.card (Subgroup.normalizer (H0 : Set G)) = 2 * d0)
    (h1_norm : Nat.card (Subgroup.normalizer (H1 : Set G)) = 2 * d1)
    (h2_norm : Nat.card (Subgroup.normalizer (H2 : Set G)) = 2 * d2)
    (hdisj : ∀ K L : Subgroup G,
      (∃ (i : Fin 3) (g : G), K = (![H0, H1, H2] i).map (MulAut.conj g).toMonoidHom) →
      (∃ (j : Fin 3) (h : G), L = (![H0, H1, H2] j).map (MulAut.conj h).toMonoidHom) →
      K ≠ L → K ⊓ L = ⊥)
  ...
  · exfalso; exact lt_irrefl (1 : ℚ) <|
      calc (1 : ℚ)
        ...
        _ ≤ (1 : ℚ) / (f i) * (1 - 1 / (d i)) +
            ...
            (1 : ℚ) / (f j) * (1 - 1 / (d j)) :=
              add_le_add (by rw [hfi]; gcongr <;>
                first | exact le_refl 1 | exact Nat.cast_le.mpr (hd i))
...
lemma tame_hasCyclicPartition (G : Subgroup (PGL p)) [Finite G]
  ...
  · have h_fin2_disj (a b : Fin 2) : ∀ K L : Subgroup ↥G,
    ...
    · have h_ne : H a ≠ H b := fun h_eq ↦ by
      ...
      · obtain ⟨k, hk⟩ : ∃ k, d a = 2 * k + 1 := h_odd
        have hk_pos : k > 0 := Nat.pos_of_ne_zero fun h ↦ by
          subst h; have h_ge:= hge2 a; linarith only [hk, h_ge]
        linarith only [hk, hk_pos]
  ...
  · have hf2 : ∀ i, f i = 2 := r3_allF_eq_two d f (Nat.card (↥G)) hG2 hge2 hf12 heq
    have hsum' : (∑ i : Fin 3, (1 : ℚ) / (d i)) = 1 + 2 / (Nat.card (↥G)) := by
      simp only [Fin.sum_univ_three, hf2, Nat.cast_two] at heq ⊢
