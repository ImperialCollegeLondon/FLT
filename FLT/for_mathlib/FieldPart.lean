...
theorem division_char_is_commutative {D : Type*} [DivisionRing D] {p : ℕ} [Fact p.Prime] [CharP D p]
    ...
    mul_comm := by
      ...
      let fun1 : Module.End D D :={
          toFun := fun a ↦ x * a * x⁻¹
          map_add' := fun y1 y2 ↦ by simp only ; rw [mul_add, add_mul]
          map_smul' := fun d y ↦ by simp only [smul_eq_mul, RingHom.id_apply]; sorry
        }
      let id : Module.End D D :={
          toFun := id
          map_add' := fun x y ↦ by simp only [id_eq]
          map_smul' := fun d x ↦ by simp only [smul_eq_mul, id_eq, RingHom.id_apply]
        }
      have x_neq_0 : x ≠ 0 := by
          intro hx0
          rw [hx0] at hx
