...
lemma conj_char_pow_eq_one (x : D) (hx' : x ≠ 0)
  ...
  rw [← ((Subring.mem_center_iff (R := D)).1 hx d), mul_assoc, mul_inv_cancel (by aesop)]
  simp only [mul_one, sub_self]

lemma conj_one_compose (n : ℕ) (x y : D): (conj x - 1) (((conj x - 1) ^ n) y) =
    ((conj x - 1) ^ (n + 1)) y := by
  rw [add_comm, pow_add, pow_one]; simp
...
theorem division_char_is_commutative {D : Type*} [DivisionRing D] {p : ℕ} [Fact p.Prime] [CharP D p]
    ...
    mul_comm := by
      intro x
      suffices ∀ (y : D), y * x = x * y by intro y ; exact (this y).symm
      obtain ⟨m, hm⟩ := h x
      by_contra! hy
      cases' hy with y hy
      have hx : x ≠ 0 := by intro hx; simp_all
      have ineq1 : (conj x) - 1 ≠ 0 := by
        intro h1; obtain h1 := (DFunLike.ext_iff.1 h1) y
        simp only [LinearMap.sub_apply, LinearMap.coe_mk, AddHom.coe_mk, LinearMap.one_apply,
          LinearMap.zero_apply, sub_eq_zero] at h1
        apply hy; rw [← h1, mul_assoc, (inv_mul_cancel hx), h1, mul_one]
      obtain ⟨l, hl, hn⟩ := upper_bound p x hx ⟨m, hm⟩
      have h1 : ∃ b : D, (((conj x) - 1) ^ l) b ≠ 0 := by
        by_contra! hb; exact hl (DFunLike.ext_iff.2 hb)
      cases' h1 with b hb
      ...
      have hw0 : w ≠ 0 := by
        intro hw; simp only [w, z] at hw; apply hb
        rw [show l = 1 + (l - 1) by omega, pow_add] ; simp [hw]
      have hw : (conj x) w = w := by
        ...
        rw [conj_one_compose (l - 1) x b, show (l - 1) + 1 = l by omega, conj_one_compose,
      ...
      have hq_add : (conj x) q = q + 1 := by
        ...
        nth_rewrite 1 [← one_mul z, ← (inv_mul_cancel hx)]
        rw [← mul_assoc, ← mul_assoc, ← mul_assoc, this, mul_assoc, mul_assoc]
        nth_rewrite 2 [← mul_assoc]; simp only [h1, mul_add, (inv_mul_cancel hw0)]
      cases' h q with qm hq
      have final : q ^ p ^ (qm + 1) = q ^ p ^ (qm + 1) + 1 := by
        nth_rw 1 [← mul_one (q ^ p ^ (qm + 1)), ← (mul_inv_cancel hx), ← mul_assoc,
          ← (Subring.mem_center_iff.1 hq) x]
        calc
        _ = (x * q * x⁻¹) ^ p ^ (qm + 1) := by
          set e := p ^ (qm + 1)
          induction' e with e he
          · simp only [pow_zero, mul_one, (DivisionRing.mul_inv_cancel x hx)]
          · rw [pow_add, pow_add, pow_one, pow_one, ← he]
            nth_rewrite 2 [← one_mul (a := q)]; rw [← (inv_mul_cancel hx)]; group
        _ = (q + 1) ^ p ^ (qm + 1) := by
    ...
    mul_inv_cancel := by intro a ha ; exact ⟨a⁻¹, mul_inv_cancel ha⟩
...
lemma edison_lemma3 (d : D) {hff : f = minpoly K d}{m : ℕ}(g : K[X])
  ...
  { not_unit:= by
    ...
    isUnit_or_isUnit':= by
      ...
      rw [hq, hff] at this
      have irr_f := minpoly.irreducible (A := K) (x := d) (Algebra.IsIntegral.isIntegral d)
      obtain ⟨-, hf2⟩ := irr_f ; obtain ⟨ha, hb⟩ := hab
...
lemma findim_divring_over_sep_closed [Infinite K] (D : Type*)
  ...
  have p_rad : p_radical_extension K D p := by
    ...
    have g_irr : Irreducible g := edison_lemma3 p g hg (hff := rfl) --lemma 3
