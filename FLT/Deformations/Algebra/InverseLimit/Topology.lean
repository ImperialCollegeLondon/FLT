...
lemma minimumOpens_isSubbasis : instTopologicalSpace func = generateFrom (minimumOpens func) := by
  ...
  . intro ⟨i, W, ⟨hW₁, hW₂⟩⟩
    ...
    . use (fun (j : ι) ↦ by
      ...
      use { i }
      simp only [Finset.mem_singleton, eq_mp_eq_cast, Set.bot_eq_empty, forall_eq, ↓reduceDIte,
        cast_eq, Finset.coe_singleton, Set.singleton_pi]
      split_ands
      . exact hW₁
      . rw [hW₂]
        sorry
...
lemma minimumOpens_isOpen {s : Set (InverseLimit obj func)} : s ∈ minimumOpens func → IsOpen s := by
  rw [minimumOpens_isSubbasis]
  exact GenerateOpen.basic s
