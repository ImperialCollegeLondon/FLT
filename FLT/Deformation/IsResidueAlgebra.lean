...
def relativeResidueAlgebra (f : A →ₐ[𝓞] B) [isLocalHom : IsLocalHom f.toRingHom] :
    letI : Algebra A B := f.toAlgebra; IsResidueAlgebra A B :=
    ...
    isSurjective := by
      ...
      have hsurj1 : Surjective (modMap A B) := by
        have hcomp : (modMap A B) ∘ (modMap_high 𝓞 A) = (modMap_high 𝓞 B) := by
          ...
          have : modMap A B ∘ (IsLocalRing.residue A) = (IsLocalRing.residue B) ∘ f := by
            aesop
          rw [← comp_assoc, this, comp_assoc]
          have : f ∘ algebraMap 𝓞 A = algebraMap 𝓞 B := by
            aesop
          rw [this]
