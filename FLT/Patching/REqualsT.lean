...
-- Let `R` be a family of noetherian local `Λ`-algebras, compact with respect to the `𝔪_R`-adic topology.
variable {ι : Type*} (R : ι → Type*)
variable [∀ i, CommRing (R i)] [∀ i, IsLocalRing (R i)] [∀ i, IsNoetherianRing (R i)] [∀ i, Algebra Λ (R i)]
...
variable [Algebra.UniformlyBoundedRank R] -- For each `k`, the cardinality of `Rᵢ⧸(𝔪_Rᵢ)ᵏ` is uniformly bounded
variable [∀ i, Module.Free (Λ ⧸ Module.annihilator Λ (M i)) (M i)] -- `Mᵢ` is free over `Λ ⧸ Ann Mᵢ`.
...
-- Let `R₀` be a noetherian local ring, compact with respect to the `𝔪_R₀`-adic topology, and `M₀` an `R₀`-module.
...
variable (HCompat : ∀ i m (r : R i),
  sM i (Submodule.Quotient.mk (r • m)) = sR i (Ideal.Quotient.mk _ r) • sM i (Submodule.Quotient.mk m))
...
-- Let `T₀` be a ring that acts on `M₀`, `R₀ →+* T₀` be a ring homomorphism compatible with the module structure.
...
theorem ker_RtoT_le_nilradical : RingHom.ker RtoT ≤ nilradical R₀ := by
  ...
  simp only [Module.support_eq_zeroLocus, PrimeSpectrum.mem_zeroLocus, SetLike.coe_subset_coe] at this
