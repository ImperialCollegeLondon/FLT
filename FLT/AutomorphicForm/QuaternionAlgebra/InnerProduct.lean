...
def haar : MeasureTheory.Measure GL₂(𝔸ᶠ[F]) :=
  MeasureTheory.Measure.haarMeasure ⟨⟨GL2.maximalCompact F, GL2.maximalCompact.isCompact⟩,
    .mono GL2.maximalCompact.isOpen.interior_eq.superset (by simp)⟩

instance : (haar F).IsHaarMeasure := by delta haar; infer_instance
instance : (haar F).Regular := by delta haar; infer_instance
...
def toProdEigenform : anemic D 𝒮 →ₐ[R] Π (𝒻 : Eigenform D 𝒮), 𝒻.integers :=
  Pi.algHom _ _ fun 𝒻 ↦ 𝒻.toIntegers
...
lemma Eigenform.pi_lift_injective :
    Function.Injective ((Pi.algHom _ _ fun 𝒻 ↦
...
def tensorEquivPi : ℂ ⊗[R] anemic D 𝒮 ≃ₐ[ℂ] (Eigenform D 𝒮 → ℂ) := by
  refine .ofBijective (Pi.algHom _ _ fun 𝒻 ↦
...
def Eigenform.equivPrimesOver : Eigenform D 𝒮 ≃ (⊥ : Ideal R).primesOver (anemic D 𝒮) :=
  .ofBijective (fun 𝒻 ↦ ⟨𝒻.prime.asIdeal, inferInstance, ⟨𝒻.under_prime.symm⟩⟩) <| by
    refine ⟨?_, ?_⟩
    · intro 𝒻 𝒻' e
      simp only [Subtype.mk.injEq] at e
      sorry
    sorry
...
lemma Eigenform.exists_le_of_isPrime (P : Ideal (anemic D 𝒮)) [P.IsPrime] (hP : P.under R = ⊥) :
  ...
  have : P.map (Pi.algHom _ _ fun 𝒻 ↦ 𝒻.1 : anemic D 𝒮 →ₐ[R] (Eigenform D 𝒮 → ℂ)) ≠ ⊤ := by
