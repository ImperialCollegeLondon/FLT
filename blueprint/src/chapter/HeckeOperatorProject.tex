...
Say $\Gamma$ is a subgroup of $GL_2(k_v)$. Then $\Gamma$ is finite. Consider it as a submonoid of
the multiplicative monoid $M_2(k_v)$. Its preimage~$\tilde{\Gamma}$ in $M_2(\calO_v)$
...
Note that $\Gamma$ is actually a group, so $\Gamma=\Gamma^\times$, which is hence a compact
open subgroup of $\GL_2(K_v)$, again by lemmas~\ref{Submonoid.units_isOpen}
