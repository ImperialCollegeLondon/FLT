...
\begin{theorem}
  ...
  If $R=\bbC$ then the space $S_{W,\chi}(U;\bbC)$ is a finite-dimensional vector space.
\end{theorem}

This theorem is in fact an easy consequence of the following finiteness claim:
\begin{theorem}
  ...
  then the double coset space $D^\times\backslash(D\otimes_F\A_F^\infty)^\times/U$ is a finite set.
\end{theorem}

Let's prove finite-dimensionality assuming this theorem.
\begin{proof}
  Choose a set of coset representative $g_1,g_2,\ldots,g_n$ for
  $D^\times\backslash(D\otimes_F\A_F^\infty)^\times/U$. TODO
\end{proof}
