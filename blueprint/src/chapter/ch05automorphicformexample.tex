...
\begin{definition}\label{ZHat}\lean{ZHat}\leanok The profinite completion $\Zhat$ of $\Z$ is the set of 
    ...
    Such a collection is said to be \emph{compatible} if for all positive integers
...
\begin{lemma}
    ...
    \uses{ZHat}
    \leanok
    $\Zhat$ is a subring of $\prod_{N\geq1}(Z/N\Z)$ and in particular a ring.
\end{lemma}
...
Humanity started off studying concrete polynomial equations such as $x^2+y^2$ and then moved on to 
...
is a ``constructor'' for the type. In other words, how can we make elements of this type?
Well, given elements $a\in A$ and $b\in B$, we can form the element $a\otimes_t b\in A\otimes B$. 
