...
\begin{proof}
    ...
    In particular, if $L=\lcm(M,N)$ then $y_L$ is a multiple of both $M$ and $N$ and is
...
\end{lemma}
\begin{proof}
    Clearly $\Z\subseteq\Q\cap\Zhat$. Now suppose that $q\otimes_t z\in\Q\cap\Zhat$.
    Then $q\in\Q$ and $z\in\Zhat$, so $q\in\Z$ and $z\in\Z$. Thus $q\otimes_t z\in\Z$.
    Shouldn't be hard.
\end{proof}
