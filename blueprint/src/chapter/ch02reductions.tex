...
\begin{definition}[Frey]
  ...
  before him, Hellegouarch) is the elliptic curve $E$ defined by the equation
  $Y^2=X(X-a^p)(X+b^p).$
\end{definition}
...
\begin{proof}
  ...
  \notready
  This follows from a profound result of Mazur \cite{mazur-torsion} from 1979, namely the fact
  that the torsion subgroup of an elliptic curve over $\Q$ can have size at most~16.
  In fact a fair amount of work still needs to be done to deduce the theorem from Mazur's result.
  A pre-1990 reference for the proof of this claim is Proposition 6 in~\S4.1 of~\cite{serreconj}.
\end{proof}
...
\begin{theorem}[Wiles,Taylor--Wiles, Ribet,\ldots]
  ...
  \uses{FLT.FreyCurve}
  \leanok
  $\rho$ is not irreducible.\end{theorem}
\begin{proof}
