...
\begin{theorem} If $R$ is a coefficient ring with residue field of characteristic $\ell$,
  then there is a unique continuous map $\Z_\ell\to R$.
\end{theorem}
\begin{proof}
  It suffices to prove that there is a unique continuous map $\Z_\ell\to R/I$ for each
...
As a consequence, we can talk about the $\ell$-adic cyclotomic character $\GQ\to R^\times$.
