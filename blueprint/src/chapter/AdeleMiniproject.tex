...
\begin{proof}
  ...
  globally-defined $f_w$, (ii) an extension of local fields has degree $ef$.

  For transparency I'll leave the original proposal here too.
...
\begin{definition} There's a natural $L$-algebra homomorphism
  $L\otimes_K\A_{A,K}^\infty\to\A_{B,L}^\infty$.
\end{definition}

Our next goal in this section is the following result:
...
together. Formally, we really need to spell this out.

\subsection{Base change for nonarchimedean completions.}

Here are the theorems which we seem to need. As usual we are
...
\begin{definition} If $M$ is a $K$-module then there's a canonical
...
conjugatesof $b$ in some normal closure of $L$, and hence it is integral, and thus in $B$.
\end{proof}
...
\end{corollary}
\begin{proof} Follows immediately from lemma~\ref{templabel1}.
\end{proof}

\begin{corollary} The natural map $B\otimes_AK\to L$ is an isomorphism.
\end{corollary}
\begin{proof} It suffices to show that the bilinear map $B\times K\to L$ obeys the universal
property of the tensor product.
So let $N$ be an $A$-module and assume that we have a bilinear map $B\times K\to N$.
We need to show that this extends uniquely to an $A$-linear map $L\to N$. Uniqueness is
clear from the surjectivity proved in corollary~\ref{templabel2}. For uniqueness,
we first observe that if $b_1k_1=b_2k_2\in L$ then any $A$-bilinear map $\phi:B\times K\to N$
sends $(b_1,k_1)$ and $(b_2,k_2)$ to the same element. For we can write $k_1=x/a$ and $k_2=y/a$
with a common denominator $0\not=a\in A$ and $x,y\in A$, and then observe that
$b_1x=b_1k_1a=b_2k_2a=b_2y$ and that
$\phi(b_1,k_1)=\phi(b_1,x/a)=\phi(b_1x,1/a)=\phi(b_2y,1/a)=\phi(b_2,y/a)=\phi(b_2,k_2).$
So we can define our map $L\to N$ on $l\in L$ by setting $l=bk$ and sending it to $\phi(b,k)$;
this is independent of the decomposition, and $A$-linearity and additivity both follow
using similar sorts of arguments (I assume! Someone should check this before we start
formalizing it!)
\end{proof}

\begin{corollary} If $M$ is any $K$-module then the canonical map $B\otimes_A M\to L\otimes_K M$
  is an isomorphism.
\end{corollary}
\begin{proof} We can factor this map as $B\otimes_AM\equiv B\otimes_A(K\otimes_KM)\equiv
  (B\otimes_A K)\otimes_KM\to L\otimes_KM$ and we just showed that the latter map was an
  isomorphism.
\end{proof}
...
\begin{theorem} If $R$ is a commutative ring, if $M$ is a finitely presented $R$-module
...
\begin{corollary} If $S$ is a finite set of nonzero primes of $A$ then
  the natural map
...
\begin{corollary} The natural map $B\otimes_A\A_K^\infty\to R$ is an isomorphism.
\end{corollary}
...
So there is some natural isomorphism from the ring $R$ above to the restricted product of the $L_w$
with respect to the $B_w$ and this is a construction which we will have to abstract out.
Once we have done this, we can deduce

\begin{theorem} The natural map $B\otimes_A\A_K^\infty\to\A_L^\infty$ is an algebraic
  isomorphism
\end{theorem}
Because this map factors through the isomorphism $B\otimes_A\A_K^\infty\to L\otimes_K\A_K^\infty$
we deduce
\begin{corollary} The natural map $L\otimes_K\A_K^\infty\to\A_K^\infty$ is an algebraic
  isomorphism.
\end{corollary}
