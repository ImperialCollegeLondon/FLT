...
\begin{proof}
  ...
  $L_i=K_v[x]/(f_i)$. We have shown that $L\otimes_KK_v=\prod_i L_i$ so what we need to do
  informally is to identify the fields $L_i$ with the fields $L_w$. More formally, we need
  ...
  as $L\otimes_KK_v$-algebras, because (thinking of $L_i$ as $K_v(\alpha)$ such an isomorphism would send $x$ to $x$ and thus
  show $f_i=f_j$. Hence the map
