...
\begin{lemma}
  \label{MeasureTheory.isHaarMeasure_comap_of_isOpenEmbedding}
  \lean{MeasureTheory.isHaarMeasure_comap_of_isOpenEmbedding}
  \leanok
  \discussion{507}
...
\begin{lemma}
  \label{MeasureTheory.regular_comap_of_isOpenEmbedding}
  \lean{MeasureTheory.regular_comap_of_isOpenEmbedding}
  \leanok
  \discussion{513}
...
\begin{proof} Choose a regular Haar measure $\mu_B$ on $B$. We just saw
  in lemmas~\ref{MeasureTheory.isHaarMeasure_comap_of_isOpenEmbedding}
  and~\ref{MeasureTheory.regular_comap_of_isOpenEmbedding} that its pullback
...
adelic rings. So let $K$ be a number field and let $\A_K$ be the adeles of $K$.
...
\begin{corollary}
  \label{needslabel2}
  If $K\subseteq L$ are number fields and $V$ is an $L$-module, then
  the natural isomorphism $V\otimes_L\A_L=V\otimes_K\A_K$ induced by the natural
  isomorphism $\A_L=L\otimes_K\A_K$ is a homeomorphism if the left hand side has the $\A_L$-module
  topology and the right hand side has the $\A_K$-module topology.
\end{corollary}
\begin{proof}
...
That theorem only applies to inner forms of matrix algebras, but the below theorem,
