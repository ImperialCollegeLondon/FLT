...
Note that {\tt mathlib} offers a fixed Borel regular Haar measure
...
\begin{proof}
  ...
  $\phi^*\mu'(X)=d_A(\phi)\mu'(X)$
  cancel.
\end{proof}

Now let $\mu$ be any regular additive Haar measure on $A$.
Recall that if $\phi$ is a homeomorphism then the pullback measure
$\phi^*(\mu)$ is defined by $\phi^*(\mu)(X)=\mu(\phi(X))$.
...
\begin{lemma}
  ...
  or equivalently $\phi^*\mu(X)=d_A(\phi)\mu(X)$.
\end{lemma}
\begin{proof}
...
Note that as a consequence of lemma~\ref{MeasureTheory.addEquivAddHaarChar_mul_volume},
...
Similarly, by lemma~\ref{MeasureTheory.addEquivAddHaarChar_mul_integral},
...
\begin{proof}
  ...
  Choose a compact set $X$ containing an open neighbourhood of the identity
  (possible because $G$ is locally compact). Then $X$ has measure
  $0<\mu(X)<\infty$. Now we can calculate:
  $d_A(\phi\circ\psi)=\mu(\phi(\psi(X)))/\mu(X)=(\mu(\phi(\psi(X)))/\mu(\psi(X)))(\mu(\psi(X))/\mu(X))=d_A(\phi)d_A(\psi).$
\end{proof}

Instead of pulling back measures along isomorphisms, we can push them
forwards. Let $\phi_*\mu$ denote the pushforward of the Borel measure $\mu$
along a homeomorphism.

\begin{theorem}
  \lean{MeasureTheory.addEquivAddHaarChar_mul_volume}
  \uses{MeasureTheory.addEquivAddHaarChar_mul_volume}
  If $\phi:A\to A$ is an additive homeomorphism and $\mu$
  is a regular Haar measure on $A$ and $X\subseteq A$ is a Borel subset
  then $d_A(\phi)\phi_*\mu(X)=\mu(X)$.
\end{theorem}
\begin{proof}
  Set $Y=\phi^{-1}(X)$. Then $\mu(X)=\mu(\phi(Y))=d_A(\phi)\mu(Y)=d_A(\phi)\phi_*\mu(X)$
  by lemma~\ref{MeasureTheory.addEquivAddHaarChar_mul_volume}
\end{proof}

\begin{theorem}
  \uses{MeasureTheory.addEquivAddHaarChar_mul_integral}
  If $\phi:A\to A$ is an additive homeomorphism and $\mu$
  is a regular Haar measure on $A$ and $f:A\to\R$ is a Borel measurable
  function
  then $\int f(x)d\phi_*\mu(x)=d_A(\phi)\int f(x)d\mu(x).$
\end{theorem}
\begin{proof} Apply lemma~\ref{MeasureTheory.addEquivAddHaarChar_mul_integral}
  to the measure $\phi_*\mu$ to deduce $\int f(x)d\phi_*\mu(x)=d_A(\phi)\int f(d)d\phi^*\phi_*\mu(x)$
  and then observe that $\phi^*\phi_*\mu=\mu$.
\end{proof}
...
\begin{proof}
  \uses{MeasureTheory.addEquivAddHaarChar_mul_integral}
  A short calculation using lemma~\ref{MeasureTheory.addEquivAddHaarChar_mul_integral}.
\end{proof}

\begin{lemma}
  \label{MeasureTheory.ringHaarChar_mul_volume}
  \lean{MeasureTheory.ringHaarChar_mul_volume}
  \uses{MeasureTheory.ringHaarChar,MeasureTheory.addEquivAddHaarChar_mul_volume}
  \leanok
  \discussion{515}
