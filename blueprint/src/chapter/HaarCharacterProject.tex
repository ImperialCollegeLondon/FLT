...
\end{theorem}
\begin{proof} We think of $B_{\A}$ as $B\otimes_K\A_K$.
...
\begin{lemma}
  \label{adeledelta} If $x\in\A_{\Q}^\times$ then $\delta_{\A_{\Q}}(x)=\prod_v|x_v|_v.$
\end{lemma}
\begin{proof} By theorem~\ref{MeasureTheory.addEquivAddHaarChar_prodCongr}
  we have $\delta_{\A_{\Q}}(x)=\delta_{\A_{\Q}^\infty}(x^\infty)\times\delta_{\R}(x_\infty)$.
  By lemma~\ref{ringHaarChar_real} we have $\delta_{\R}(x_\infty)=|x|_\infty$, and by
  theorem~\ref{ringHaarChar_restricted_product} we have
  $\delta_{\A_{\Q}^\infty}=\prod_p\delta_{\Q_p}(x_p)$. By lemma~\ref{ringHaarChar_padic}
...
\begin{lemma}
  \label{productformula} If $x\in\Q^\times\subseteq\A_{\Q}^\times$ then $\delta_{\A_{\Q}}(x)=1.$
\end{lemma}
\begin{proof}
  By lemma~\ref{adeledelta} we have $\delta_{\A_{\Q}}(x)=\prod_v|x|_v$. But the product
  formula says that this is 1.
