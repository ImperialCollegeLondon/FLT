...
%   Right multiplication can have a different determinant to left multiplication.
...
%   then $\delta_B(u)=d_B(u_l)$ is equal to $d_B(u_r)$.
% \end{corollary}
% \begin{proof}
%   If $u_l$ and $u_r$ denote left and right multiplication by $u$ on $B$, then we have
%   seen in lemma~\ref{addHaarScalarFactor_eq_distribHaarChar_det} that $d_B(u_r)=\delta_F(\det(u_r))$.
