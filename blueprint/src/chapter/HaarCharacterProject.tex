...
KMB would like to heartily thank Sebastian Gouezel for the help he gave during the preparation
of this material. TODO accents

\section{Initial definitions}

\subsection{$d_A$}

TODO is this subsection name problematic?
...
\end{lemma}
\begin{proof} If $\mu'$ is a second choice then $\mu'=\lambda\mu$ for some
  positive real $\lambda$, and the $\lambda$s on each side of $\phi^*\mu'(X)=d_A(\phi)\mu'(X)$
  cancel.
\end{proof}

We list the two properties of $d_A(\phi)$ which we continually use.
