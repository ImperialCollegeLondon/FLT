...
\begin{lemma}
  \label{AdeleRing.DivisionAlgebra.Aux.existsE}
...
\begin{definition}
  \label{AdeleRing.DivisionAlgebra.Aux.E}
  \uses{AdeleRing.DivisionAlgebra.Aux.existsE}
  We let $E$ denote any compact set satisfying the hypothesis of the previous lemma.
\end{definition}

\begin{definition}
  \label{AdeleRing.DivisionAlgebra.Aux.X}
  \uses{AdeleRing.DivisionAlgebra.Aux.E}
Define $X:=E-E:=\{e-f:e,f\in E\}\subseteq D_{\A}$.
\end{definition}

\begin{definition}
  \label{AdeleRing.DivisionAlgebra.Aux.Y}
  \uses{AdeleRing.DivisionAlgebra.Aux.X}
Define $Y:=X.X:=\{xy:x,y\in X\}\subseteq D_{\A}$.
\end{definition}

\begin{lemma}
  \label{AdeleRing.DivisionAlgebra.Aux.X_compact}
  \uses{AdeleRing.DivisionAlgebra.Aux.X}
...
\begin{lemma}
  \label{AdeleRing.DivisionAlgebra.Aux.Y_compact}
  \uses{AdeleRing.DivisionAlgebra.Aux.Y}
...
\begin{lemma}
  \label{AdeleRing.DivisionAlgebra.Aux.X_meets_kernel}
  If $\beta\in D_{\A}^{(1)}$ then
$\beta X\cap D^\times\not=\emptyset$.
\end{lemma}
\begin{proof}
  \uses{AdeleRing.DivisionAlgebra.Aux.E}
Indeed by lemma~\ref{AdeleRing.DivisionAlgebra.Aux.existsE},
...
\end{lemma}
\begin{proof}
  \uses{AdeleRing.DivisionAlgebra.Aux.E, addHaarScalarFactor.left_mul_eq_right_mul}
...
\begin{lemma}
  \label{AdeleRing.DivisionAlgebra.Aux.Y_meet_units_D}
...
\begin{definition}
  \label{AdeleRing.DivisionAlgebra.Aux.T}
  Let $T:=Y\cap D^\times$ be this finite subset $Y\cap D^\times\subseteq D_{\A}$.
\end{definition}

\begin{definition}
  \label{AdeleRing.DivisionAlgebra.Aux.K}
  Define $K:= (T^{-1}.X) \times X\subset D_{\A}\times D_{\A}$.
\end{definition}

\begin{lemma}
  \label{AdeleRing.DivisionAlgebra.Aux.K_compact}
  \uses{AdeleRing.DivisionAlgebra.Aux.K, AdeleRing.DivisionAlgebra.Aux.T,
  AdeleRing.DivisionAlgebra.Aux.X_compact}
  $K$ is compact.
\end{lemma}
\begin{proof}
...
\begin{lemma}
  \label{AdeleRing.DivisionAlgebra.Aux.antidiag_mem_K}
  For every $\beta\in D_{\A}^{(1)}$, there exists $b\in D^\times$
  and $\nu\in D_{\A}^{(1)}$ such that $\beta=b\nu$ and $(\nu,\nu^{-1})\in K.$
\end{lemma}
\begin{proof}
  \uses{AdeleRing.DivisionAlgebra.Aux.E, AdeleRing.DivisionAlgebra.Aux.X_meets_kernel,
  AdeleRing.DivisionAlgebra.Aux.X_meets_kernel'}
  By lemma~\ref{AdeleRing.DivisionAlgebra.Aux.X_meets_kernel}, $\beta X\cap D^\times\not=\emptyset$,
  and lemma~\ref{AdeleRing.DivisionAlgebra.Aux..X_meets_kernel'}, $X\beta^{-1}\cap D^\times\not=\emptyset$,
...
\begin{proof}
  ...
  Lemma~\ref{AdeleRing.DivisionAlgebra.Aux.antidiag_mem_K} shows that $M$ surjects onto
